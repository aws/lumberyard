...
.\%BINFOLDER%\AssetProcessorBatch.exe /gamefolder=FeatureTests /platforms=ios
IF ERRORLEVEL 1 GOTO AssetProcessingFailed

echo ----- Creating Packages ----
rem lowercase is intentional, since cache folders are lowercase
.\%BINFOLDER%\rc\rc.exe /job=%BINFOLDER%\rc\RCJob_Generic_MakePaks.xml /p=ios /game=featuretests /trg=cache\featuretests\ios_paks
