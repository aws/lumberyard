...
// the 3d Engine dll (which loads particle files)
// actually needs named type info for these types.
// until particle loading from xml can be removed from the 3d engine
// we must include type info names in order to be able to load
// xml text that contains things like (x=0.2, y=0.5...)
// This must be defined in any module that includes typeInfo_impl.h
// and parses files with that kind of format inside it.
