...
namespace mesh_compiler
{
    ...
    static void debugDumpMesh(CMesh& mesh, const char* filename)
    {
        ...
        SMeshTexCoord* texCoords = mesh.GetStreamPtr<SMeshTexCoord>(CMesh::TEXCOORDS, 1);
        if (texCoords)
        {
            fprintf(f, "[TexCoord UV Set 1]\n");
            for (int i = 0; i < mesh.GetTexCoordCount(); ++i)
            {
                const Vec2 uv = texCoords[i].GetUV();
                fprintf(f, "\t%g %g\n", uv.x, uv.y);
            }
        }
