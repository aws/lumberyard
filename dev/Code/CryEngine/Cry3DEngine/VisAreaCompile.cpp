...
// Segmented World
#if ENGINE_ENABLE_COMPILATION
int CVisArea::GetSegmentData(byte*& pData, int& nDataSize, std::vector<IStatObj*>* pStatObjTable, std::vector<_smart_ptr<IMaterial>>* pMatTable, std::vector<IStatInstGroup*>* pStatInstGroupTable, EEndian eEndian, SHotUpdateInfo* pExportInfo)
{
    CRY_ASSERT(false); // SegmentedWorld has been removed
    return false;
}
#endif
int CSWVisArea::Load(byte*& f, int& nDataSizeLeft, int nSID, std::vector<IStatObj*>* pStatObjTable, std::vector<_smart_ptr<IMaterial>>* pMatTable, EEndian eEndian, SHotUpdateInfo* pExportInfo, const Vec2& indexOffset)
{
    int objBlockSize = 0;
    if (!LoadHeader_T(f, nDataSizeLeft, eEndian, objBlockSize))
    {
