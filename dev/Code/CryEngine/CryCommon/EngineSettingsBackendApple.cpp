...
bool CEngineSettingsBackendApple::StoreEngineSettingsToRegistry()
{
    ...
    // Engine Specific
    if (parent()->GetValueByRef("ENG_RootPath", SettingsManagerHelpers::CWCharBuffer(buffer, sizeof(buffer))))
    {
        m_registry->setStrValue(REG_BASE_SETTING_KEY, L"ENG_RootPath", buffer);
    }
    else
    {
        bRet = false;
    ...
    if (parent()->GetValueByRef("ENG_BinFolder", SettingsManagerHelpers::CWCharBuffer(buffer, sizeof(buffer))))
    {
        m_registry->setStrValue(REG_BASE_SETTING_KEY, L"ENG_BinFolder", buffer);
    }
    else
    {
        bRet = false;
...
void CEngineSettingsBackendApple::LoadEngineSettingsFromRegistry()
{
    ...
    if (m_registry->getStrValue(REG_BASE_SETTING_KEY, L"ENG_BinFolder", wStrResult))
    {
        parent()->SetKey("ENG_BinFolder", wStrResult.c_str());
    }
