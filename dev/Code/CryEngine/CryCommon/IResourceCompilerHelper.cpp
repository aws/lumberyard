...
void IResourceCompilerHelper::GetOutputFilename(const char* szFilePath, char* buffer, size_t bufferSizeInBytes)
{
    if (IResourceCompilerHelper::IsSourceImageFormatSupported(szFilePath))
    {
        ...
        strncpy_s(buffer, bufferSizeInBytes, newString.c_str(), bufferSizeInBytes - 1);
        return;
    }

    strncpy_s(buffer, bufferSizeInBytes, szFilePath, bufferSizeInBytes - 1);
}
...
IResourceCompilerHelper::ERcCallResult IResourceCompilerHelper::InvokeResourceCompiler(const char* szSrcFilePath, const char* szDstFilePath, const bool bUserDialog)
{
    ...
    {
        strcat_s(szFullPathToSourceFile, maxStringSize, "#ENGINEROOT#");
        strcat_s(szFullPathToSourceFile, maxStringSize, "\\");
    }
    strcat_s(szFullPathToSourceFile, maxStringSize, szSrcFilePath);

    strcat_s(szRemoteCmdLine, maxStringSize, " /targetroot=\"");
    strcat_s(szRemoteCmdLine, maxStringSize, pathOnly.c_str());
    strcat_s(szRemoteCmdLine, maxStringSize, "\"");

    strcat_s(szRemoteCmdLine, maxStringSize, " /overwritefilename=\"");
    strcat_s(szRemoteCmdLine, maxStringSize, szDstFileName);
    strcat_s(szRemoteCmdLine, maxStringSize, "\"");

    return CallResourceCompiler(szFullPathToSourceFile, szRemoteCmdLine, nullptr, true, IResourceCompilerHelper::eRcExePath_registry, false, !bUserDialog);
}
...
bool IResourceCompilerHelper::IsSourceImageFormatSupported(const char* szFileNameOrExtension)
{
    ...
    {
        if (!_stricmp(szExtension, GetSourceImageFormat(i, szExtension[0] == '.')))
        {
            return true;
        }
    }
...
bool IResourceCompilerHelper::IsGameImageFormatSupported(const char* szFileNameOrExtension)
{
    ...
    {
        if (!_stricmp(szExtension, GetEngineImageFormat(i, szExtension[0] == '.')))
        {
            return true;
        }
    }
