...
void CFFont::RenderCallback(float x, float y, float z, const char* pStr, const bool asciiMultiLine, const STextDrawContext& ctx)
{
    ...
    for (size_t j = 0, numPasses = fx.m_passes.size(); j < numPasses; ++j)
    {
        ...
        pRenderer->FontSetBlending(pPass->m_blendSrc, pPass->m_blendDest, ctx.m_baseState);
        ...
        while (uint32_t ch = *pChar)
        {
            ...
            int horizontalAdvance = m_pFontTexture->GetHorizontalAdvance(ch);
            float advance;
    ...
    pRenderer->FontRestoreRenderingState(ctx.m_overrideViewProjMatrices, backupSceneMatrices);
}
...
void CFFont::AddCharsToFontTexture(const char* pChars)
{
    AZStd::lock_guard<AZStd::recursive_mutex> locker(m_fontMutex);
    Prepare(pChars, false);
}
...
{
    bool texUpdateNeeded = m_pFontTexture->PreCacheString(pStr) == 1 || m_fontTexDirty;
