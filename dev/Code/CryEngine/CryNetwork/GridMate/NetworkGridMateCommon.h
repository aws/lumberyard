...
namespace GridMate
{
    ...
    /*!
     * Helpers for grabbing common interfaces.
     */
    inline IGameFramework* GetGameFramework()
    {
        return (gEnv && gEnv->pGame) ? gEnv->pGame->GetIGameFramework() : nullptr;
    }
    ...
    inline IGameRulesSystem* GetGameRulesSystem()
    {
        if (auto* gameFramework = GetGameFramework())
        {
            return gameFramework->GetIGameRulesSystem();
        }

        return nullptr;
    }

    inline IGameObjectSystem* GetGameObjectSystem()
    {
        if (auto* gameFramework = GetGameFramework())
        {
            return gameFramework->GetIGameObjectSystem();
        }

        return nullptr;
    }

    inline IGameRules* GetGameRules()
    {
        if (auto* gameRulesSystem = GetGameRulesSystem())
        {
            return gameRulesSystem->GetCurrentGameRules();
        }

        return nullptr;
    }

    inline IActorSystem* GetActorSystem()
    {
        if (auto* gameFramework = GetGameFramework())
        {
            return gameFramework->GetIActorSystem();
        }

        return nullptr;
    }

    inline IEntitySystem* GetEntitySystem()
    {
        return gEnv ? gEnv->pEntitySystem : nullptr;
    }
