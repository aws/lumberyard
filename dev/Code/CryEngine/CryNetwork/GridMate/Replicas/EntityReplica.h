...
namespace GridMate
{
    ...
    public:
        ...
        */
        ...
        //! Returns the native params struct wrapped by our m_spawnParams data set.
        SEntitySpawnParams GetEngineSpawnParams() const;
    ...
    private: 
        ...
        //! Called when the replica is activated. For level entities, we need to
        //! retrieve the already-existing entity. For dynamic entities, we need
        //! to spawn locally.
        IEntity* EstablishLocalEntity();

        //! Spawns the local IEntity that this replica replicates (should not be invoked on the master)
        //! or in the case of level entities.
        IEntity* SpawnLocalEntity() const;

        //! Stores the Id of the *local* entity corresponding to this replica.
        //! It's critical to note that the Id may not match the server's.
        void AssignLocalEntity(IEntity* entity);

        //! For the master, handles gathering new data for dirtied aspects and triggering to send.
        //! For the proxy, handles dispatching new data to game layer (GameObject, etc).
        void UpdateAspects();

        //! Helper function to serialize a single aspect to/from game
        void SerializeAspect(TSerialize ser, IEntity* entity, IGameObject* gameObject, EEntityAspects aspect);
