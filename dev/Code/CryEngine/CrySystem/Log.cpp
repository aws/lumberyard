...
void CLog::LogV(const ELogType type, int flags, const char* szFormat, va_list args)
{
    ...
    if (bufferlen > 0)
    {
        ...
        #include "Salem/Log_cpp_salem.inl"
    #endif
#endif
...
//////////////////////////////////////////////////////////////////////
#if !defined(EXCLUDE_NORMAL_LOG)
void CLog::LogStringToFile(const char* szString, ELogType logType, bool bAdd, MessageQueueState queueState)
{
    ...
    if (m_pLogIncludeTime && gEnv && gEnv->pTimer)
    {
        ...
        else if (dwCVarState == 5)             // Log_IncludeTime
        {
            if ((gEnv) && (gEnv->pGame) && (gEnv->pGame->GetIGameFramework()))
            {
                ...
                timeStr.clear();
                timeStr.Format("<%.2f> ", serverTime.GetSeconds());
                tempString = timeStr + tempString;
            }
