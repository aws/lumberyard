...
void CLocalMemoryUsage::CheckCharacterP1(ICharacterInstance* pCharacter, IRenderNode* pRenderNode, _smart_ptr<IMaterial> pSlotMat, AABB bounding, float maxViewDist, float scale, int nMaxDepth)
{
    ...
    if (pAttMan)
    {
        ...
        for (int i = 0; i < nCount; i++)
        {
            if (IAttachment* pAtt = pAttMan->GetInterfaceByIndex(i))
            {
                ...
                {
                    _smart_ptr<IMaterial> pAttMatOverride = pAttObj->GetReplacementMaterial();
                    _smart_ptr<IMaterial> pAttMat = pSlotMat ? pSlotMat : (pAttMatOverride ? pAttMatOverride : pAttObj->GetBaseMaterial());
                    ...
                    if (pIAttachmentSkin)
                    {
                        _smart_ptr<IMaterial> pCharObjMat = pIAttachmentSkin->GetISkin()->GetIMaterial(0);
