    ...
    const StreamReadParams* params)
{
    ...
    retReq->m_buffer = azmalloc(fileSize);
    if (params)
    {
...
{
    {
        ...
        if (m_fileRequest)
        {
            AZ::IO::Streamer::Instance().CancelRequestAsync(m_fileRequest);
        }
...
void AZRequestReadStream::SetPriority(EStreamTaskPriority ePriority)
{
    ...
    if (m_params.ePriority != ePriority)
    {
        ...
        if (m_fileRequest && (m_fileRequest->m_state == AZ::IO::Request::StateType::ST_PENDING))
        {
            AZ::IO::Streamer::Instance().RescheduleRequest(m_fileRequest,
                CStreamEngine::CryStreamPriorityToAZStreamPriority(ePriority),
                AZ::IO::ExecuteWhenIdle);
        }
    }
}
...
void AZRequestReadStream::Wait(int maxWaitMillis)
{
    ...
    {
        AZ_Error("AZRequestReadStream", false, "Stream for file %s is unwaitable", m_fileName);
        return;
    }
...
void AZRequestReadStream::FreeTemporaryMemory()
{
    ...
    if (!m_fileRequest || m_fileRequest->HasCompleted())
    {
        azfree(m_buffer);
        m_buffer = nullptr;
        m_numBytesRead = 0;
...
//////////////////////////////////////////////////////////////////////////
void AZRequestReadStream::OnRequestComplete(AZ::IO::SizeType numBytesRead, void* buffer, AZ::IO::Request::StateType requestState)
{
    CryAutoCriticalSection lock(m_callbackLock);
    ...
    if (!m_isFileRequestComplete)
    {
        ...
        case AZ::IO::Request::StateType::ST_ERROR_FAILED_TO_OPEN_FILE:
            m_IOError = ERROR_CANT_OPEN_FILE;
            m_numBytesRead = 0;
            m_isError = true;
            break;
        default:
