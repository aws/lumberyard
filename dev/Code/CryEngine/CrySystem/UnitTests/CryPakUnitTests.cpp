...
TEST_F(Integ_CryPakUnitTests, TestCryPakViaFileIO)
{
    ...
    cpfio.FindFiles("@assets@", "*.xml", [&foundIt](const char* foundName)
        {
            if (azstricmp(foundName, "testfile.xml") == 0)
            {
                foundIt = true;
                return false;
            }
...
TEST_F(Integ_CryPakUnitTests, TestCryPakFolderAliases)
{
    ...
    EXPECT_TRUE(pArchive->UpdateFile("foundit.dat", "test", 4, ICryArchive::METHOD_COMPRESS, ICryArchive::LEVEL_BEST) == 0);
