...
namespace
{
    ...
    static inline void RelinkTail(util::list<CRenderMesh>& instance, util::list<CRenderMesh>& list, int threadId) ///< Added thread id parameter so we can determine if we're modifying the list for the fill thread (the render thread must lock for this case as worker threads are also using it)
    {
        ...
        bool doConditionalLock = !isRenderThread || threadId == gRenDev->m_pRT->CurThreadFill() || CRenderer::CV_r_multithreaded == 0;  ///< Always lock if the thread id is the current fill thread as other threads can access it
...
void* CRenderMesh::LockVB(int nStream, uint32 nFlags, int nVerts, int* nStride, bool prefetchIB, bool inplaceCachePos)
{
...
size_t CRenderMesh::SetMesh_Int(CMesh& mesh, int nSecColorsSetOffset, uint32 flags)
{
...
byte* CRenderMesh::GetHWSkinPtr(int32& nStride, uint32 nFlags, bool remapped)
{
...
bool CRenderMesh::RT_CheckUpdate(CRenderMesh* pVContainer, uint32 nStreamMask, bool bTessellation, bool stall)
{
    MEMSTAT_CONTEXT(EMemStatContextTypes::MSC_RenderMeshType, 0, this->GetTypeName());
    MEMSTAT_CONTEXT(EMemStatContextTypes::MSC_RenderMesh, 0, this->GetSourceName());
...
const PodArray<std::pair<int, int> >* CRenderMesh::GetTrisForPosition(const Vec3& vPos, _smart_ptr<IMaterial> pMaterial)
{
#ifdef RENDER_MESH_TRIANGLE_HASH_MAP_SUPPORT
...
bool CRenderMesh::GetRemappedSkinningData(uint32 guid, CRendElementBase::SGeometryStreamInfo &streamInfo)
{
#if !defined(NULL_RENDERER)
