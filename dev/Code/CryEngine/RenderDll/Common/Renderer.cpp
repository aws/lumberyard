...
{
#if !defined(OPENGL)
    if (gRenDev && gRenDev->GetGPUParticleEngine())
    {
        gRenDev->GetGPUParticleEngine()->OnCubeDepthMapResolutionChanged(pCVar);
    }
#endif
}
...
#include AZ_RESTRICTED_FILE(Renderer_cpp, AZ_RESTRICTED_PLATFORM)
     ...
     * Setting this variable to 1 will cause the output to be resolved into an RGBA8 buffer
     * after deferred shading when in the LDR GMEM path.
     * Trade off is that we are going from a floating point buffer into an unormalized one.
     * However this will cause opaque and transparent passes to be rendered in a new command encoder.
     * This might be useful in some situation when the app is GPU bound.
     */
    DefineConstIntCVar3("r_GMEM_LDR_ForceResolvePostComposition", CV_r_GMEM_LDR_ForceResolvePostComposition, 0, VF_NULL,
        "Mobile LDR GMEM Path\n"
        "Usage: \n"
        "0 disable\n"
...
void CRenderer::InitSystemResources(int nFlags)
{
    ...
    if (!m_bSystemResourcesInit || m_bDeviceLost == 2)
    {
        ...
        if (m_gpuParticleEngine != nullptr)
        {
            m_gpuParticleEngine->Initialize(this);
        }
...
void CRenderer::FreeResources(int nFlags)
{
    ...
    {
        // GPUParticleEngine's resources are shaders. We ensure they are released here;
        // if not, the ShaderResources they hold are considered leaks in the ShaderMan shutdown
        // logic which deletes the resources no matter their ref count; without this Release,
        // future GPUParticleEngine Release()s on empty-level game load/shutdown would try to
        // release the dangling ShaderResource pointers
...
bool CRenderer::EF_ReloadFile(const char* szFileName)
{
    ...
    else if (szExt && !azstricmp(szExt, ".cgf"))
    {
        ...
        if (pStatObjectToReload)
        {
            ...
            EBUS_EVENT_ID(pStatObjectToReload, StatObjEventBus, OnStatObjReloaded);
            return true;
        }
        return false;
    }
...
CRenderObject* CRenderer::EF_AddPolygonToScene(SShaderItem& si, CRenderObject* obj, const SRenderingPassInfo& passInfo, int numPts, int ninds, SVF_P3F_C4B_T2F*& verts, SPipTangents*& tangs, uint16*& inds, int nAW, const SRendItemSorter& rendItemSorter)
{
    ASSERT_IS_MAIN_THREAD(m_pRT)
    int nThreadID = m_RP.m_nFillThreadID;
    const uint32 nPersFlags = m_RP.m_TI[nThreadID].m_PersFlags;
...
void CRenderer::EF_AddClientPolys(const SRenderingPassInfo& passInfo)
{
#if !defined(NULL_RENDERER)
    AZ_TRACE_METHOD();
    uint32 i;
...
//=============================================================================
// Precaching
bool CRenderer::EF_PrecacheResource(IRenderMesh* _pPB, _smart_ptr<IMaterial> pMaterial, float fMipFactor, float fTimeToReady, int nFlags, int nUpdateId)
{
    int i;
    ...
    if (CRenderer::CV_r_rain == 2 && !bDisableOcclusion)
    {
        ...
        if (bUpdateOcc)
        {
                ...
                || fOccTreshold != CRenderer::CV_r_rainOccluderSizeTreshold)
            {
                ...
                const AZ::u32 numBrushOccluders = gEnv->p3DEngine->GetObjectsByTypeInBox(eERType_Brush, bbArea);

                numAllOccluders = numAZStaticMeshOccluders + numAZSkinnedMeshOccluders + numBrushOccluders;
                ...
                //Retrieve all brushes in bounding box after AZ components
                //because we prefer AZ components over legacy entities
                if (numBrushOccluders > 0)
                {
...
void CRenderer::RT_UpdateLightVolumes(int32 nFlags, int32 nRecurseLevel)
{
    AZ_TRACE_METHOD();
