...
void CShaderMan::mfInitLevelPolicies(void)
{
    ...
    AZ::IO::HandleType fileHandle = gEnv->pCryPak->FOpen(szN, "rb", ICryPak::FOPEN_HINT_QUIET);
    if (fileHandle != AZ::IO::InvalidHandle)
    {
...
void CShaderMan::mfInit (void)
{
    ...
    if (!m_bInitialized)
    {
        ...
        gEnv->pCryPak->OpenPack("@assets@", "Shaders.pak", ICryPak::FLAGS_PAK_IN_MEMORY);
        ...
        gEnv->pCryPak->OpenPack("@assets@", "shaderCache.pak", ICryPak::FLAGS_PAK_IN_MEMORY);
#endif
        ParseShaderProfiles();
