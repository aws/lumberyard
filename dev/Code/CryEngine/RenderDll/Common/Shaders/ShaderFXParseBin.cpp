    ...
    uint32 nSourceCRC32, const char* szName, bool bInclude, AZ::IO::HandleType srcFileHandle)
{
    ...
    AZ::IO::HandleType dstFileHandle = gEnv->pCryPak->FOpen(szFileName, "wb", ICryPak::FLAGS_NEVER_IN_PAK | ICryPak::FLAGS_PATH_REAL | ICryPak::FOPEN_ONDISK);
    if (dstFileHandle != AZ::IO::InvalidHandle)
    {
        ...
        Header.m_nOffsetParamsLocal = gEnv->pCryPak->FTell(dstFileHandle);
...
bool CShaderManBin::SaveBinShaderLocalInfo(SShaderBin* pBin, uint32 dwName, uint64 nMaskGenFX, uint64 maskGenStatic, TArray<int32>& Funcs, std::vector<SFXParam>& Params, std::vector<SFXSampler>& Samplers, std::vector<SFXTexture>& Textures)
{
    ...
    AZ::IO::HandleType binFileHandle = gEnv->pCryPak->FOpen(pBin->m_szName, "r+b", ICryPak::FLAGS_NEVER_IN_PAK | ICryPak::FLAGS_PATH_REAL | ICryPak::FOPEN_ONDISK);
    ...
    int nSeek = gEnv->pCryPak->FTell(binFileHandle);
    CRY_ASSERT(nSeek > 0);
    if (nSeek <= 0)
    {
        return false;
...
SShaderBin* CShaderManBin::GetBinShader(const char* szName, bool bInclude, uint32 nRefCRC, bool* pbChanged)
{
    ...
    for (; i < n; i++)
    {
        ...
        {
            dstFileHandle = gEnv->pCryPak->FOpen(szDst.c_str(), "rb", ICryPak::FLAGS_NEVER_IN_PAK | ICryPak::FLAGS_PATH_REAL | ICryPak::FOPEN_ONDISK);
        }
        if (dstFileHandle == AZ::IO::InvalidHandle)
        {
            continue;
