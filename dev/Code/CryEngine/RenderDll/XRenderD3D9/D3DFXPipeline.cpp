...
void CD3D9Renderer::FX_ClearTarget(SDepthTexture* pTex, const int nFlags, const float cDepth, const uint8 cStencil, const uint numRects, const RECT* pRects, const bool bOptional)
{
    assert((
            (nFlags & CLEAR_ZBUFFER ? D3D11_CLEAR_DEPTH : 0) |
            (nFlags & CLEAR_STENCIL ? D3D11_CLEAR_STENCIL : 0)
...
bool CD3D9Renderer::FX_GetTargetSurfaces(CTexture* pTarget, D3DSurface*& pTargSurf, SRTStack* pCur, int nCMSide, int nTarget, uint32 nTileCount)
{
    if (pTarget)
    {
...
void CD3D9Renderer::FX_FlushShader_General()
{
    ...
    if (pTech)
    {
        ...
        if (ef->m_eShaderType == eST_Particle)
        {
            ...
            if ((objFlags & (FOB_PARTICLE_SHADOWS)))
            {
