...
bool CREParticle::mfPreDraw(SShaderPass* sl)
{
    ...
    if (rp.m_RendNumVerts)
    {
        ...
        rp.m_PS[rp.m_nProcessThreadID].m_DynMeshUpdateBytes += rp.m_RendNumIndices * sizeof(uint16);
         ...
         * Following is a hack! For some reason, a stream remains in slot 1 which causes
         * a crash on iOS.
         * Either DX is smart enough not to use that slot or DX path cleans up slot 1 properly.
         * TODO: investigate further
         */
#ifdef CRY_USE_METAL
        rd->FX_SetVStream(1, NULL, 0, 0);
#endif
...
bool CREParticle::mfDraw(CShader* ef, SShaderPass* sl)
{
    ...
    {
        if (CD3D9Renderer::CV_r_VolumetricFog != 0
...
{
    // predraw is not called for customdraw/sky based render calls.
    return true;
}
...
bool CREParticleGPU::mfDraw(CShader* ef, SShaderPass* sl)
{
    ...
    gRenDev->GetGPUParticleEngine()->Render(m_instance, m_pass, m_shadowMode, m_cameraFOV, m_aspectRatio, m_isWireframeEnabled);
