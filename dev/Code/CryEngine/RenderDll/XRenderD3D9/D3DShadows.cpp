...
bool CD3D9Renderer::PrepareShadowGenForFrustum(ShadowMapFrustum* pCurFrustum, SRenderLight* pLight, int nLightFrustumBaseID, int nLOD)
{
    int nThreadID  = m_RP.m_nFillThreadID;
...
//-----------------------------------------------------------------------------
void CD3D9Renderer::PrepareShadowGenForFrustumNonJobs(const int nFlags)
{
    int nThreadID  = m_RP.m_nFillThreadID;
    int nCurRecLevel = SRendItem::m_RecurseLevel[nThreadID];
    assert(nCurRecLevel >= 0);
...
void CD3D9Renderer::FX_SetupShadowsForTransp()
{
    ...
    FX_SetupForwardShadows(bLegacyShaderPermutations);
}
