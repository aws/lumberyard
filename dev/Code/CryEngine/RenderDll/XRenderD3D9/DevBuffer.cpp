    ...
    public:
        ...
        void EndReadWrite()
        {
            ...
            if (m_resources.m_staged_open[StagingResources::WRITE])
            {
                ...
                contents.left = m_resources.m_staged_offset;
                contents.right = m_resources.m_staged_offset + m_resources.m_staged_size;
                contents.top = 0;
                contents.bottom = 1;
                contents.front = 0;
                contents.back = 1;
                ...
                gcpRendD3D->GetDeviceContext().CopySubresourceRegion(
                    m_resources.m_staged_buffer, 0, m_resources.m_staged_offset, 0, 0, m_uploadBuffer, 0, &contents);
...
#    else
    ...
    {
    public:
        DirectBufferUpdater(StagingResources& resources)
        {}
...
namespace
{
    ...
    public:
        ...
        virtual void Free(BufferPoolItem* item) { }
        ...
        virtual void* BeginRead(BufferPoolItem* item) { return NULL; }
        virtual void* BeginWrite(BufferPoolItem* item) { return NULL; }
        virtual void  EndReadWrite(BufferPoolItem* item, bool requires_flush) {}
        virtual void Write(BufferPoolItem* item, const void* src, size_t size) { __debugbreak(); }
...
////////
        ...
        void EndReadWrite(BufferPoolItem* item, bool requires_flush)
        {
            ...
            IF (item->m_bank != ~0u, 1)
            {
                ...
                IF (CRenderer::CV_r_buffer_enable_lockless_updates, 1)
                {
...
# if ENABLE_STATOSCOPE
    class CStatoscopeDevBufferStats
    ...
    public:
        ...
        virtual void Write(IStatoscopeFrameRecord& fr)
        {
            ...
            fr.AddValue(poolManager.m_sdata[1].m_written_bytes / 1024.f);
            fr.AddValue(poolManager.m_sdata[1].m_read_bytes / 1024.f);
            fr.AddValue((float)(poolManager.m_sdata[1].m_creator_time * rfreq));
            fr.AddValue((float)(poolManager.m_sdata[1].m_io_time * rfreq) * 1000.f);
            fr.AddValue((float)(poolManager.m_sdata[1].m_cpu_flush_time * rfreq) * 1000.f);
            fr.AddValue((float)(poolManager.m_sdata[1].m_gpu_flush_time * rfreq) * 1000.f);

#           if CONSTANT_BUFFER_ENABLE_DIRECT_ACCESS
            size_t cbsize =
                poolManager.m_constant_allocator.m_pages;
...
#     endif
            ...
            memset(&poolManager.m_sdata[1], 0, sizeof(poolManager.m_sdata[1]));
        }
    };

    class CStatoscopeDevBufferDG
        : public IStatoscopeDataGroup
    {
        SDeviceBufferPoolStats m_stats[BBT_MAX][BU_MAX];
    public:
...
# if ENABLE_STATOSCOPE
    poolManager.m_sdata[1].m_written_bytes += poolManager.m_sdata[0].m_written_bytes;
    poolManager.m_sdata[1].m_read_bytes += poolManager.m_sdata[0].m_read_bytes;
    poolManager.m_sdata[1].m_creator_time += poolManager.m_sdata[0].m_creator_time;
    poolManager.m_sdata[1].m_io_time += poolManager.m_sdata[0].m_io_time;
    poolManager.m_sdata[1].m_cpu_flush_time += poolManager.m_sdata[0].m_cpu_flush_time;
    poolManager.m_sdata[1].m_gpu_flush_time += poolManager.m_sdata[0].m_gpu_flush_time;
    memset(&poolManager.m_sdata[0], 0, sizeof(poolManager.m_sdata[0]));
# endif
    ...
    AzRHI::ConstantBufferFlags flags)
{
    ...
    STATOSCOPE_TIMER(poolManager.m_sdata[0].m_creator_time);
...
SStatoscopeData& GetStatoscopeData(uint32 nIndex)
{
    return PoolManager::GetInstance().m_sdata[nIndex];
}
#   endif
/////////////////////////////////////////////////////////////
// Legacy interface
...
#   endif
    ...
    void* ConstantBuffer::BeginWrite()
    {
        ...
        STATOSCOPE_TIMER(poolManager.m_sdata[0].m_io_time);
...
void WrappedDX11Buffer::Create(uint32 numElements, uint32 elementSize, DXGI_FORMAT elementFormat, uint32 flags, const void* pData, int32 nESRAMOffset /*=-1*/)
{
