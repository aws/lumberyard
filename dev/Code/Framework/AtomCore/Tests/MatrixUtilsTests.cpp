...
namespace UnitTest
{
    ...
    TEST_F(MatrixUtilsTests, PerspectiveMatrixFovRH)
    {
        ...
        MakePerspectiveFovMatrixRH(matrix, fovY, aspectRatio, near, far, true);
        ...
        EXPECT_FALSE(MakePerspectiveFovMatrixRH(matrix, fovY, -1, near, far));
        EXPECT_EQ(1, UnitTest::TestRunner::Instance().m_numAssertsFailed);
        EXPECT_FALSE(MakePerspectiveFovMatrixRH(matrix, 0, aspectRatio, near, far));
        EXPECT_EQ(2, UnitTest::TestRunner::Instance().m_numAssertsFailed);
        EXPECT_FALSE(MakePerspectiveFovMatrixRH(matrix, fovY, aspectRatio, -near, far));
        EXPECT_EQ(3, UnitTest::TestRunner::Instance().m_numAssertsFailed);
        EXPECT_FALSE(MakePerspectiveFovMatrixRH(matrix, fovY, aspectRatio, near, -far));
        EXPECT_EQ(4, UnitTest::TestRunner::Instance().m_numAssertsFailed);
        EXPECT_FALSE(MakePerspectiveFovMatrixRH(matrix, fovY, aspectRatio, 0, far));
    ...
    TEST_F(MatrixUtilsTests, OrthographicMatrixRH)
    {
        ...
        EXPECT_TRUE(IsClose(result.GetX(), 0, floatEpsilon));
        EXPECT_TRUE(IsClose(result.GetY(), 0, floatEpsilon));
        EXPECT_TRUE(IsClose(result.GetZ(), 0.5f, floatEpsilon));

        // left top and far
        result = MatrixTransformPosition(matrix, Vector3(-100, 200, -1000));
        EXPECT_TRUE(IsClose(result.GetX(), -1, floatEpsilon));
        EXPECT_TRUE(IsClose(result.GetY(), 1, floatEpsilon));
        EXPECT_TRUE(IsClose(result.GetZ(), 1, floatEpsilon));
    ...
    TEST_F(MatrixUtilsTests, FrustumMatrixRH)
    {
        ...
        // further than far
        Vector3 far(-50, 150, -1000);
        result = MatrixTransformPosition(matrix, far);
        EXPECT_TRUE(IsClose(result.GetZ(), 1, floatEpsilon));
