...
namespace AZ
{
    ...
    inline ConsoleDataWrapper<BASE_TYPE, THREAD_SAFETY>::ConsoleDataWrapper(const BASE_TYPE& value, CallbackFunc callback, const char* name, const char* desc, ConsoleFunctorFlags flags)
    :   m_callback(callback)
    ,   m_functor(name, desc, flags, *this, &ConsoleDataWrapper<BASE_TYPE, THREAD_SAFETY>::CvarFunctor)
    {
        this->m_value = value;
    }
    ...
    template <typename BASE_TYPE, ThreadSafety THREAD_SAFETY>
    bool ConsoleDataWrapper<BASE_TYPE, THREAD_SAFETY>::StringToValue(const StringSet& arguments)
    {
        const BASE_TYPE currentValue = this->m_value;
    ...
    template <typename BASE_TYPE, ThreadSafety THREAD_SAFETY>
    void ConsoleDataWrapper<BASE_TYPE, THREAD_SAFETY>::ValueToString(AZStd::string& outString) const
    {
        const BASE_TYPE currentValue = this->m_value;
    ...
    template <typename BASE_TYPE, ThreadSafety THREAD_SAFETY>
    inline void ConsoleDataWrapper<BASE_TYPE, THREAD_SAFETY>::CvarFunctor(const StringSet& arguments)
    {
