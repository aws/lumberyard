...
#   define idxof(_i) _i
         ...
         * @return output
         */
        AZ_FORCE_INLINE static __m128i mm_recursion(__m128i* a, __m128i* b,  __m128i c, __m128i d, __m128i mask)
        {
            __m128i v, x, y, z;
            x = _mm_load_si128(a);
        ...
        inline void gen_rand_all(Sfmt& g)
        {
            ...
            r1 = _mm_load_si128(&g.m_sfmt[N - 2].si);
            r2 = _mm_load_si128(&g.m_sfmt[N - 1].si);
            for (i = 0; i < N - POS1; i++)
            {
                r = mm_recursion(&g.m_sfmt[i].si, &g.m_sfmt[i + POS1].si, r1, r2, mask);
                _mm_store_si128(&g.m_sfmt[i].si, r);
                r1 = r2;
                r2 = r;
            }
            for (; i < N; i++)
            {
                r = mm_recursion(&g.m_sfmt[i].si, &g.m_sfmt[i + POS1 - N].si, r1, r2, mask);
                _mm_store_si128(&g.m_sfmt[i].si, r);
                r1 = r2;
                r2 = r;
            }
        ...
        inline void gen_rand_array(Sfmt& g, w128_t* array, int size)
        {
            ...
            for (; i < N; i++)
            {
                r = mm_recursion(&g.m_sfmt[i].si, &array[i + POS1 - N].si, r1, r2, mask);
                _mm_store_si128(&array[i].si, r);
                r1 = r2;
                r2 = r;
            }
            ...
            for (j = 0; j < 2 * N - size; j++)
            {
                ...
                _mm_store_si128(&g.m_sfmt[j].si, r);
            }
            for (; i < size; i++)
            {
                r = mm_recursion(&array[i - N].si, &array[i + POS1 - N].si, r1, r2, mask);
...
{
    AZ_Assert(m_index == SfmtInternal::N32, "Invalid m_index! Reinitialize!");
    AZ_Assert(size % 4 == 0, "Size must be multiple of 4!");
    AZ_Assert(size >= SfmtInternal::N32, "Size must be bigger than %d GetMinArray32Size()!", SfmtInternal::N32);
...
Sfmt::FillArray64(AZ::u64* array, int size)
{
    AZ_Assert(m_index == SfmtInternal::N32, "Invalid m_index! Reinitialize!");
    AZ_Assert(size % 4 == 0, "Size must be multiple of 4!");
    AZ_Assert(size >= SfmtInternal::N64, "Size must be bigger than %d GetMinArray64Size()!", SfmtInternal::N64);
