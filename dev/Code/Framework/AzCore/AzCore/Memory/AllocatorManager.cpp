...
namespace AZ
{
    namespace Internal
    {
        static AZ::EnvironmentVariable<MallocSchema> s_mallocSchema = nullptr;

        static MallocSchema& GetMallocSchema()
        {
            if (!s_mallocSchema)
            {
                AZ_Assert(Environment::IsReady(), "Environment must be ready before calling GetMallocSchema()");
                s_mallocSchema = AZ::Environment::CreateVariable<MallocSchema>(AZ_CRC("AZ::AllocatorManager::Internal::s_mallocSchema", 0x6f4242c5));
            }

            return *s_mallocSchema;
        }

        class AZStdMallocAllocator : public AZStdIAllocator
        {
        public:
            AZStdMallocAllocator(MallocSchema* schema = &GetMallocSchema()) : AZStdIAllocator(schema)
            {
            }
        };

        typedef AZStd::basic_string<char, AZStd::char_traits<char>, AZStdMallocAllocator> AMString;
        typedef AZStd::unordered_map<AMString, IAllocator*, AZStd::hash<AMString>, AZStd::equal_to<AMString>, AZStdMallocAllocator> AllocatorNameMap;
        typedef AZStd::unordered_map<AMString, AMString, AZStd::hash<AMString>, AZStd::equal_to<AMString>, AZStdMallocAllocator> AllocatorRemappings;
    ...
    : m_profilingRefcount(0)
{
    ...
    m_data = new (Internal::GetMallocSchema().Allocate(sizeof(InternalData), AZStd::alignment_of<InternalData>::value, 0)) InternalData;
}
...
AllocatorManager::InternalDestroy()
{
    ...
    {
        m_data->~InternalData();
        Internal::GetMallocSchema().DeAllocate(m_data);
        m_data = nullptr;
    }
...
{
    auto record = m_data->m_allocatorMap.emplace(alloc->GetName(), alloc);
    ...
    {
        // All allocators must switch to an OverrideEnabledAllocationSource proxy if they are to support allocator overriding.
        auto overrideEnabled = Internal::AllocatorOverrideShim::Create(alloc, &Internal::GetMallocSchema());
...
AllocatorManager::AddAllocatorRemapping(const char* fromName, const char* toName)
{
    ...
    m_data->m_remappings.emplace(fromName, toName);
    m_data->m_remappingsReverse.emplace(toName, fromName);
#endif
}
