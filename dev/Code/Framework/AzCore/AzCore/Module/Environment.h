...
namespace AZ
{
    ...
    namespace Internal
    {
        ...
        */
            ...
            : public EnvironmentVariableHolderBase
        {
            ...
            void DestructImpl(const AZStd::true_type& /* AZStd::has_trivial_constructor<T> */)
            {
                // do nothing
            }

            void DestructImpl(const AZStd::false_type& /* AZStd::has_trivial_constructor<T> */)
            {
        ...
        public:
            ...
            void Construct()
            {
                ...
                if (!m_isConstructed)
                {
                    ConstructImpl(typename AZStd::has_trivial_constructor<T>::type());
            ...
            void DestructNoLock()
            {
                ...
                m_isConstructed = false;
                m_moduleOwner = nullptr;
                DestructImpl(typename AZStd::has_trivial_constructor<T>::type());
            }
    ...
    {
    public:
        typedef typename Internal::EnvironmentVariableHolder<T> HolderType;
        ...
        T& operator*() const
        {
            AZ_Assert(m_data != nullptr, "You can't dereference a null pointer");
        ...
        typedef HolderType* EnvironmentVariable::* unspecified_bool_type;
        operator unspecified_bool_type() const {
            return m_data == nullptr ? nullptr : &EnvironmentVariable::m_data;
        }
    ...
    {
        return CreateVariable<T>(Internal::EnvironmentVariableNameToId(uniqueName), AZStd::forward<Args>(args)...);
    }
    ...
    EnvironmentVariable<T>  Environment::CreateVariableEx(u32 guid, bool isConstruct, bool isTransferOwnership, Args&&... args)
    {
        ...
        Internal::EnvironmentVariableResult result = Internal::AddAndAllocateVariable(guid, sizeof(HolderType), AZStd::alignment_of<HolderType>::value, &addLock);
        if (result.m_state == Internal::EnvironmentVariableResult::Added)
        {
            variable = new(result.m_variable)HolderType(guid, isTransferOwnership, Internal::GetAllocator());
        }
        else if (result.m_state == Internal::EnvironmentVariableResult::Found)
        {
    ...
    {
        typedef typename EnvironmentVariable<T>::HolderType HolderType;
        Internal::EnvironmentVariableResult result = Internal::GetVariable(guid);
        if (result.m_state == Internal::EnvironmentVariableResult::Found)
        {
