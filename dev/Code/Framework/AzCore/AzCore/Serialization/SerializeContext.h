...
namespace AZ
{
    ...
    public:
        ...
        */
        ...
        template<class T>
        bool EnumerateObject(const T* obj, const BeginElemEnumCB& beginElemCB, const EndElemEnumCB& endElemCB, unsigned int accessFlags, ErrorHandler* errorHandler = nullptr);
        ...
        struct ClassElement
        {
            ...
            AZStd::unordered_map<AttributeId, Attribute*> m_attributes; ///< Attributes attached to ClassElement
            int m_flags;    ///<
        };
    ...
    */
    ...
    template<class T>
    bool SerializeContext::EnumerateObject(const T* obj, const BeginElemEnumCB& beginElemCB, const EndElemEnumCB& endElemCB, unsigned int accessFlags, ErrorHandler* errorHandler)
    {
        AZ_Assert(!AZStd::is_pointer<T>::value, "Cannot enumerate pointer-to-pointer as root element! This makes no sense!");
    ...
    SerializeContext::ClassBuilder* SerializeContext::ClassBuilder::Field(const char* name, FieldType ClassType::* member, AZStd::initializer_list<AttributePair> attributes)
    {
        ...
        ed.m_attributes.insert(attributes);

        if (ed.m_genericClassInfo)
        {
            auto specializedTypeId = ed.m_genericClassInfo->GetSpecializedTypeId();
            m_context->m_uuidGenericMap.emplace(specializedTypeId, ed.m_genericClassInfo);
            m_context->m_uuidAnyCreationMap.emplace(specializedTypeId, &AnyTypeInfoConcept<ValueType>::CreateAny);
            m_context->m_classNameToUuid.emplace(AZ::Crc32(ed.m_name), specializedTypeId);
        }
