    ...
    public:
            ...
            , m_end(0)
        {
            if (numElements > 0)
            {
                ...
                AZStd::uninitialized_fill_n(m_start, numElements, value, Internal::is_fast_fill<pointer>());
                m_last  = m_end;
            }
        }
            ...
            : m_allocator(rhs.m_allocator)
        {
            ...
            {
                m_start = reinterpret_cast<pointer>(m_allocator.allocate(byteSize, alignment_of<node_type>::value));
                m_last  = AZStd::uninitialized_copy(rhs.m_start, rhs.m_last, m_start, has_trivial_copy<value_type>());
            }
            else
            {
                m_start = 0;
        ...
        this_type& operator=(const this_type& rhs)
        {
            ...
            if (newSize > capacity)
            {
                ...
                m_start = reinterpret_cast<pointer>(m_allocator.allocate(byteSize, alignment_of<node_type>::value));
                m_last  = AZStd::uninitialized_copy(rhs.m_start, rhs.m_last, m_start, has_trivial_assign<value_type>());
                m_end   = m_last;
            }
            else
            {
                ...
                if (size >= newSize)
                {
                    if (newSize > 0)
                    {
                        ...
                        pointer newLast = Internal::copy(rhs.m_start, rhs.m_last, m_start, has_trivial_assign<value_type>());
                ...
                {
                    pointer newLast = Internal::copy(rhs.m_start, rhs.m_start + size, m_start, has_trivial_assign<value_type>());

                    m_last = AZStd::uninitialized_copy(rhs.m_start + size, rhs.m_last, newLast, has_trivial_assign<value_type>());
                }
            }
        ...
        void reserve(size_type numElements)
        {
            ...
            if (numElements > capacity)
            {
                ...
                pointer newLast = AZStd::uninitialized_move(m_start, m_last, newStart, Internal::is_fast_copy<pointer, pointer>());
        ...
        AZ_FORCE_INLINE void resize(size_type newSize)
        {
            ...
            if (size < newSize)
            {
                ...
                Internal::construct<pointer, value_type, AZStd::false_type>::range(m_last, newLast);
                m_last = newLast;
            }
        ...
        */
        ...
        void    insert(const_iterator insertPos, size_type numElements, const_reference value)
        {
            ...
            if (capacity < newSize)
            {
                ...
                pointer newLast = AZStd::uninitialized_move(m_start, insertPosPtr, newStart, Internal::is_fast_copy<pointer, pointer>());
                // add new data
                AZStd::uninitialized_fill_n(newLast, numElements, value, Internal::is_fast_fill<pointer>());

                newLast += numElements;
                // Copy the elements after the insert position.
                newLast = AZStd::uninitialized_move(insertPosPtr, m_last, newLast, Internal::is_fast_copy<pointer, pointer>());
        ...
        inline iterator erase(const_iterator elementIter)
        {
            ...
            Internal::move(element + 1, m_last, element, Internal::is_fast_copy<pointer, pointer>());
            --m_last;
        ...
        void                set_capacity(size_type numElements)
        {
            ...
            if (capacity != numElements)
            {
                ...
                if (numElements > 0)
                {
                    ...
                    if (numMoved > 0)
                    {
                        newLast  = AZStd::uninitialized_move(m_start, m_start + numMoved, newStart, Internal::is_fast_copy<pointer, pointer>());
                    }
                    else
                    {
                        newLast = newStart;
    ...
    private:
        ...
        void insert_iter(const_iterator insertPos, const Iterator& first, const Iterator& last, const forward_iterator_tag&)
        {
            ...
            if (capacity <  newSize)
            {
                ...
                newLast = AZStd::uninitialized_copy(first, last, newLast, Internal::is_fast_copy<Iterator, pointer>());
                // Copy the elements after the insert position.
                newLast = AZStd::uninitialized_move(insertPosPtr, m_last, newLast, Internal::is_fast_copy<pointer, pointer>());
            ...
            else if (size_type(m_last - insertPosPtr) < numElements)
            {
                ...
                pointer newLast = AZStd::uninitialized_move(insertPosPtr, m_last, insertPosPtr + numElements, Internal::is_fast_copy<pointer, pointer>());
            ...
            else
            {
                ...
                // first copy the data that will not overlap.
                pointer nonOverlap = m_last - numElements;
                pointer newLast = AZStd::uninitialized_move(nonOverlap, m_last, m_last, Internal::is_fast_copy<pointer, pointer>());
