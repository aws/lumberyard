...
namespace UnitTest
{
    ...
    void Test_WrapperFunctions_MutableIterator_MutableContainer()
    {
        ...
        EXPECT_EQ(*prior(iter_end), 9);
        EXPECT_EQ(*prior(iter_end, 2), 8);
        --iter_end;
        ...
        EXPECT_EQ(*prior(iter_rend), 0);
        EXPECT_EQ(*prior(iter_rend, 2), 1);
        --iter_rend;
    ...
    void Test_WrapperFunctions_ConstIterator_MutableContainer()
    {
        ...
        EXPECT_EQ(*prior(iter_cend), 9);
        EXPECT_EQ(*prior(iter_cend, 2), 8);
        --iter_cend;
        ...
        EXPECT_EQ(*prior(iter_crend), 0);
        EXPECT_EQ(*prior(iter_crend, 2), 1);
        --iter_crend;
    ...
    {            
        ...
        EXPECT_EQ(*prior(end(int_array)), 29);
        EXPECT_EQ(*prior(end(int_array), 2), 28);

        EXPECT_EQ(*rbegin(int_array), 29);
        EXPECT_EQ(*next(rbegin(int_array)), 28);
        EXPECT_EQ(*next(rbegin(int_array), 2), 27);

        EXPECT_EQ(*prior(rend(int_array)), 20);
        EXPECT_EQ(*prior(rend(int_array), 2), 21);

        EXPECT_EQ(*crbegin(int_array), 29);
        EXPECT_EQ(*next(crbegin(int_array)), 28);
        EXPECT_EQ(*next(crbegin(int_array), 2), 27);

        EXPECT_EQ(*prior(crend(int_array)), 20);
        EXPECT_EQ(*prior(crend(int_array), 2), 21);
    ...
    TEST_F(Iterators, FunctionWrappers_ConstRawArray)
    {
        ...
        EXPECT_EQ(*prior(cend(const_int_array)), 39);
        EXPECT_EQ(*prior(cend(const_int_array), 2), 38);
    }
}