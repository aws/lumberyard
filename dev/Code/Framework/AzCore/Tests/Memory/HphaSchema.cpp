...
namespace UnitTest
{
    ...
    TEST_P(HphaSchemaTestFixture, Allocate)
    {
        ...
        for (size_t i = 0; i < totalNumberOfAllocations; ++i)
        {
            ...
            void* allocation = AZ::AllocatorInstance<TestAllocator>::Get().Allocate(allocationSize, 0);
            EXPECT_NE(nullptr, allocation);
            EXPECT_LE(allocationSize, AZ::AllocatorInstance<TestAllocator>::Get().AllocationSize(allocation));
        ...
        {
            AZ::AllocatorInstance<TestAllocator>::Get().DeAllocate(allocations[i], testParameters.m_allocationSizes[i % testParameters.m_allocationSizes.size()]);
        }
    }
...
namespace Benchmark
{
    ...
    public:
        ...
        static void BM_Allocations(benchmark::State& state, const AllocationSizeArray& allocationArray)
        {
            ...
            {
                AZ::AllocatorInstance<TestAllocator>::Get().DeAllocate(allocations[allocationIndex], allocationArray[allocationIndex % allocationArray.size()]);
            }
            AZ::AllocatorInstance<TestAllocator>::Get().GarbageCollect();
        }
    };
