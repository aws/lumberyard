...
namespace UnitTest
{
    ...
    public:
        ...
        TestAllocator()
            : Base("TestAllocator", "Allocator for Test")
        {}
    };
    ...
    {
    public:
        AZ_CLASS_ALLOCATOR(TestClass, TestAllocator, 0);
...
AZ_PUSH_DISABLE_WARNING(4700, "-Wuninitialized")
    TEST_F(HphaSchemaErrorDetectionTest, InvalidDelete)
    {
        ...
        delete leakyObject; // this will free with TestAllocator (not AnotherTestAllocator)
...
AZ_POP_DISABLE_WARNING
    ...
    {
        TestClass<1>* leakyObject = (TestClass<1>*)AZ::AllocatorInstance<TestAllocator>::Get().Allocate(sizeof(TestClass<1>), AZStd::alignment_of<TestClass<1>>::value);

        leakyObject->~TestClass<1>();
        AZ::AllocatorInstance<TestAllocator>::Get().DeAllocate(leakyObject, sizeof(TestClass<1>), AZStd::alignment_of<TestClass<1>>::value);
    }
    ...
    {
        // Make an allocation
        TestClass<1>* someObject = (TestClass<1>*)AZ::AllocatorInstance<TestAllocator>::Get().Allocate(sizeof(TestClass<1>), AZStd::alignment_of<TestClass<1>>::value);

        // Make a reallocation, this usually is don internally in a container. Allocators do not support new[]
        const size_t newSize = AZ::AllocatorInstance<TestAllocator>::Get().Resize(someObject, 2 * sizeof(TestClass<1>));
        EXPECT_EQ(8, newSize); // it will extend to 8

        AZ::AllocatorInstance<TestAllocator>::Get().DeAllocate(someObject, newSize);
    }
    ...
    TEST_F(HphaSchemaErrorDetectionTest, Reallocation)
    {
        ...
        // Make a reallocation, this usually is don internally in a container. Allocators do not support new[]
        someObject = (TestClass<1>*)AZ::AllocatorInstance<TestAllocator>::Get().ReAllocate(someObject, 2 * sizeof(TestClass<1>), AZStd::alignment_of<TestClass<1>>::value);

        AZ::AllocatorInstance<TestAllocator>::Get().DeAllocate(someObject, 2 * sizeof(TestClass<1>));
    }
}
