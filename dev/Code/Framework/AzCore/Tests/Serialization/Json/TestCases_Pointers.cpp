...
namespace JsonSerializationTests
{
    ...
    bool SharedPointer::Equals(const SharedPointer& rhs, bool fullReflection) const
    {
        ...
        if (azrtti_typeid(m_pointer.get()) != azrtti_typeid(rhs.m_pointer.get()))
        {
            return false;
        }

        if (azrtti_typeid(m_pointer.get()) != azrtti_typeid<SimpleInheritence>())
        {
            return false;
        }

        return static_cast<SimpleInheritence*>(m_pointer.get())->Equals(
            *static_cast<SimpleInheritence*>(rhs.m_pointer.get()), fullReflection);
    }

    void SharedPointer::Reflect(AZStd::unique_ptr<AZ::SerializeContext>& context, bool fullReflection)
    {
        SimpleInheritence::Reflect(context, fullReflection);
        context->Class<SharedPointer>()
            ->Field("pointer", &SharedPointer::m_pointer);
    }

    InstanceWithSomeDefaults<SharedPointer> SharedPointer::GetInstanceWithSomeDefaults()
    {
        auto instance = AZStd::make_unique<SharedPointer>();
        auto member = AZStd::make_shared<SimpleInheritence>();
        member->m_var2 = 88.0f;
        instance->m_pointer = AZStd::move(member);
    ...
    InstanceWithoutDefaults<SharedPointer> SharedPointer::GetInstanceWithoutDefaults()
    {
        auto instance = AZStd::make_unique<SharedPointer>();
        auto member = AZStd::make_shared<SimpleInheritence>();
        member->m_baseVar = -88.0;
        member->m_var1 = 88;
        member->m_var2 = 88.0f;
        instance->m_pointer = AZStd::move(member);
