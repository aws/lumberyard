...
namespace AzFramework
{
    ...
    AZ::Debug::Result LogComponent::OnPrintf(const AZ::Debug::TraceMessageParameters& parameters)
    {
        if (azstrnicmp(parameters.window, "debug", 5) == 0)
        {
            ...
            OutputMessage(LogFile::SEV_DEBUG, parameters.window, parameters.message);
            return AZ::Debug::Result::Handled;
        }

        OutputMessage(LogFile::SEV_NORMAL, parameters.window, parameters.message);

        return AZ::Debug::Result::Continue;
    }


    AZ::Debug::Result LogComponent::OnAssert(const AZ::Debug::TraceMessageParameters& parameters)
    {
        OutputMessage(LogFile::SEV_ASSERT, "*ASSERT*", parameters.message);

        return AZ::Debug::Result::Continue;
    }

    AZ::Debug::Result LogComponent::OnWarning(const AZ::Debug::TraceMessageParameters& parameters)
    {
        OutputMessage(LogFile::SEV_WARNING, parameters.window, parameters.message);

        return AZ::Debug::Result::Continue;
    }

    AZ::Debug::Result LogComponent::OnError(const AZ::Debug::TraceMessageParameters& parameters)
    {
        OutputMessage(LogFile::SEV_ERROR, parameters.window, parameters.message);

        return AZ::Debug::Result::Continue;
    }

    AZ::Debug::Result LogComponent::OnException(const AZ::Debug::TraceMessageParameters& parameters)
    {
        OutputMessage(LogFile::SEV_EXCEPTION, "*EXCEPTION*", parameters.message);
    ...
    void LogComponent::SetMachineReadable(bool newValue)
    {
        ...
        bool isActive = ((this->GetEntity()) && (this->GetEntity()->GetState() == AZ::Entity::ES_ACTIVE));
        if (isActive)
        {
