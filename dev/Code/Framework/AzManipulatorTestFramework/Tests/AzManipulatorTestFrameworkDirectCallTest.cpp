...
#include "AzManipulatorTestFrameworkDirectCallTest.h"

namespace UnitTest
{
    TEST_F(AzManipulatorTestFrameworkCustomManagerTest, RegisterManipulatorWithCustomManager)
    {
        // Expect the manager to contain one manipulator
        EXPECT_EQ(m_manipulatorManager->RegisteredManipulatorCount(), 1);
    }

    TEST_F(AzManipulatorTestFrameworkCustomManagerTest, ConsumeViewportLeftMouseClick)
    {
        // given a left mouse down ray in world space
        const auto interaction = CreateMouseInteraction(
            m_mouseStartingPositionRay,
            AzToolsFramework::ViewportInteraction::MouseButton::Left);

        // consume the mouse down and up events
        m_manipulatorManager->ConsumeViewportMousePress(interaction);
        m_manipulatorManager->ConsumeViewportMouseRelease(interaction);
    ...
    TEST_F(AzManipulatorTestFrameworkCustomManagerTest, ConsumeViewportMouseMoveHover)
    {
        // given a left mouse down ray in world space
        const auto interaction = CreateMouseInteraction(
            m_mouseStartingPositionRay,
        ...
        m_manipulatorManager->ConsumeViewportMouseMove(interaction);
    ...
    TEST_F(AzManipulatorTestFrameworkCustomManagerTest, ConsumeViewportMouseMoveActive)
    {
        // given a left mouse down ray in world space
        const auto interaction = CreateMouseInteraction(
            m_mouseStartingPositionRay,
    ...
    TEST_F(AzManipulatorTestFrameworkCustomManagerTest, MoveManipulatorAlongAxis)
    {
        ...
        // given a left mouse down ray in world space
        auto interaction = CreateMouseInteraction(
            m_mouseStartingPositionRay,
            AzToolsFramework::ViewportInteraction::MouseButton::Left);
        ...
        interaction.m_mousePick.m_rayOrigin += expectedPositionAfterMovementAlongAxis;
        m_manipulatorManager->ConsumeViewportMouseMove(interaction);
