...
namespace AzToolsFramework
{
    namespace ComponentModeFramework
    {
        ...
        void ComponentModeCollection::BeginComponentMode()
        {
            ...
            RecordMetrics(&EditorMetricsEventsBusTraits::EnteredComponentMode);
        ...
        void ComponentModeCollection::EndComponentMode()
        {
            ...
            RecordMetrics(&EditorMetricsEventsBusTraits::LeftComponentMode);
        ...
        void ComponentModeCollection::RecordMetrics(MetricFn metricFn)
        {
            AZStd::vector<AZ::EntityId> entityIds;
            entityIds.reserve(m_entitiesAndComponentModeBuilders.size());

            AZStd::unordered_set<AZ::Uuid> componentTypeIds;
            // note: reserve size here is not exactly the right size but is better
            // than no reserve in the common case
            componentTypeIds.reserve(m_entitiesAndComponentModeBuilders.size());

            for (const auto& entityComponentModeBuilders : m_entitiesAndComponentModeBuilders)
            {
                // store all entities that entered ComponentMode
                entityIds.push_back(entityComponentModeBuilders.m_entityId);
                for (const auto& componentModeBuilder : entityComponentModeBuilders.m_componentModeBuilders)
                {
                    // store all unique component types that entered ComponentMode
                    componentTypeIds.insert(componentModeBuilder.m_componentType);
                }
            }

            EditorMetricsEventsBus::Broadcast(
                metricFn, entityIds, AZStd::vector<AZ::Uuid>(componentTypeIds.begin(), componentTypeIds.end()));
        }
    } // namespace ComponentModeFramework
} // namespace AzToolsFramework