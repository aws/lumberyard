...
namespace AzToolsFramework
{
    ...
    {
        // Manage child process ids by using a map that is updated by the child signal handler
        std::mutex              s_childProcessMutex;
        std::map<pid_t,int>     s_processDataByChildId;

        /*! Signal handler for child processes that are completed
         *
         * \param signum - The signum identifier of the signal received
         */
        void childProcessSignalHandler(int signum)
        {
            ...
            while (true)
            {
                ...
                if (pid==-1)
                {
                    ...
                    {
                        // This indicates there are no child processes left
                        break;
                    }
                    else
                    {
                        AZ_TracePrintf("ProcessWatcher", "childProcessSignalHandler terminated prematurely (waitpid errno %d). assuming process either failed to launch or terminated unexpectedly\n", errno);
                        break;
                    }
                }
                else if (pid==0)
                ...
                {
                    std::lock_guard<std::mutex> lock(s_childProcessMutex);
                    s_processDataByChildId[pid] = wait_status;
                }
            }
        }
        ...
        {
            // Rely on the map that manages the completed child process ids to know if the child process is done or not
            std::lock_guard<std::mutex> lock(s_childProcessMutex);
            std::map<pid_t,int>::iterator processResultIter = s_processDataByChildId.find(static_cast<pid_t>(childProcessId));
            if (processResultIter == s_processDataByChildId.end())
            {
                return false;
            }
            int wait_result = processResultIter->second;

            if (WIFEXITED(wait_result))
            {
                if (outExitCode)
                {
                    *outExitCode = static_cast<AZ::u32>(WEXITSTATUS(wait_result));
                }
            }
            else
            {
                if (outExitCode)
                {
                    AZ_TracePrintf("ProcessWatcher", "Child process id %d terminated abnormally. assuming process either failed to launch or terminated unexpectedly\n", childProcessId);
                    *outExitCode = 1; // Since the process terminated abnormally, return a general error status code
                }
            }
            return true;
        }
    ...
    bool ProcessLauncher::LaunchProcess(const ProcessLaunchInfo& processLaunchInfo, ProcessData& processData)
    {
        ...
        signal(SIGCHLD, childProcessSignalHandler);
