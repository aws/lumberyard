...
namespace AzToolsFramework
{
    ...
    namespace Internal
    {
        ...
        bool PropertyRead(DestType& value, AZ::Edit::Attribute* attr, void* instance, Args&& ... args)
        {
            // try a value
            AZ::Edit::AttributeData<AttrType>* data = azdynamic_cast<AZ::Edit::AttributeData<AttrType>*>(attr);
            if (data)
            {
            ...
            AZ::Edit::AttributeFunction<AttrType(Args...)>* func = azdynamic_cast<AZ::Edit::AttributeFunction<AttrType(Args...)>*>(attr);
            if (func)
            {
     ...
     * The same applied for flating point types, we can convert float <-> double, but we don't allow to ints
    ...
    {
        static bool Read(DestType& value, AZ::Edit::Attribute* attr, void* instance, Args&& ... args)
        {
            return Internal::PropertyRead<AttrType, DestType>(value, attr, instance, AZStd::forward<Args>(args) ...);
        }
    };

    template<class AttrType, class DestType, typename ... Args >
    struct PropertyReaderArgs
    {
        static bool Read(DestType& value, AZ::Edit::Attribute* attr, void* instance, Args&& ... args)
        {
    ...
    template<class AttrType, class DestType, typename ... Args>
    struct PropertyReader<AttrType, DestType, true, false, Args...>
    {
        static bool Read(DestType& value, AZ::Edit::Attribute* attr, void* classInstance, Args&& ... args)
        {
            if (Internal::PropertyRead<bool, DestType, Args...>(value, attr, classInstance, AZStd::forward<Args>(args) ...))
            {
                return true;
            }
            if (Internal::PropertyRead<char, DestType, Args...>(value, attr, classInstance, AZStd::forward<Args>(args) ...))
            {
                return true;
            }
            if (Internal::PropertyRead<unsigned char, DestType, Args...>(value, attr, classInstance, AZStd::forward<Args>(args) ...))
            {
                return true;
            }
            if (Internal::PropertyRead<short, DestType, Args...>(value, attr, classInstance, AZStd::forward<Args>(args) ...))
            {
                return true;
            }
            if (Internal::PropertyRead<unsigned short, DestType, Args...>(value, attr, classInstance, AZStd::forward<Args>(args) ...))
            {
                return true;
            }
            if (Internal::PropertyRead<int, DestType, Args...>(value, attr, classInstance, AZStd::forward<Args>(args) ...))
            {
                return true;
            }
            if (Internal::PropertyRead<unsigned int, Args..., DestType>(value, attr, classInstance, AZStd::forward<Args>(args) ...))
            {
                return true;
            }
            if (Internal::PropertyRead<AZ::s64, DestType, Args...>(value, attr, classInstance, AZStd::forward<Args>(args) ...))
            {
                return true;
            }
            if (Internal::PropertyRead<AZ::u64, DestType, Args...>(value, attr, classInstance, AZStd::forward<Args>(args) ...))
            {
                return true;
            }
            if (Internal::PropertyRead<AZ::Crc32, DestType, Args...>(value, attr, classInstance, AZStd::forward<Args>(args) ...))
            {
                return true;
            }
            return false;
    ...
    template<class AttrType, class DestType, typename ... Args>
    struct PropertyReader<AttrType, DestType, false, true, Args...>
    {
        static bool Read(DestType& value, AZ::Edit::Attribute* attr, void* classInstance, Args&& ... args)
        {
            if (Internal::PropertyRead<float, DestType, Args...>(value, attr, classInstance, AZStd::forward<Args>(args) ...))
            {
                return true;
            }
            if (Internal::PropertyRead<double, DestType, Args...>(value, attr, classInstance, AZStd::forward<Args>(args) ...))
            {
                return true;
            }
            return false;
    ...
    struct PropertyReader<AZStd::string, AZStd::string, false, false, Args...>
    {
        static bool Read(AZStd::string& value, AZ::Edit::Attribute* attr, void* classInstance, Args&& ... args)
        {
            if (Internal::PropertyRead<const char*, AZStd::string, Args...>(value, attr, classInstance, AZStd::forward<Args>(args) ...))
            {
                return true;
            }
            if (Internal::PropertyRead<AZStd::string, AZStd::string, Args...>(value, attr, classInstance, AZStd::forward<Args>(args) ...))
            {
                return true;
            }
            return false;
