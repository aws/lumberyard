...
namespace AzToolsFramework
{
    ...
    bool PickEntity(
        ...
        AZ::VectorFloat& closestDistance, int viewportId);
    ...
    public:
        ...
        {
            return m_minPosition + (m_maxPosition - m_minPosition) * AZ::VectorFloat(0.5f);
        }

    private:
        AZ::Vector3 m_minPosition = AZ::Vector3(AZ::VectorFloat(std::numeric_limits<float>::max()));
        AZ::Vector3 m_maxPosition = AZ::Vector3(AZ::VectorFloat(-std::numeric_limits<float>::max()));
    };
} // namespace AzToolsFramework