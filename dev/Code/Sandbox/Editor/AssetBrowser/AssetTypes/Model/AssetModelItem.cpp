...
namespace AssetViewer
{
    ...
    bool s_bStaticsInitialized =  false;
};

UINT CAssetModelItem::s_frameBufferScreenshot[CAssetModelItem::kAssetDisplay_MaxThumbImageBufferSize];
QPointer<CAssetBrowserPreviewModelDlg> CAssetModelItem::s_modelPreviewDlgHeader = nullptr;
QPointer<CAssetBrowserPreviewModelDlgFooter> CAssetModelItem::s_modelPreviewDlgFooter = nullptr;
CPreviewModelCtrl* CAssetModelItem::s_pPreviewCtrl = NULL;

QImage CAssetModelItem::s_uncachedModelThumbBmp;
bool CAssetModelItem::s_bWireframe = false;
bool CAssetModelItem::s_bPhysics = false;
bool CAssetModelItem::s_bNormals = false;
float CAssetModelItem::s_fAmbience = AssetBrowser::kModelAssetAmbienceMultiplyer;
    ...
    , m_dragCreationMode(eAssetModelItemDragCreationMode_AsBrush)
{
    ...
    if (!AssetViewer::s_bStaticsInitialized)
    {
        ...
        AssetViewer::s_bStaticsInitialized = true;
    }

    if (s_uncachedModelThumbBmp.isNull())
    {
        s_uncachedModelThumbBmp.load("Editor/UI/Icons/asset_model.png");
    }
...
void CAssetModelItem::CacheCurrentThumbAngle()
{
...
void CAssetModelItem::OnBeginPreview(QWidget* hQuickPreviewWnd)
{
    if (!LoadModel())
    {
        return;
...
QWidget* CAssetModelItem::GetCustomPreviewPanelHeader(QWidget* pParentWnd)
{
    if (s_modelPreviewDlgHeader == nullptr)
    {
        s_modelPreviewDlgHeader = new CAssetBrowserPreviewModelDlg(pParentWnd);
    }
...
QWidget* CAssetModelItem::GetCustomPreviewPanelFooter(QWidget* pParentWnd)
{
    if (s_modelPreviewDlgHeader == nullptr)
    {
        s_modelPreviewDlgHeader = new CAssetBrowserPreviewModelDlg;
    }

    if (s_modelPreviewDlgFooter == nullptr)
    {
        s_modelPreviewDlgFooter = new CAssetBrowserPreviewModelDlgFooter(pParentWnd);
    }
...
void CAssetModelItem::PreviewRender(
...
void CAssetModelItem::OnPreviewRenderKeyEvent(bool bKeyDown, UINT aChar, UINT aKeyFlags)
{
    if (bKeyDown && aChar == AssetViewer::kModelAssetWireframeToggleKey)
    {
        CAssetModelItem::s_bWireframe = !CAssetModelItem::s_bWireframe;
    }

    if (bKeyDown && aChar == AssetViewer::kModelAssetPhysicsToggleKey)
    {
        CAssetModelItem::s_bPhysics = !CAssetModelItem::s_bPhysics;
    }

    if (bKeyDown && aChar == AssetViewer::kModelAssetNormalsToggleKey)
    {
        CAssetModelItem::s_bNormals = !CAssetModelItem::s_bNormals;
    }
}

bool CAssetModelItem::Render(QWidget* hRenderWindow, const QRect& rstViewport, bool bCacheThumbnail)
{
    if (!m_pObject)
    {
        return false;
...
void CAssetModelItem::CacheThumbnail(QWidget* hRenderWindow, const QRect& rc)
{
    ...
    if (s_pPreviewCtrl)
    {
        ...
        if (pTimeOfDay && AssetViewer::s_assetThumbTimeOfDay != NULL)
        {
...
void CAssetModelItem::CreateViewport(QWidget* hRenderWindow, const QRect& rc, bool bThumbnail)
{
...
void CAssetModelItem::SetCamera(CCamera& cam, const QRect& rcViewportRect)
{
    if (!m_bCameraUpdate)
    {
        return;
...
QVariant CAssetModelItem::GetAssetFieldValue(const char* pFieldName) const
{
...
void CAssetModelItem::DrawTextOnReportImage(QPaintDevice* pd) const
{
...
void CAssetModelItem::MakeLODsTrisString(QString& rOutValue) const
{
    rOutValue.clear();
...
void CAssetModelItem::MakeLODsVertsString(QString& rOutValue) const
{
    rOutValue.clear();
...
void CAssetModelItem::MakeLODsSubMeshesString(QString& rOutValue) const
{
    rOutValue.clear();
...
void CAssetModelItem::MakeLODsMeshSizeString(QString& rOutValue) const
{
    rOutValue.clear();
...
void CAssetModelItem::ResetView()
{
    m_camZoom = 1.0f;
    m_translateX = 0.0f;
    m_translateY = 0.0f;
...
void CAssetModelItem::SetAssetModelViewport(CPreviewModelCtrl* pView)
{
    s_pPreviewCtrl = pView;
}

CPreviewModelCtrl* CAssetModelItem::GetAssetModelViewport()
{
