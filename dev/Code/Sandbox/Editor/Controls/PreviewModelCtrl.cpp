...
void CPreviewModelCtrl::LoadParticleEffect(IParticleEffect* pEffect)
{
    ...
    m_bHaveAnythingToRender = true;
    m_pEmitter = pEffect->Spawn(tm, ePEF_Nowhere);
    if (m_pEmitter)
    {
        m_pEmitter->AddRef();
        m_pEmitter->Update();
...
_smart_ptr<IMaterial> CPreviewModelCtrl::GetCurrentMaterial()
{
    ...
    {
        return m_pCharacter->GetIMaterial();
    }
    else if (m_pEmitter)
    {
        return m_pEmitter->GetMaterial();
    }
...
void CPreviewModelCtrl::showEvent(QShowEvent* event)
{
...
#include <Controls/PreviewModelCtrl.moc>
