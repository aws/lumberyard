...
const char* QBitmapPreviewDialogImp::GetShowModeDescription(EShowMode eShowMode, bool bShowInOriginalSize) const
{
    switch (eShowMode)
    {
    case ESHOW_RGB:
    ...
    else if (m_showMode == ESHOW_ALPHA)
    {
        for (int h = 0; h < scaledImage.GetHeight(); h++)
        {
            for (int w = 0; w < scaledImage.GetWidth(); w++)
            {
                ...
                scaledImage.ValueAt(w, h) = RGB(a, a, a) | (a << 24);
            }
        }
    }
    else if (m_showMode == ESHOW_RGB_ALPHA)
    {
        ...
        for (int h = 0; h < scaledImage.GetHeight(); h++)
        {
            for (int w = 0; w < halfWidth; w++)
            {
                ...
                int a = scaledImage.ValueAt(w, h) >> 24;
                scaledImage.ValueAt(w, h) = RGB(r, g, b) | (a << 24);
                scaledImage.ValueAt(w + halfWidth, h) = RGB(a, a, a) | (a << 24);
            }
        }
    }
...
#include <Controls/QBitmapPreviewDialogImp.moc>