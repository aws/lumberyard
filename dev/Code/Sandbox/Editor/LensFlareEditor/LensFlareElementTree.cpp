...
void CLensFlareElementTree::OnTvnSelchangedTree(const QItemSelection& selected, const QItemSelection& deselected)
{
...
void CLensFlareElementTree::OnDataChanged(const QModelIndex& index)
{
    CallChangeListeners();
}

void CLensFlareElementTree::OnRowsInserted(const QModelIndex& parent, int first, int last)
{
    if (parent.isValid())
    {
        expand(parent);
...
void CLensFlareElementTree::OnRowsRemoved(const QModelIndex& parent, int first, int last)
{
    if (parent.isValid())
    {
        SelectItem(parent.child(first, 0));
...
bool LensFlareElementTreeModel::dropMimeData(const QMimeData* data, Qt::DropAction action, int row, int column, const QModelIndex& parent)
{
...
void LensFlareElementTreeModel::OnLensFlareDeleteItem(CLensFlareItem* pLensFlareItem)
{
}
...
#include <LensFlareEditor/LensFlareElementTree.moc>
