...
void CLensFlareItemTree::startDrag(Qt::DropActions supportedDropActions)
{
    GetIEditor()->GetParticleUtils()->SetViewportDragOperation([](CViewport* vp, int dpx, int dpy, void* custom)
        {
		CLensFlareItemTree *itemTree = reinterpret_cast<CLensFlareItemTree *>(custom);
		itemTree->AssignLensFlareToLightEntity(vp);
	}, this);
...
#include <LensFlareEditor/LensFlareItemTree.moc>
