...
private:
    ...
    struct SResolveInfo
    {
        SResolveInfo()
            : id(0) {}
        SResolveInfo(uint32 _id, int _slot)
            : id(_id) { slotsv.push_back(_slot); }
        uint32 id;
        std::vector<int> slotsv;
    };
    typedef std::map<QString, SResolveInfo> TTexResolveReq;
    TTexResolveReq m_resolveRequests;
    bool m_allowResolve;
};
