...
private:
    void Attach()
    {
        ...
        {
            if (qobject_cast<CGroup*>(pParentObject))
            {
    ...
    void Detach()
    {
        ...
        if (pObject)
        {
            CPrefabObject* pPrefab = pObject->GetPrefab();
...
void CUndoBaseObjectMinimal::Undo(bool bUndo)
{
    ...
    pObject->SetLayerModified();
    if (s_objectPanel && s_objectPanel->GetObject() == pObject)
    {
...
void CBaseObject::DeleteUIPanels()
{
...
//! Initialize Object.
bool CBaseObject::Init(IEditor* ie, CBaseObject* prev, const QString& file)
{
...
void CBaseObject::RefreshUIPanel()
{
    if (s_varsPanel)
    {
        s_varsPanel->UpdateVarBlock(GetVarBlock());
    }
}
...
void CBaseObject::BeginEditParams(IEditor* ie, int flags)
{
    ...
    if (GetVarBlock())
    {
        if (!s_varsPanel)
        {
            s_varsPanel = new ReflectedPropertiesPanel;
            s_varsPanel->Setup(true, 180);
        }
        else
        {
            s_varsPanel->DeleteVars();
        ...
        if (s_varsPanel)
        {
            s_varsPanel->SetUpdateCallback(functor(*this, &CBaseObject::OnPropertyChanged));
        }
    }
...
void CBaseObject::EndEditParams(IEditor* ie)
{
    ...
    {
        s_pMaterialLayersMaskPanel->ClearUpdateCallback();
    }

    if (s_varsPanel)
    {
        s_varsPanel->ClearUpdateCallback();
    }
    ...
    if (s_pMaterialLayersMaskPanel)
    {
        s_pMaterialLayersMaskPanel->DeleteVars();
...
void CBaseObject::BeginEditMultiSelParams(bool bAllOfSameType)
{
    ...
    if (bAllOfSameType)
    {
        if (GetVarBlock())
        {
            ...
            if (s_varsPanel)
            {
                s_varsPanel->SetUpdateCallback(functor(*this, &CBaseObject::OnMultiSelPropertyChanged));
            }
        }
    }
    else
...
void CBaseObject::OnUIUpdate()
{
    if (s_objectPanel && !s_objectPanel->IsMultiSelect())
    {
        ...
        const bool objectChanged = (uip.area != m_flattenArea) || (uip.name != m_name) || (uip.color != m_color) || (pLayer != m_layer) || (uip.minSpec != GetMinSpec());
        ...
        {
            ChangeColor(uip.color);
        }

        if (pLayer && pLayer != m_layer)
        {
            if (SupportsLayers())
            {
                SetLayer(pLayer);
...
void CBaseObject::DrawDefault(DisplayContext& dc, const QColor& labelColor)
{
    ...
    if (dc.flags & DISPLAY_LINKS)
    {
        ...
        {
            prefabOpenedCheck = GetPrefab() && GetPrefab()->IsOpen();
        }
        ...
        if (nChildCount && !qobject_cast<CGroup*>(this))
        {
...
void CBaseObject::DrawDimensionsImpl(DisplayContext& dc, const AABB& localBoundBox,  AABB* pMergedBoundBox)
{
    ...
    bHave2Axis = GetType() == OBJTYPE_DECAL || fabs(zLength) < kEpsilon;
    ...
    for (int i = 0; i < 3; ++i)
    {
        if (picker.PickObject(cameraPos, textPos[i] - cameraPos, hitInfo, this))
        {
...
//////////////////////////////////////////////////////////////////////////
void CBaseObject::DrawSelectionHelper(DisplayContext& dc, const Vec3& pos, const QColor& labelColor, float alpha)
{
...
{
    return CheckFlags(OBJFLAG_FROZEN) || (m_layer && m_layer->IsFrozen());
}
...
void CBaseObject::Serialize(CObjectArchive& ar)
{
    ...
    if (ar.bLoading)
    {
        ...
        {
            pLayer = GetObjectManager()->GetLayersManager()->FindLayer(layerGUID);
        }
...
//////////////////////////////////////////////////////////////////////////
XmlNodeRef CBaseObject::Export(const QString& levelPath, XmlNodeRef& xmlNode)
{
...
void CBaseObject::GetAllPrefabFlagedChildren(CSelectionGroup& outAllChildren, CBaseObject* pObj) const
{
    const CBaseObject* pBaseObj = pObj ? pObj : this;
...
void CBaseObject::ResolveParent(CBaseObject* parent)
{
    ...
    if (parent)
    {
        bool bDerivedFromGroup = qobject_cast<CGroup*>(parent);
        bool bSuspended(false);
        if (bDerivedFromGroup)
        {
            bSuspended = ((CGroup*)parent)->SuspendUpdate();
        }
        parent->AddMember(this, false);
        if (bSuspended)
        {
            ((CGroup*)parent)->ResumeUpdate();
        }
    }
    else
    {
...
int CBaseObject::AddUIPage(const char* sCaption, QWidget* pDialog, int iIndex, bool bAutoExpand)
{
    return GetIEditor()->AddRollUpPage(ROLLUP_OBJECTS, sCaption, pDialog, iIndex, bAutoExpand);
}
...
//////////////////////////////////////////////////////////////////////////
void CBaseObject::RenameUIPage(int iIndex, const char* pNewName)
{
    GetIEditor()->RenameRollUpPage(ROLLUP_OBJECTS, iIndex, pNewName);
}
...
void CBaseObject::OnMenuProperties()
{
    ...
    GetIEditor()->GetEditTool()->EndEditParams();
    GetIEditor()->SetEditTool("EditTool.ObjectMode");
    BeginEditParams(GetIEditor(), OBJECT_EDIT);
...
CGroup* CBaseObject::GetGroup() const
{
    ...
    while (pParentObject)
    {
        if (qobject_cast<CGroup*>(pParentObject))
        {
            return static_cast<CGroup*>(pParentObject);
        }
...
CPrefabObject* CBaseObject::GetPrefab() const
{
    CBaseObject* pParentObject = GetParent();
    while (pParentObject)
    {
        ...
        {
            return static_cast<CPrefabObject*>(pParentObject);
        }
        pParentObject = pParentObject->GetParent();
    }
    return nullptr;
...
{
    CPrefabObject* pPrefab = GetPrefab();
    if (!pPrefab || pPrefab->IsModifyInProgress())
    {
        return;
    }
    ...
    pPrefab->SyncPrefab(context);
}

bool CBaseObject::CanBeHightlighted() const
{
    return !GetGroup() || GetGroup()->IsOpen();
}
...
void CBaseObject::AddMember(CBaseObject* pMember, bool bKeepPos /*=true */)
{
    ...
    if (CPrefabObject* pPrefab = GetPrefab())
    {
...
#include <Objects/BaseObject.moc>