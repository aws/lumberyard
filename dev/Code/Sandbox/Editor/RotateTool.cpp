...
bool CRotateTool::CanSelectObject(CBaseObject* object)
{
    return true;
}
...
bool CRotateTool::OnKeyDown(CViewport* view, uint32 nChar, uint32 nRepCnt, uint32 nFlags)
{
    if (nChar == VK_ESCAPE)
    {
        ...
        void Draw(DisplayContext& dc, const Vec3& position, const Vec3& axisToAlign, float startAngleRadians, float sweepAngleRadians, float stepAngleRadians, float radius, float screenScale)
        {
            ...
            if (numVertices >= 2)
            {
                ...
                for (int i = 0; i < numVertices; ++i)
                {
                    ...
                    p1.x = position.x + (cosAngle * a.x + sinAngle * b.x) * radius * screenScale;
                    p1.y = position.y + (cosAngle * a.y + sinAngle * b.y) * radius * screenScale;
                    p1.z = position.z + (cosAngle * a.z + sinAngle * b.z) * radius * screenScale;
