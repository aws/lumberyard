...
namespace VariableOArchiveHelpers
{
    ...
    {
        IGame* const pGame = gEnv->pGame;
        if (pGame)
        {
            IGameFramework* const pGameFramework = pGame->GetIGameFramework();
            if (pGameFramework)
            {
                return pGameFramework->GetIForceFeedbackSystem();
            }
        }
        return NULL;
    }
...
bool CVariableOArchive::SerializeAnimationName(const Serialization::IResourceSelector* pSelector, const char* name, const char* label)
{
    ...
    pVariable->SetUserData(m_animationEntityId);
