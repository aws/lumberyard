...
void ThumbnailsSampleWidget::RegisterViewClass()
{
    ...
    options.sendViewPaneNameBackToAmazonAnalyticsServers = false;
...
void ThumbnailsSampleWidget::UpdateThumbnail() const
{
    ...
    if (selectedAssets.size() > 0)
    {
        ...
        for (const auto* product : products)
        {
            ...
            thumbnailWidget->SetThumbnailKey(MAKE_TKEY(AzToolsFramework::Thumbnailer::ProductThumbnailKey, product->GetAssetId()));
...
#include <Thumbnails/Example/ThumbnailsSampleWidget.moc>
