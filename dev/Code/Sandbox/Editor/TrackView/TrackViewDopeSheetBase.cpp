...
void CTrackViewDopeSheetBase::OnLButtonDblClk(Qt::KeyboardModifiers modifiers, const QPoint& point)
{
    ...
    else
    {
        ...
        if (pTrack)
        {
            ...
            else if (pTrack->GetValueType() != eAnimValue_Bool)
            {
                ...
                ShowKeyPropertyCtrlOnSpot(p.x(), p.y(), pSequence->GetSelectedKeys().GetKeyCount() > 1, bKeyChangeInSameTrack);
            }
        }
...
void CTrackViewDopeSheetBase::OnRButtonDown(Qt::KeyboardModifiers modifiers, const QPoint& point)
{
    ...
    if (keyHandle.IsValid())
    {
        ...
        QMenu menu;
        CTrackViewKeyBundle selectedKeys = pSequence->GetSelectedKeys();
        const bool bEnableEditOnSpot = ((pTrack && pTrack->GetValueType() != eAnimValue_Bool) &&
...
//////////////////////////////////////////////////////////////////////////
void CTrackViewDopeSheetBase::OnRButtonUp(Qt::KeyboardModifiers modifiers, const QPoint& point)
{
    CTrackViewSequence* pSequence = GetIEditor()->GetAnimation()->GetSequence();
    if (!pSequence)
    {
...
//////////////////////////////////////////////////////////////////////////
void CTrackViewDopeSheetBase::MouseMoveMove(const QPoint& p, Qt::KeyboardModifiers modifiers)
{
    CTrackViewSequence* pSequence = GetIEditor()->GetAnimation()->GetSequence();
    CTrackViewSequenceNotificationContext context(pSequence);
...
//////////////////////////////////////////////////////////////////////////
void CTrackViewDopeSheetBase::LButtonDownOnTimeAdjustBar(const QPoint& point, CTrackViewKeyHandle& keyHandle, bool bStart)
{
    CTrackViewSequence* pSequence = GetIEditor()->GetAnimation()->GetSequence();
...
void CTrackViewDopeSheetBase::EditSelectedColorKey(CTrackViewTrack* pTrack)
{
    if (pTrack->IsCompoundTrack())
    {
        ...
        if (selectedKeyBundle.GetKeyCount())
        {
            ...
            float keyTime = selectedKeyBundle.GetKey(0).GetTime();
            Vec3  color;
            pTrack->GetValue(keyTime, color);
            ...
            if (dlg.exec() == QDialog::Accepted)
            {
                ...
                {
                    CTrackViewTrack* subTrack = static_cast<CTrackViewTrack*>(pTrack->GetChild(i));
                    CTrackViewKeyHandle subTrackKey = subTrack->GetKeyByTime(keyTime);
                    I2DBezierKey bezierKey;
                    ...
                    else
                    {
                        ...
                        subTrackKey = subTrack->CreateKey(keyTime);
                        subTrackKey.GetKey(&bezierKey);
                    }
...
//////////////////////////////////////////////////////////////////////////
void CTrackViewDopeSheetBase::DrawKeys(CTrackViewTrack* pTrack, QPainter* painter, QRect& rect, Range& timeRange)
{
...
//////////////////////////////////////////////////////////////////////////
void CTrackViewDopeSheetBase::DrawClipboardKeys(QPainter* painter, const QRect& rc)
{
    CTrackViewSequence* pSequence = GetIEditor()->GetAnimation()->GetSequence();
...
void CTrackViewDopeSheetBase::DrawTimeLineInFrames(QPainter* painter, const QRect& rc, const QColor& lineCol, const QColor& textCol, double step)
{
...
void CTrackViewDopeSheetBase::DrawTimeLineInSeconds(QPainter* painter, const QRect& rc, const QColor& lineCol, const QColor& textCol, double step)
{
    Range VisRange = GetVisibleRange();
    const Range& timeRange = m_timeRange;
    int nNumberTicks = 10;
