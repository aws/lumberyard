...
bool CPakFile::Open(const char* filename, bool bAbsolutePath)
{
    ...
    if (bAbsolutePath)
    {
        m_pArchive = pCryPak->OpenArchive(filename, nullptr, ICryArchive::FLAGS_ABSOLUTE_PATHS);
    }
    else
    {
...
bool CPakFile::OpenForRead(const char* filename)
{
    ...
    m_pArchive = pCryPak->OpenArchive(filename, nullptr, ICryArchive::FLAGS_OPTIMIZED_READ_ONLY | ICryArchive::FLAGS_ABSOLUTE_PATHS);
    if (m_pArchive)
    {
        return true;
    }
