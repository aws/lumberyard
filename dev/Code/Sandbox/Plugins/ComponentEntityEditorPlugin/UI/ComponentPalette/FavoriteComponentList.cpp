...
void FavoritesList::rowsInserted(const QModelIndex& /*parent*/, int start, int end)
{
...
int FavoritesDataModel::rowCount(const QModelIndex &parent /*= QModelIndex()*/) const
{
    return m_favorites.size();
}

int FavoritesDataModel::columnCount(const QModelIndex &parent /*= QModelIndex()*/) const
{
...
void FavoritesDataModel::SetSavedStateKey(AZ::u32 key)
{
}
...
bool FavoritesDataModel::dropMimeData(const QMimeData *data, Qt::DropAction action, int row, int column, const QModelIndex &parent)
{
...
#include <UI/ComponentPalette/FavoriteComponentList.moc>
