...
void CPreviewModelView::OnViewportKey(const SKeyEvent& ev)
{
}

void CPreviewModelView::OnViewportMouse(const SMouseEvent& ev)
{
}
////////////////////////////////////////////////////////
...
void CPreviewModelView::RenderModels(SRendParams& rendParams, SRenderingPassInfo& passInfo)
{
    ...
    if (m_pCharacterModel)
    {
        if (IsFlagSet(PreviewModelViewFlag::PRECACHE_MATERIAL))
        {
            _smart_ptr<IMaterial>  pCurMat = m_pCharacterModel->GetIMaterial();
            if (pCurMat)
            {
                pCurMat->PrecacheMaterial(0.0f, nullptr, true, true);
            }
        }
        m_pCharacterModel->Render(rendParams, QuatTS(IDENTITY), passInfo);
    }
}
...
void CPreviewModelView::ReleaseModel()
{
    ...
    SAFE_RELEASE(m_pCharacterModel);
}
...
void CPreviewModelView::LoadModelFile(const QString& modelFile)
{
    ...
    if (!modelFile.isEmpty())
    {
        ...
        if (m_ModelFilename != modelFile)
        {
            ...
            bool isSKEL = (QString::compare(strFileExt, CRY_SKEL_FILE_EXT, Qt::CaseInsensitive) == 0);
            bool isSKIN = (QString::compare(strFileExt, CRY_SKIN_FILE_EXT, Qt::CaseInsensitive) == 0);
            bool isCDF = (QString::compare(strFileExt, CRY_CHARACTER_DEFINITION_FILE_EXT, Qt::CaseInsensitive) == 0);
            bool isCGA = (QString::compare(strFileExt, CRY_ANIM_GEOMETRY_FILE_EXT, Qt::CaseInsensitive) == 0);
            ...
            if (m_pCharacterModel || m_pStaticModel)
            {
...
void CPreviewModelView::FocusOnScreen()
{
    ...
    if (camera)
    {
        ...
        if (m_pCharacterModel)
        {
            accumulated.Add(m_pCharacterModel->GetAABB());
        }
