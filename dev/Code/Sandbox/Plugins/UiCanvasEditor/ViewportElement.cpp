...
namespace
{
    ...
    AZ::Vector2 MoveSnap(HierarchyWidget* hierarchy,
        ...
        const AZ::Vector3& mouseTranslation)
    {
        ...
        // because the offsets are in parent space.
        AZ::Vector3 localTranslation;
        if (interactionType == ViewportInteraction::InteractionType::TRANSFORM_GIZMO && grabbedGizmoParts.Single())
        {
            AZ::Vector3 viewportTranslation = mouseTranslation;

            if (coordinateSystem == ViewportInteraction::CoordinateSystem::LOCAL)
            {
                // Adjust viewport translation for this element to allow for a different local translation from the selected element
                viewportTranslation = GetTranslationForSelectedElement(activeElementId, element, mouseTranslation);

                // For LOCAL MOVE, we need to transform the translation from viewport to the parent element's local space
                ...
                localTranslation = transformFromViewport.Multiply3x3(viewportTranslation);

                // Zero-out the non-moving axis in local space.
                if (!grabbedGizmoParts.m_right)
                {
                    localTranslation.SetX(0.0f);
                }
            ...
            else // if (coordinateSystem == ViewportInteraction::CoordinateSystem::VIEW)
            {
                ...
                // To get to parent space, we use the parent's transform from viewport
                AZ::Matrix4x4 transformToParent;
                EBUS_EVENT_ID(parentElement->GetId(), UiTransformBus, GetTransformFromViewport, transformToParent);
                localTranslation = transformToParent.Multiply3x3(viewportTranslation);
            }
        }
        else // if (interactionType == ViewportInteraction::InteractionType::DIRECT || (Both() or None, of the grabbedGizmoParts))
        {
            // The final translation vector needs to be in the element's parent space,
            // because the offsets are in parent space.
            //
            // This is the same as (above): if (coordinateSystem == ViewportInteraction::CoordinateSystem::VIEW)
            AZ::Matrix4x4 thisParentTransformFromViewport;
            EBUS_EVENT_ID(parentElement->GetId(), UiTransformBus, GetTransformFromViewport, thisParentTransformFromViewport);
            localTranslation = thisParentTransformFromViewport.Multiply3x3(mouseTranslation);
        }

        AZ::Vector3 localTranslationRounded3 = EntityHelpers::RoundXY(localTranslation);
        return ViewportSnap::Move(hierarchy, canvasId, coordinateSystem, grabbedGizmoParts, element, AZ::Vector2(localTranslationRounded3.GetX(), localTranslationRounded3.GetY()));
    }

    void MoveNoSnap(AZ::Entity* element,
        const AZ::Vector2& deltaInCanvasSpace)
    {
...
void ViewportElement::Move(HierarchyWidget* hierarchy,
    ...
    const AZ::Vector3& mouseTranslation)
{
    AZ_Assert(activeElement, "Active element is null");

    // MoveSnap() is only applied to the topLevelActiveElement.
    // All other selected elements are kept in constant distance from topLevelActiveElement.
    //
    // IMPORTANT: ViewportElement::Move() is called for every mouseMoveEvent.
    // selectedItems is constant thru multiple mouseMoveEvent.
    // Functions, whose result only changes when selectedItems changes, can be
    // optimized for speed by caching their result on the first mouseMoveEvent,
    // or whenever selectedItems changes.

    LyShine::EntityArray topLevelSelectedElements = SelectionHelpers::GetTopLevelSelectedElements(hierarchy, selectedItems);

    AZ::Entity* topLevelActiveElement = SelectionHelpers::GetTopLevelParentOfElement(topLevelSelectedElements, activeElement);

    if (topLevelActiveElement && !ViewportHelpers::IsControlledByLayout(topLevelActiveElement))
    {
        SelectionHelpers::RemoveEntityFromArray(topLevelSelectedElements, topLevelActiveElement);

        AZ::Vector2 deltaInCanvasSpace = MoveSnap(hierarchy, canvasId, coordinateSystem, grabbedGizmoParts, interactionType, activeElement->GetId(), topLevelActiveElement, mouseTranslation);

        for (auto element : topLevelSelectedElements)
        {
            if (!ViewportHelpers::IsControlledByLayout(element))
            {
                MoveNoSnap(element, deltaInCanvasSpace);
            }
        }
    }
}
