...
bool CSTLHelper::ToFile(const std::string& rFileName, const std::vector<uint8_t>&    rOut)
{
    ...
    {
        AZ_Error(0, wasSuccessful, "Could not create file: %s", rFileName.c_str());
        return false;
    }
...
bool CSTLHelper::ToFileCompressed(const std::string& rFileName, const std::vector<uint8_t>&  rOut)
{
    ...
    if (destLen > 0)
    {
        AZ::IO::SystemFile outputFile;
        const bool wasSuccessful = outputFile.Open(rFileName.c_str(), AZ::IO::SystemFile::SF_OPEN_WRITE_ONLY | AZ::IO::SystemFile::SF_OPEN_CREATE);
        AZ_Error(0, wasSuccessful, "Could not create compressed file: %s", rFileName.c_str());
        ...
        AZ_Error(0, bytesWritten == sizeof(sourceLen), "Could not save out size of compressed data to %s", rFileName.c_str());
        ...
        AZ_Error(0, bytesWritten == destLen, "Could not save out compressed data to %s", rFileName.c_str());
...
bool CSTLHelper::FromFileCompressed(const std::string& rFileName, std::vector<uint8_t>&  rIn)
{
    ...
    AZ_Error(0, wasSuccessful, "Could not read: ", rFileName.c_str());
    ...
    AZ_Error(0, FileLen > 0, "Error getting file-size of ", rFileName.c_str());
    ...
    AZ_Warning(0, bytesReadIn == sizeof(uncompressedLen), "Expected to read in %d but read in %d from file %s", sizeof(uncompressedLen), bytesReadIn, rFileName.c_str());

    bytesReadIn = inputFile.Read(buf.size(), buf.data());
    AZ_Warning(0, bytesReadIn == buf.size(), "Expected to read in %d but read in %d from file %s", buf.size(), bytesReadIn, rFileName.c_str());
...
{
    AZ::IO::SystemFile outputFile;
    const bool wasSuccessful = outputFile.Open(rFileName.c_str(), AZ::IO::SystemFile::SF_OPEN_APPEND);
    AZ_Error(0, wasSuccessful, "Could not open file for appending: %s", rFileName.c_str());
    if (wasSuccessful == false)
    {
        return false;
    }
    ...
    AZ_Warning(0, bytesWritten == rOut.size(), "Did not write out all the data to the file: %s", rFileName.c_str());
    return true;
}
