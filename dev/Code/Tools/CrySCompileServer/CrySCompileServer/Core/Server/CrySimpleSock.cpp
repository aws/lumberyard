...
struct CCrySimpleSock::Implementation
{
    ...
    void SetWhitelist(const std::vector<AZStd::string>& whiteList)
    {
        ...
        hostent* hostInfo = gethostbyname(hostNameBuffer);
        ...
        if (hostInfo != nullptr)
        {
            ...
            while (hostInfo->h_addr_list[i] != 0)
            {
                ...
                whitelistAddress.m_address = *(reinterpret_cast<uint32_t *>(hostInfo->h_addr_list[i]));
        ...
        for (const auto& address : whiteList)
        {
            ...
            {
                whitelistAddress.m_address = inet_addr(address.c_str());
            }
