...
#define VERSION_SEPARATOR_STR "."
    ...
    struct Version
    {
        ...
        */
        static AZ::Outcome<Version, AZStd::string> ParseFromString(const AZStd::string& versionStr)
        {
            ...
            size_t partCount = AzFramework::StringFunc::CountCharacters(versionStr.c_str(), VERSION_SEPARATOR_CHAR) + 1;
            if (N != partCount)
            {
    ...
    using GemVersion = SemanticVersion;
...
{
    template <size_t N>
    struct hash<Gems::Version<N>>
    {
        size_t operator()(const Gems::Version<N>& ver) const
        {
    ...
    template <>
    struct hash<Gems::SemanticVersion>
        : public hash<Gems::Version<3>>
    { };
} // namespace AZStd
