...
public:
    inline void locate(Rect2D take)
    {
        ...
        for (int p = 0; p < planes; p++)
        {
            ...
            buffer = (char*)NULL;
            assert((reinterpret_cast<AZ::u64>(buffer) % 16) == 0);
...
/* #################################################################################################################### \
 */
void ImageObject::FilterImage(int filterIndex, int filterOp, float blurH, float blurV, const ImageObject* srcImg, int srcMip, int dstMip, QRect* srcRect, QRect* dstRect)
{
...
void ImageObject::FilterImage(const CImageProperties* pProps, const ImageObject* srcImg, int srcMip, int dstMip, QRect* srcRect, QRect* dstRect)
{
    if (pProps)
    {
