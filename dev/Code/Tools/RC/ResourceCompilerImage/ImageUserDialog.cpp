...
class ProcessActionsThread : public QThread
{
public:
    ProcessActionsThread(CImageUserDialog* dialog)
    {
        connect(this, &QThread::started, this, [this, dialog]() {
            dialog->ProcessActions();
...
CImageUserDialog::~CImageUserDialog()
{
    if (m_actionProcessingThread)
    {
        m_actionProcessingThread->wait();
    }
...
bool CImageUserDialog::DoModal(CImageCompiler* inpImageCompiler)
{
    ...
    m_dialogFinished = false;
    m_quitRequest = QuitRequestNone;

    // if we're already running on the main thread, then launch a thread to do our work
    if (QThread::currentThread() == qApp->thread())
    {
        InitDialog();

        // start the worker thread - after the dialog is initialized
        m_actionProcessingThread.reset(new ProcessActionsThread(this));
        m_actionProcessingThread->start();

        // show the dialog, feeding actions to the action processing thread
        ShowDialog();
    }
    else // otherwise, tell the main thread to show the dialog
    {
        // initialize the dialog and block for initialization to finish
        const bool initDialogBlocks = true;
        MainThreadExecutor::execute([this]() { InitDialog(); }, initDialogBlocks);

        // start the dialog now, non-blocking
        const bool showDialogBlocks = false;
        MainThreadExecutor::execute([this]() { ShowDialog(); }, showDialogBlocks);

        // process actions generated by the dialog now
        ProcessActions();
    }
...
void CImageUserDialog::ProcessActions()
{
    while (!m_dialogFinished)
    {
...
void CImageUserDialog::InitDialog()
{
...
void CImageUserDialog::ShowDialog()
{
    // message loop
...
void MainThreadExecutor::execute(AZStd::function<void()> f, bool blocking)
{
    static MainThreadExecutor e;

    if (QThread::currentThread() == e.thread())
    {
        f();
    }
    else
    {
        Qt::ConnectionType connectionType = blocking ? Qt::BlockingQueuedConnection : Qt::QueuedConnection;

        QMetaObject::invokeMethod(&e, "executeInternal", connectionType, Q_ARG(AZStd::function<void()>, f));
    }
}
...
MainThreadExecutor::MainThreadExecutor()
{
    ...
    qRegisterMetaType< AZStd::function<void()> >("AZStd::function<void()>");
}

void MainThreadExecutor::executeInternal(AZStd::function<void()> function)
{
    if (function)
    {
        function();
    ...
    , m_ui(new Ui::ChooseResolutionDialog)
{
    ...
    // We will use GetFinalImageInfo() which requires platform specific. For that we can
