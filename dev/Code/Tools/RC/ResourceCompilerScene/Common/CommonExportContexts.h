...
namespace AZ
{
    ...
    namespace RC
    {
        ...
        struct AnimationExportContext
            : public SceneAPI::Events::ICallContext
        {
            AZ_RTTI(AnimationExportContext, "{DEAA7845-96BF-4629-9499-76542DB6EDE7}", SceneAPI::Events::ICallContext);

            AnimationExportContext(const SceneAPI::Containers::Scene& scene, const AZStd::string& rootBoneName, uint32_t startFrame, uint32_t endFrame,
            ...
            AnimationExportContext(const SceneAPI::Containers::Scene& scene, const SceneAPI::DataTypes::IAnimationGroup& animationGroup, 
                CInternalSkinningInfo& controllerSkinningInfo, Phase phase);
            AnimationExportContext(const AnimationExportContext& copyContext, Phase phase);
            AnimationExportContext(const AnimationExportContext& copyContext) = delete;
            ~AnimationExportContext() override = default;

            AnimationExportContext& operator=(const AnimationExportContext& other) = delete;

            const SceneAPI::Containers::Scene& m_scene;
            const AZStd::string& m_rootBoneName;
            const uint32_t m_startFrame;
            const uint32_t m_endFrame;
            CInternalSkinningInfo& m_controllerSkinningInfo;
            const Phase m_phase;
            const SceneAPI::DataTypes::IAnimationGroup::PerBoneCompressionList& m_perBoneCompression;
        };
    } // RC
} // AZ
