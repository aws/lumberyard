...
namespace AZ
{
    namespace RC
    {
        ...
        {
            BindToCall(&TouchBendingExporter::ValidateRules);
        ...
        SceneEvents::ProcessingResult TouchBendingExporter::ValidateRules(CgfGroupExportContext& context)
        {
            if (context.m_phase != Phase::Construction)
            {
                return SceneEvents::ProcessingResult::Ignored;
            }
            //ITouchBendingRule can not coexist with IPhysicsRule. The reason being is that a Mesh can only be one physical type
            //as listed in enum EPhysicsGeomType. When IPhysicsRule was introduced (which predates ITouchBendingRule for more than two years)
            //it only conceived two types: PHYS_GEOM_TYPE_NONE and PHYS_GEOM_TYPE_DEFAULT_PROXY. For touchbending the collision proxy mesh
            //should be PHYS_GEOM_TYPE_NO_COLLIDE.
            ...
            AZStd::shared_ptr<const SceneDataTypes::IPhysicsRule> physicsRule = context.m_group.GetRuleContainerConst().FindFirstByType<SceneDataTypes::IPhysicsRule>();
            if (touchBendingRule && physicsRule)
            {
                AZ_TracePrintf(TraceWindowName, "Mesh Group can not have a Touch Bending rule and a Physics Rule.");
