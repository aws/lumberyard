...
namespace AZ
{
    namespace SceneAPI
    {
        namespace FbxSceneBuilder
        {
            ...
            CoreProcessingResult AddDataNodeWithContexts(SceneDataPopulatedContext& dataPopulated)
            {
                ...
                SceneNodeAppendedContext nodeAppended(dataPopulated, dataPopulated.m_currentGraphPosition);
            ...
            CoreProcessingResult AddAttributeDataNodeWithContexts(SceneAttributeDataPopulatedContext& dataPopulated)
            {
                AZ_TraceContext("Node Name", dataPopulated.m_dataName);
                const char* nodeTypeName = dataPopulated.m_graphData ? dataPopulated.m_graphData->RTTI_GetTypeName() : "Null";
                AZ_TraceContext("Node Type", (!nodeTypeName || nodeTypeName[0] == '\0' ? "Null" : nodeTypeName));
                ...
                SceneAttributeNodeAppendedContext nodeAppended(dataPopulated, dataPopulated.m_currentGraphPosition);
                nodeResults += Events::Process(nodeAppended);
            ...
            bool GetBindPoseLocalTransform(const FbxSDKWrapper::FbxSceneWrapper& sceneWrapper,
                FbxSDKWrapper::FbxNodeWrapper& nodeWrapper, Transform& xf)
            {
                PoseList poseList;
