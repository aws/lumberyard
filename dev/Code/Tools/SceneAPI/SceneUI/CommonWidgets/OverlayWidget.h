...
namespace AZ
{
    namespace SceneAPI
    {
        namespace UI
        {
            ...
            //      lower layers.
            class SCENE_UI_API OverlayWidget : public QWidget
            {
            ...
            protected:
                struct LayerInfo
                {
                    OverlayWidgetLayer* m_layer;
                    // Unique number to identify the layer by.
                    int m_layerId;
                    // The index of the layer on the stack.
                    int m_layerIndex;

                    LayerInfo(OverlayWidgetLayer* layer, int id, int index);
                };

                int PushLayer(OverlayWidgetLayer* layer);
                bool PopLayer(OverlayWidgetLayer* layer);
                void OnStackEntryRemoved(int index);

                static int PushLayer(OverlayWidget* targetOverlay, OverlayWidgetLayer* layer, bool hasBreakoutWindow);

                int m_layerIdCounter;
                AZStd::vector<LayerInfo> m_layers;
