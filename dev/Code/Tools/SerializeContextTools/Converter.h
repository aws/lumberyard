...
namespace AZ
{
    ...
    namespace SerializeContextTools
    {
        ...
        private:
            ...
            static bool ConvertSystemSettings(PathDocumentContainer& documents, const ComponentApplication::Descriptor& descriptor, const GemInfo& gems, 
                const AZStd::string& configurationName, const AZStd::string& projectFolder, const AZStd::string& applicationRoot);
            static bool ConvertSystemComponents(PathDocumentContainer& documents, const Entity& entity, const AZStd::string& configurationName, const AZStd::string& projectFolder,
                const JsonSerializerSettings& convertSettings, const JsonDeserializerSettings& verifySettings);
            static bool ConvertModuleComponents(PathDocumentContainer& documents, const ModuleEntity& entity, const GemInfo& gems, const AZStd::string& configurationName,
                const JsonSerializerSettings& convertSettings, const JsonDeserializerSettings& verifySettings);
            static bool VerifyConvertedData(rapidjson::Document& convertedData, const void* original, const Uuid& originalType, const JsonDeserializerSettings& settings);

            static AZStd::string GetClassName(const Uuid& classId, SerializeContext* context);
            static bool WriteDocumentToDisk(const AZStd::string& filename, const rapidjson::Document& document,
