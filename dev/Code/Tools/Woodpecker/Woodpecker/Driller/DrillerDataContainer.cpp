...
#include "IO/StreamerDataAggregator.hxx"
...
namespace Driller
{
    ...
    public:
        ...
        {
            m_drillerSessionInfo.m_platform = AZ::g_currentPlatform; /// Init with current platform so no endian swapping till we read all the initial settings.
    ...
    Aggregator* DrillerDataContainer::CreateAggregator(AZ::u32 id, bool createUnsupported)
    {
        ...
        else if( id == StreamerDataAggregator::DrillerId() )
        {
            return aznew StreamerDataAggregator(m_identity);
        }
    ...
    void DrillerDataContainer::Reflect(AZ::ReflectContext* context)
    {
        ...
        StreamerDataAggregator::Reflect(context);
