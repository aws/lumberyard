...
namespace Blast
{
        ...
        AZStd::unordered_set<BlastActor*>& actorsToDelete)
    {
        ...
        for (uint32_t childIndex = 0; childIndex < newActorsCount; ++childIndex)
        {
            ...
            if (parentBody)
            {
                ...
                parentTransform.MultiplyByScale(m_initialTransform.RetrieveScale());
            }
            else
            {
        ...
        Physics::WorldBody* parentBody, bool parentStatic, AZ::Transform parentTransform, Nv::Blast::TkActor* tkActor)
    {
        ...
        actorDesc.m_parentCenterOfMass = parentTransform *
    ...
    BlastActorDesc BlastFamilyImpl::CalculateActorDesc(const AZ::Transform& transform, Nv::Blast::TkActor* tkActor)
    {
        ...
        configuration.m_scale = transform.RetrieveScale();
    ...
    void BlastFamilyImpl::DestroyActor(BlastActor* blastActor)
    {
        if (m_actorTracker.GetActors().find(blastActor) == m_actorTracker.GetActors().end())
        {
            AZ_Warning(
                ...
                blastActor->GetEntity()->GetId().ToString());
            return;
        }
        ...
        AZStd::vector<DebugLine>& lines, AZ::Vector3 pos, AZ::Color color, const float& area, const AZ::Vector3& normal)
    {
        ...
        // draw square of area 'area' rotated by normal
        {
            ...
            AZ::Quaternion q(axis.GetX(), axis.GetY(), axis.GetZ(), 1.f + d);
            q.Normalize();
            ...
            AZ::Vector3 p0 = t * (AZ::Vector3(-e, e, 0) * r);
            AZ::Vector3 p1 = t * (AZ::Vector3(e, e, 0) * r);
            AZ::Vector3 p2 = t * (AZ::Vector3(e, -e, 0) * r);
            AZ::Vector3 p3 = t * (AZ::Vector3(-e, -e, 0) * r);
    ...
    void BlastFamilyImpl::FillDebugRender(DebugRenderBuffer& debugRenderBuffer, DebugRenderMode mode, float renderScale)
    {
