...
def verify_unused_ami_name(config: Config) -> None:
...
def obtain_instance_profile_arn(config: Config) -> None:
...
def upload_files(config: Config) -> None:
...
def build_userdata(config: Config) -> None:
    lines = [
        "<powershell>",
...
def create_instance(config: Config) -> None:
...
if __name__ == "__main__":
    ...
    required_group.add_argument("--role", help="IAM role for the EC2 instance. You may use the role generated by the gem in your deployment for this.",
                                required=True)
    required_group.add_argument("--s3-bucket",
                                help="S3 bucket to use for file transfers. You may use the bucket generated by the gem in your deployment for this.",
                                required=True)
    ...
    parser.add_argument("--instance-type", help="EC2 instance type to use for building your AMI. (Does not have to be the same type used for deploying.)",
                        default="m5.large")
    parser.add_argument("--volume-size", help="Volume size for your AMI in GiB. Deployed instances will be required to be at least this size.", type=int,
                        default=100)
    parser.add_argument("--base-ami", help="Base AMI to use as starting point for building custom AMI. "
                                           "Should be the current Microsoft Windows Server 2016/2019 Base AMI in the region selected for AMI creation.")
    parser.add_argument("--subnet-id", help="An optional subnet ID to use for the staging EC2 instance. Must also specify --security-group-id if used.",
                        default=None)
    parser.add_argument("--security-group-id", help="An optional security group ID to use for the staging EC2 instance. Must also specify --subnet-id if used.",
                        default=None)
