...
def get_key_pair_paths(context, base_name):
    ...
    private_out_folder = dynamic_content_settings.get_private_certificate_game_folder(context.config.game_directory_path)   
...
def _load_key(context, base_name = None):
    ...
    keypath = os.path.join(dynamic_content_settings.get_private_certificate_game_folder(context.config.game_directory_path), dynamic_content_settings.get_private_keyname(base_name, 'pem'))
...
def _load_public_key(context, base_name = None):
    ...
    keypath = os.path.join(dynamic_content_settings.get_public_certificate_game_folder(context.config.game_directory_path), dynamic_content_settings.get_public_keyname(base_name, 'pem'))
...
def test_signature(context, args):
    ...
    hashedMessage = SHA256.new(to_sign)
