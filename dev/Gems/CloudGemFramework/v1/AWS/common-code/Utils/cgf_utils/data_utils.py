...
class Data(object):
    ...
    When a list typed property is returned, a copy of the original list
    ...
    object instance (wrapped, converted, or original) being returned every
    time.

    You can prevent the accidental creation of entries by setting the
    ...
    create_new parameter when initializing the object. By default new dict
    objects are created.
    ...
    def __init__(self, data=None, read_only=False, create_new=dict, **kwargs):
        ...
        Arguments:
          ...
          None, which can be useful as an alternative to setting read_only
    ...
    def __eq__(self, other):
        ...
        return (isinstance(other, Data) and self.__data == other.__data) or (isinstance(other, dict) and self.__data == other)
    ...
    def __getattr__(self, name):
        """Called by Python to retrieve the value of undefined attribute."""

        if name.startswith('__'):
            return super(Data, self).__getattribute__(name)  # use default for built in attributes
        else:

            if self.__read_only:
                raise RuntimeError('Data object is read only when attempting get non-existent property {}.'.format(name))
    ...
    def __convert_list(self, source):
        """Recursively wraps dict objects in a list."""
        target = []
        for i in source:
