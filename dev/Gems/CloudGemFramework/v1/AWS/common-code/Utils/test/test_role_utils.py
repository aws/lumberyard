...
class UnitTest_CloudGemFramework_ProjectResourceHandler_role_utils_get_role_name(unittest.TestCase):
    ...
    def test_with_too_long_name(self):
        ...
        self.assertRaises(RuntimeError, role_utils.get_access_control_role_name, stack_arn, logical_role_name)
...
class UnitTest_CloudGemFramework_ProjectResourceHandler_role_utils_create_access_control_role(unittest.TestCase):
    ...
    @mock.patch('role_utils.get_access_control_role_name', return_value = role_name)
    @mock.patch('resource_manager_common.stack_info.get_stack_info', return_value=mock_stack_info)
    def test_default(self, mock_get_stack_info, mock_get_access_control_role_name, mock_create_role):
        ...
        mock_create_role.assert_called_once_with(RoleName=self.role_name, AssumeRolePolicyDocument=self.assume_role_policy_document_matcher, Path=self.path)
    ...
    @mock.patch('role_utils.get_access_control_role_name', return_value = role_name)
    @mock.patch('resource_manager_common.stack_info.get_stack_info', return_value = mock_stack_info)
    def test_with_delay_for_propagation_false(self, mock_get_stack_info, mock_get_access_control_role_name, mock_create_role):
        id_data = {}
        role_utils.PROPAGATION_DELAY_SECONDS = 2
        start_time = time.clock()
        created_role_arn = role_utils.create_access_control_role(
    ...
    @mock.patch('role_utils.get_access_control_role_name', return_value = role_name)
    @mock.patch('resource_manager_common.stack_info.get_stack_info', return_value=mock_stack_info)
    def test_with_default_policy(self, mock_get_stack_info, mock_get_access_control_role_name, mock_put_role_policy, mock_create_role):
        id_data = {}
        created_role_arn = role_utils.create_access_control_role(
            id_data,
            self.stack_arn, 
        ...
        mock_get_access_control_role_name.assert_called_once_with(self.stack_arn, self.logical_role_name)
        mock_create_role.assert_called_once_with(RoleName=self.role_name, AssumeRolePolicyDocument=self.assume_role_policy_document_matcher, Path=self.path)
        mock_put_role_policy.assert_called_once_with(RoleName=self.role_name, PolicyName='Default', PolicyDocument=self.default_policy)
        mock_get_stack_info.assert_called_once_with(self.stack_arn)
...
class UnitTest_CloudGemFramework_ProjectResourceHandler_role_utils_delete_access_control_role(unittest.TestCase):
    ...
    @mock_aws.patch_client('iam', 'list_role_policies', side_effect = ClientError(no_such_entity_response, 'list_role_policies'), reload = role_utils)
    @mock_aws.patch_client('iam', 'delete_role_policy', return_value = delete_role_policy_response)
    @mock_aws.patch_client('iam', 'delete_role', return_value = delete_role_response)
    def test_with_list_role_policies_no_such_entity_error(self, mock_delete_role, mock_delete_role_policy, mock_list_role_policies):
        id_data = { 'AbstractRoleMappings': { self.logical_role_name: self.role_name } }
        role_utils.delete_access_control_role(id_data, self.logical_role_name)
        self.assertEquals(id_data, { 'AbstractRoleMappings': {} })
        mock_list_role_policies.assert_called_once_with(RoleName=self.role_name)
    ...
    @mock_aws.patch_client('iam', 'list_role_policies', side_effect = ClientError(access_denied_response, 'list_role_policies'), reload = role_utils)
    @mock_aws.patch_client('iam', 'delete_role_policy', return_value = delete_role_policy_response)
    @mock_aws.patch_client('iam', 'delete_role', return_value = delete_role_response)
    def test_with_list_role_policies_access_denied_error(self, mock_delete_role, mock_delete_role_policy, mock_list_role_policies):
        id_data = { 'AbstractRoleMappings': { self.logical_role_name: self.role_name } }
        role_utils.delete_access_control_role(id_data, self.logical_role_name)
        self.assertEquals(id_data, { 'AbstractRoleMappings': {} })
        mock_list_role_policies.assert_called_once_with(RoleName=self.role_name)
    ...
    @mock_aws.patch_client('iam', 'list_role_policies', side_effect = ClientError(other_error_response, 'list_role_policies'), reload = role_utils)
    @mock_aws.patch_client('iam', 'delete_role_policy', return_value = delete_role_policy_response)
    @mock_aws.patch_client('iam', 'delete_role', return_value = delete_role_response)
    def test_with_list_role_policies_other_error(self, mock_list_role_policies, mock_delete_role, mock_delete_role_policy):
        id_data = { 'AbstractRoleMappings': { self.logical_role_name: self.role_name } }
    ...
    @mock_aws.patch_client('iam', 'list_role_policies', return_value = list_role_policies_response, reload = role_utils)
    @mock_aws.patch_client('iam', 'delete_role_policy', side_effect = ClientError(no_such_entity_response, 'delete_role_policy'))
    @mock_aws.patch_client('iam', 'delete_role', return_value = delete_role_response)
    def test_with_delete_role_policy_no_such_entity_error(self, mock_delete_role, mock_delete_role_policy, mock_list_role_policies):
        id_data = { 'AbstractRoleMappings': { self.logical_role_name: self.role_name } }
        role_utils.delete_access_control_role(id_data, self.logical_role_name)
        self.assertEquals(id_data, { 'AbstractRoleMappings': {} })
        mock_list_role_policies.assert_called_once_with(RoleName=self.role_name)
    ...
    @mock_aws.patch_client('iam', 'list_role_policies', return_value = list_role_policies_response, reload = role_utils)
    @mock_aws.patch_client('iam', 'delete_role_policy', side_effect = ClientError(access_denied_response, 'delete_role_policy'))
    @mock_aws.patch_client('iam', 'delete_role', return_value = delete_role_response)
    def test_with_delete_role_policy_access_denied_error(self, mock_delete_role, mock_delete_role_policy, mock_list_role_policies):
        id_data = { 'AbstractRoleMappings': { self.logical_role_name: self.role_name } }
        role_utils.delete_access_control_role(id_data, self.logical_role_name)
        self.assertEquals(id_data, { 'AbstractRoleMappings': {} })
        mock_list_role_policies.assert_called_once_with(RoleName=self.role_name)
    ...
    @mock_aws.patch_client('iam', 'list_role_policies', return_value = list_role_policies_response, reload = role_utils)
    @mock_aws.patch_client('iam', 'delete_role_policy', side_effect = ClientError(other_error_response, 'delete_role_policy'))
    @mock_aws.patch_client('iam', 'delete_role', return_value = delete_role_response)
    def test_with_delete_role_policy_other_error(self, mock_list_role_policies, mock_delete_role_policy, mock_delete_role):
    ...
    def test_with_delete_role_no_such_entity_error(self, mock_delete_role, mock_delete_role_policy, mock_list_role_policies):
        id_data = { 'AbstractRoleMappings': { self.logical_role_name: self.role_name } }
        role_utils.delete_access_control_role(id_data, self.logical_role_name)
        self.assertEquals(id_data, { 'AbstractRoleMappings': {} })
        mock_list_role_policies.assert_called_once_with(RoleName=self.role_name)
    ...
    def test_with_delete_role_access_denied_error(self,  mock_delete_role, mock_delete_role_policy, mock_list_role_policies):
        id_data = { 'AbstractRoleMappings': { self.logical_role_name: self.role_name } }
        role_utils.delete_access_control_role(id_data, self.logical_role_name)
        self.assertEquals(id_data, { 'AbstractRoleMappings': {} })
        mock_list_role_policies.assert_called_once_with(RoleName=self.role_name)
