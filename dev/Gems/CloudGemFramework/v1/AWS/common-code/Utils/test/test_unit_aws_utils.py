...
class UnitTest_CloudGemFramework_AwsUtils_ArnParser(unittest.TestCase):
    ...
    def test_with_valid_simple_arn(self):
        ...
        # Checking arn parts
        self.assertEquals('partition', arn.partition, 'partition')
        self.assertEquals('service', arn.service, 'service')
        self.assertEquals('region', arn.region, 'region')
        self.assertEquals('account-id', arn.account_id, 'account id')
        self.assertEquals('resource-id', arn.resource_id, 'resource id')
        self.assertEquals(None, arn.resource_type, 'resource type')
        ...
        self.assertEquals(arn.arn, remade_arn)
    ...
    def __test_real_aws_arns(self):
        ...
        for arn in lambda_arns:
            ...
            self.assertEquals(parser.region, 'us-east-1')
            self.assertEquals(parser.resource_type, 'function')
        ...
        for arn in iam_arns:
            parser = aws_utils.ArnParser(arn)
            self.assertTrue(aws_utils.ArnParser.is_valid_arn(arn))
            self.assertEquals(parser.resource_type, 'role')
    ...
    def __test_with_separator(self, separator):
        ...
        self.assertEquals('account-id', arn.account_id, 'account id')
        self.assertEquals('resource-id', arn.resource_id, 'resource id')
        self.assertEquals('resource-type', arn.resource_type, 'resource type')
