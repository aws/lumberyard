...
def handler(event, context):
    ...
    try:
        ...
        if auth_settings is None:
            print('auth_settings object empty, attempting to acquire download lock.')
            with lock:
                print('lock acquired')
                if auth_settings is None:
                    ...
                    print('auth_settings initialized before lock was acquired, using cached version.')
        else:
        ...
        raise AuthTokenException(event, context, 'Unexpected error occurred. {}'.format(traceback.format_exc()))
