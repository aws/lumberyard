...
def get_openid_refresh_token(auth_code, auth_settings):
    connection = http_client.HTTPSConnection(auth_settings['provider_uri'], auth_settings['provider_port'])
...
def openid_refresh_access_token(refresh_token, auth_settings):
    connection = http_client.HTTPSConnection(auth_settings['provider_uri'], auth_settings['provider_port'])
    headers = {'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8'}
    body = 'grant_type=refresh_token&refresh_token={0}&client_id={1}&client_secret={2}'.format(refresh_token, auth_settings['client_id'],
                                                                                               auth_settings['client_secret'])
    connection.request('POST', auth_settings['provider_path'], body, headers)
    response = connection.getresponse()
