...
class UnitTest_CloudGemFramework_ResourceManagerCode_swagger_processor_FileHandling(unittest.TestCase):


    def setUp(self):
        self.__temp_files = []
        return super(UnitTest_CloudGemFramework_ResourceManagerCode_swagger_processor_FileHandling, self).setUp()


    def tearDown(self):
        for temp_file in self.__temp_files:
            os.remove(temp_file)            
        return super(UnitTest_CloudGemFramework_ResourceManagerCode_swagger_processor_FileHandling, self).tearDown()


    def test_invalid_file(self):
        time.sleep(15)
        self.assertRaises(HandledError, swagger_processor.process_swagger_path, 'C:\\bad_swagger_file_path')


    def test_invalid_json(self):
        swagger_path = self.__temp_file('{ this is invalid json }')
        self.assertRaises(HandledError, swagger_processor.process_swagger_path, swagger_path)
    ...
    def test_valid_json(self, mock_process_swagger):
        ...
        swagger_processor.process_swagger_path(swagger_path)
        expected_json = json.loads(input_json)
        mock_process_swagger.assert_called_once_with(expected_json)
...
class UnitTest_CloudGemFramework_ResourceManagerCode_swagger_processor_SwaggerProcessing(unittest.TestCase):


    def __init__(self, *args, **kwargs):
        super(UnitTest_CloudGemFramework_ResourceManagerCode_swagger_processor_SwaggerProcessing, self).__init__(*args, **kwargs)
        self.maxDiff = None


    def test_invalid_swagger(self):
        swagger = { "this is" : "not valid swagger" }
        self.assertRaises(ValueError, swagger_processor.process_swagger, swagger)
    ...
    def __remove_lambda_dispatch_object(self, object):
        if swagger_processor.LAMBDA_DISPATCH_OBJECT_NAME in object:
            del object[swagger_processor.LAMBDA_DISPATCH_OBJECT_NAME]
    ...
    def __add_error_responses(self, swagger, path, operation):
        ...
        operation_object['responses']['400'] = {
            "description": swagger_processor.RESPONSE_DESCRIPTION_400,
    ...
    def __add_integration_object(self, swagger, path, operation, lambda_arn, module, function, additional_properties = None, additional_request_template_content = None, additional_response_template_content = None, cors_enabled = True):
        ...
        operation_object[swagger_processor.API_GATEWAY_INTEGRATION_OBJECT_NAME] = integration_object
    ...
    def __add_error_definition_object(self, swagger):
        self.__add_definition_object(swagger, 'Error', swagger_processor.ERROR_RESPONSE_SCHEMA)

    def __add_empty_response_definition_object(self, swagger):
        self.__add_definition_object(swagger, 'EmptyResponse', swagger_processor.EMPTY_RESPONSE_SCHEMA)
