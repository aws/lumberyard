...
def main():
    ...
    try:
        metrics_interface = MetricsContext('cli')
        context = Context(metrics_interface)
        ...
        context.hooks.call_module_handlers('resource-manager-code/command.py', 'add_cli_view_commands', kwargs={'view_context': context.view})
        ...
        try:
            ...
            metrics_interface.submit_command_error()
        ...
        metrics_interface.set_command_name(command_name)
        metrics_interface.submit_attempt()
        ...
        try:
            ...
            metrics_interface.submit_success()

        except KeyboardInterrupt:
            metrics_interface.submit_interrupted()
            raise
        except:
            metrics_interface.submit_failure()
            raise
...
# plus any other properties added to the args object as configured by the parsers
COMMON_ARG_KEYS = set([
...
def __add_common_args(parser, no_assume_role=False):
    ...
    if not no_assume_role:
        parser.add_argument('--assume-role', '-R', metavar='ROLE-NAME',
                            help='Specifies an IAM role that will be assumed when performing the requested actions. The credentials taken from the ~/.aws/credentials file must be able to assume this role.')
    __add_bootstrap_args(parser)
...
def __add_bootstrap_args(parser):
    ...
    parser.add_argument('--user-directory',
                        help='Location of user cache directory. The default is {root-directory}\\Cache\{game}\\AWS where {game} is determined by the sys_game_folder setting in the {root-directory}\\bootstrap.cfg file.')
    parser.add_argument('--verbose', action='store_true', help='Show additional output when executing commands.')
    parser.add_argument('--no-prompt', action='store_true',
                        help='Special flag set automatically when entering from tests - calls which would raise an option for user input will instead raise an error')
    parser.add_argument('--region-override',
                        help='An override to manually indicate which region to use in your local-project-setting.json other than the default.')
...
def __add_project_stack_commands(stack_subparser):
    ...
    subparser = subparsers.add_parser('create',
                                      help='Creates the AWS resources needed for a Lumberyard project. If the {game}\\AWS directory contains no resource definitions, default ones will be created.')
    subparser.add_argument('--stack-name', help='The name used for the project stack. The default is the name of the {game} directory.')
    subparser.add_argument('--confirm-aws-usage', '-C', action='store_true',
                           help='Confirms that you know this command will create AWS resources for which you may be charged and that it may perform actions that can affect permissions in your AWS account.')
    ...
    subparser.add_argument('--files-only', action='store_true',
    ...
    subparser.add_argument('--confirm-aws-usage', '-C', action='store_true',
                           help='Confirms that you know this command will create AWS resources for which you may be charged and that it may perform actions that can affect permissions in your AWS account.')
    subparser.add_argument('--confirm-resource-deletion', '-D', action='store_true',
    ...
    subparser = subparsers.add_parser('delete',
                                      help='Deletes the AWS resources used by Lumberyard project. This command will not delete projects with deployments.')
    subparser.add_argument('--confirm-resource-deletion', '-D', action='store_true',
                           help='Confirms that you know this command will permanently delete resources.')
    __add_common_args(subparser)
    ...
    subparser.add_argument('--deployment-access', action='store_true',
                           help='Create a deployment-access-template-extensions.json file in the project directory.')
    __add_common_args(subparser)
...
def __add_resource_group_commands(group_subparser):
    ...
    subparser.add_argument('--deployment', '-d', metavar='DEPLOYMENT',
                           help='The name of the deployment that contains the resource group. If not specified, the default deployment is used.')
    ...
    subparser.add_argument('--deployment', '-d', metavar='DEPLOYMENT',
                           help='The name of the deployment used when determining the resource group status. If not specified, the default deployment is used.')
    subparser.add_argument('--show-id', action='store_true', help='Include the AWS resource id in the output.')
    __add_common_args(subparser)
    subparser.set_defaults(func=resource_group.list)

    subparser = subparsers.add_parser('upload-resources', aliases=['upload', 'update'],
                                      help='Uploads and applies changes made to local resource-template.json files.')
    ...
    subparser.add_argument('--resource-group', '-r', metavar='GROUP',
                           help='The name of the resource group to update. If not specified, all the resource groups in the deployment are updated.')
    subparser.add_argument('--confirm-aws-usage', '-C', action='store_true',
                           help='Confirms that you know this command will create AWS resources for which you may be charged and that it may perform actions that can affect permissions in your AWS account.')
    subparser.add_argument('--confirm-resource-deletion', '-D', action='store_true',
                           help='Confirms that you know this command will permanently delete resources.')
...
def __add_deployment_commands(deployment_subparser):
    ...
    subparser.add_argument('--confirm-aws-usage', '-C', action='store_true',
                           help='Confirms that you know this command will create AWS resources for which you may be charged and that it may perform actions that can affect permissions in your AWS account.')
    subparser.add_argument('--confirm-security-change', '-S', action='store_true',
                           help='Confirms that you know this command will modify AWS security configuration.')
    subparser.add_argument('--make-project-default', action='store_true',
                           help='After creation, the deployment will be set as the default deployment for the development builds of the project')
    subparser.add_argument('--make-release-deployment', action='store_true',
                           help='After creation, the deployment will be set as the deployment for the release builds of the project')
    ...
    subparser.add_argument('--parallel', action='store_true',
                           help='Deploy resource groups in parallel instead of one-at-a-time. (More likely to encounter resource limits.)')
    subparser.add_argument('--record-cognito-pools', action='store_true', help='Record the cognito pools that are stood up during deployment access updates')
    __add_common_args(subparser)
    ...
    subparser.add_argument('--confirm-resource-deletion', '-D', action='store_true',
                           help='Confirms that you know this command will permanently delete resources.')
    subparser.add_argument('--parallel', action='store_true',
                           help='Delete resource groups in parallel instead of one-at-a-time. (More likely to encounter resource limits.)')
    __add_common_args(subparser)
    ...
    subparser.add_argument('--deployment', '-d', metavar='DEPLOYMENT',
                           help='The name of the deployment to describe. If not specified, the default deployment is used.')
    subparser.add_argument('--show-id', action='store_true', help='Include the AWS resource id in the output.')
    __add_common_args(subparser)
    ...
    subparser.add_argument('--project', action='store_true',
                           help='Applies --set and --clear to the project default instead of the user default. Ignored for --show.')
    __add_common_args(subparser)
    ...
    subparser = subparsers.add_parser('protect',
                                      help='Sets the protection flag on a deployment so that the user will be warned if they try to run a protected deployment from a test or development client. Protected deployments will help reduce development code inadvertently impacting customer facing resources.')
    group = subparser.add_mutually_exclusive_group()
    ...
    subparser.add_argument('--confirm-security-change', '-S', action='store_true', help='Confirms that you know this command will make security changes.')
    subparser.add_argument('--parallel', action='store_true',
                           help='Update resource groups in parallel instead of one-at-a-time. (More likely to encounter resource limits.)')
    subparser.add_argument('--record-cognito-pools', action='store_true', help='Record the cognito pools that are stood up during deployment access updates')
    ...
    subparser.add_argument('--deployment', '-d', metavar='DEPLOYMENT', required=False,
                           help='The name of the deployment whos access stack is updated. If omitted, the default deployment is updated. Use * to update all deployments.')
    subparser.add_argument('--confirm-resource-deletion', '-D', action='store_true',
                           help='Confirms that you know this command will permanently delete resources.')
    subparser.add_argument('--confirm-security-change', '-S', action='store_true', help='Confirms that you know this command will make security changes.')
    subparser.add_argument('--confirm-aws-usage', '-C', action='store_true',
...
def __add_profile_commands(profile_subparser):
    ...
    subparser.add_argument('--aws-access-key', required=False,
                           help='The AWS access key associated with the updated profile. Default is to not change the AWS access key associated with the profile.')
    subparser.add_argument('--aws-secret-key', required=False,
                           help='The AWS secret key associated with the added profile. Default is to not change the AWS secret key associated with the profile.')
    subparser.add_argument('--profile', required=True, help='The name of the AWS profile to add.')
...
def __add_login_provider_commands(login_subparser):
    ...
    subparser = subparsers.add_parser('add',
                                      help='Adds a login provider to your Player Access template, so that you can connect your application to cognito-identity.')
    subparser.add_argument('--provider-name', required=True,
                           help='The name of the provider e.g. amazon, google, facebook. If you are using a generic openid provider it must be uniquely named here and the provider-path, provider-uri, provider-port options must be provided.')
    subparser.add_argument('--app-id', required=True,
                           help='The application id from your login provider (this is usually different from your client id though not always). This is the value that would go into your cognito-identity configuration.')
    ...
    subparser.add_argument('--provider-uri', required=False,
                           help='The URI for a generic openid connect provider. This option is only used for generic openid providers.')
    subparser.add_argument('--provider-port', required=False,
                           help='The port your provider listens on for their api. This option is only used for generic openid providers.')
    subparser.add_argument('--provider-path', required=False,
                           help='The path portion of your provider\'s uri. This option is only used for generic openid providers.')
    __add_common_args(subparser)
    subparser.set_defaults(func=player_identity.add_login_provider)

    subparser = subparsers.add_parser('update',
                                      help='Updates an existing login provider to your Player Access template, so that you can connect your application to cognito-identity.')
    subparser.add_argument('--provider-name', required=True,
                           help='The name of the provider e.g. amazon, google, facebook. If you are using a generic openid provider its unique name must be specified here and the provider-path, provider-uri, and/or provider-port options can be provided.')
    subparser.add_argument('--app-id', required=False,
                           help='The application id from your login provider (this is usually different from your client id though not always). This is the value that would go into your cognito-identity configuration.')
    ...
    subparser.add_argument('--provider-uri', required=False,
                           help='The URI for a generic openid connect provider. This option is only use for generic openid providers.')
    subparser.add_argument('--provider-port', required=False,
                           help='The port your provider listens on for their api. This option is only used for generic openid providers.')
    subparser.add_argument('--provider-path', required=False,
                           help='The path portion of your provider\'s uri. This option is only used for generic openid providers.')
    ...
    subparser.add_argument('--provider-name', required=True,
                           help='The name of the provider e.g. amazon, google, facebook; or the unique name of a generic openid provider.')
    __add_common_args(subparser)
...
def __add_parameter_commands(parameter_subparser):
    ...
    subparser.add_argument('--deployment', '-d', metavar='DEPLOYMENT', required=False,
                           help='Shows only parameters for the specified deployment. By default parameters for all deployments are shown.')
    subparser.add_argument('--resource-group', '-r', metavar='RESOURCE-GROUP', required=False,
                           help='Shows only parameters for the specified resource group. By default parameters for all resource groups are shown.')
    subparser.add_argument('--parameter', '-p', metavar='PARAMETER', required=False,
                           help='Shows only the specified parameter. By default all parameters are shown.')
    __add_common_args(subparser)
    ...
    subparser.add_argument('--deployment', '-d', metavar='DEPLOYMENT', required=True,
                           help='Sets the parameter value for the specified deployment. Use * to set the parameter for all deployments.')
    subparser.add_argument('--resource-group', '-r', metavar='RESOURCE-GROUP', required=True,
                           help='Sets the parameter value for the specified resource group. Use * to set the parameter for all resource groups.')
...
def __add_mappings_commands(mappings_subparser):
    ...
    group = subparser.add_mutually_exclusive_group()
    group.add_argument('--release', required=False, action='store_true',
                       help='Causes the release mappings to be updated. By default the mappings used during development are updated.')
    ...
    subparser.add_argument('--ignore-cache', required=False, action='store_true',
                           help="Ignores the cached mappings stored in the s3 configuration bucket, forcing a rebuild")
    __add_common_args(subparser)
...
def __add_function_commands(function_subparser):
    ...
    subparser = subparsers.add_parser('upload-code', aliases=['upload', 'update'],
                                      help='Uploads and applies changes made to local resource-template.json files.')
    subparser.add_argument('--deployment', '-d', metavar='DEPLOYMENT',
                           help='The deployment to update. If not specified the default deployment is updated. Ignored if --project is specified.')
    group = subparser.add_mutually_exclusive_group(required=True)
    group.add_argument('--resource-group', '-r', metavar='GROUP', required=False,
                       help='The name of the resource group that defines the function to be updated.')
    group.add_argument('--project', '-p', required=False, action='store_true', help='Indicates that a project defined is to be updated.')
    subparser.add_argument('--function', '-f', metavar='FUNCTION',
                           help='The name of the lambda function to update. If not specified, all the Lambda functions in the resource group are updated.')
    ...
    subparser.add_argument('--log-stream-name', '-l', metavar='LOG-STREAM', required=False,
                           help='The log stream name, or part of a log stream mane. If omitted, the most recent log stream will be shown.')
    subparser.add_argument('--function', '-f', metavar='FUNCTION', required=True, help='The logical name of a lambda function resource.')
    subparser.add_argument('--deployment', '-d', metavar='DEPLOYMENT', required=False,
                           help='The name of a deployment. If given then --resource-group must also be given. If omitted, then the function must exist in the project stack.')
    subparser.add_argument('--resource-group', '-r', metavar='RESOURCE-GROUP', required=False,
                           help='The name of a resource group. If given then --deployment must also be given.')
    __add_common_args(subparser)
    ...
    subparser.add_argument("--force", required=False, action='store_true',
                           help='Skips checks for existence of resource and type of resource. Used when creating folders for to-be-created functions.')
...
def __add_log_finder_commands(log_subparser):
    ...
    log_subparser.add_argument('--function', '-f', required=True,
                               help="The logical name of the function you want to read the logs for. in the form <ResourceGroup>.<LogicalFunctionName>")
...
def __add_deprecated_commands(context, subparsers):
    ...
    subparser.add_argument('--deployment', '-d', metavar='DEPLOYMENT', help='The deployment to update. If not specified the default deployment is updated.')
    subparser.add_argument('--resource-group', '-r', metavar='GROUP', required=True,
                           help='The name of the resource group to update. If not specified, all the resource groups in the deployment are updated.')
    subparser.add_argument('--function', metavar='FUNCTION',
                           help='The name of the lambda function to update. If not specified, all the Lambda functions in the resource group are updated.')
    __add_common_args(subparser)
    subparser.set_defaults(func=function.upload_lambda_code)
    ...
    subparser = subparsers.add_parser('list-resources')
    subparser.add_argument('--stack-id',
                           help='The ARN of the stack to list resources for. Defaults to project, deployment, or resource group stack id as determined by the --deployment and --resource-group parameters.')
    subparser.add_argument('--deployment', '-d',
                           help='The name of the deployment for which resources are listed. Defaults to listing all the project stack''s resources.')
    subparser.add_argument('--resource-group', '-r',
                           help='The name of the resource group for which resources are listed. If specified then deployment must also be specified. Defaults to listing all the deployment''s or project''s resources.')
    __add_common_args(subparser)
    ...
    subparser.add_argument('--log-stream-name', metavar='LOG-STREAM', required=False,
                           help='The log stream name, or part of a log stream mane. If omitted, the most recent log stream will be shown.')
...
def __add_built_in_commands(context, subparsers):
    ...
    subparser = subparsers.add_parser('login-provider',
                                      help='Manage login providers to your Player Access template, so that you can connect your application to cognito-identity.')
    ...
    # log finder commands
    subparser = subparsers.add_parser('logfind', aliases=['lf'], help='Find cloudwatch logs associated with a specific request')
...
def __add_hook_module_commands(context, subparsers):
    # Deprecated in 1.9. TODO: remove.
    context.hooks.call_module_handlers('cli-plugin-code/resource_commands.py', 'add_parser_commands', args=[subparsers, __add_common_args], deprecated=True)

    context.hooks.call_module_handlers('resource-manager-code/command.py', 'add_cli_commands',
                                       kwargs={'subparsers': subparsers, 'add_common_args': __add_common_args})
