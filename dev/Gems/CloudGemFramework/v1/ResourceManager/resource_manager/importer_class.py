...
class ResourceImporter:
    ...
    def add_resource(self, resource_name, resource_group, arn, download, context):
        ...
        for template in self.templates:
            ...
            # Code below assumes dict order so throw if template has more than one key
            if len(template.keys()) > 1:
                raise RuntimeError("Expect only a single key in template {}".format(template))

            if list(template.keys())[0] != resource_name:
                context.view.import_resource(list(template.keys())[0])
    ...
    @staticmethod
    def get_resource_group_path(context, resource_group_name):
        """Get the directory containing the resource_group"""
        resource_group_path = None
        if resource_group_name is not None:
            group = context.resource_groups.get(resource_group_name)  # raises HandledError if it doesn't exist
            resource_group_path = group.directory_path
        if os.path.exists(resource_group_path) and os.path.isdir(resource_group_path):
            return resource_group_path
        else:
            # Fall back behaviour based on legacy code, can covert to exception once tested
            return os.path.join(context.config.base_resource_group_directory_path, resource_group_name)
...
class S3Importer(ResourceImporter):
    ...
    def generate_templates(self, arn, resource_name, resource_group, context):
        ...
        if bucket_name not in self.bucket_list:
            raise HandledError('Resource {} with ARN {} does not exist in {}'.format(bucket_name, arn))
        # Generate the template for the resource
        output = {
            'Type': 'AWS::S3::Bucket',
            'Properties': {}
        ...
        VersioningConfiguration = {'Status': self.client.get_bucket_versioning(Bucket=bucket_name).get('Status', '')}
    ...
    def _get_cors_configuration(self, bucket_name):
        try:
            cors_configuration = {'CorsRules': self.client.get_bucket_cors(Bucket=bucket_name).get('CORSRules', [])}
    ...
    def _get_lifecycle_configuration(self, bucket_name):
        try:
            lifecycle_configuration = {'Rules': self.client.get_bucket_lifecycle_configuration(Bucket=bucket_name).get('Rules', [])}
        except ClientError as e:
    ...
    def _get_notification_configuration(self, bucket_name, resource_name, context):
        notification = self.client.get_bucket_notification_configuration(Bucket=bucket_name)
        ...
        for service in notification.get(service_type + 'Configurations', []):
            ...
            for event in service['Events']:
                ...
                if service.get('Filter'):
                    service_configurations.append({'Event': event, 'Filter': service.get('Filter'), service_type_name: service_type_name_detail})
                else:
                    service_configurations.append({'Event': event, service_type_name: service_type_name_detail})
            count = count + 1
    ...
    def _get_website_configuration(self, bucket_name):
        try:
            website_configuration = self.client.get_bucket_website(Bucket=bucket_name)
        except ClientError as e:
...
class LambdaImporter(ResourceImporter):
    def __init__(self, region, context):
        ...
        # Use a list to record the dependency with DynamoDB table
    ...
    def add_resource(self, resource_name, resource_group, arn, download, context):
        ...
        # Download the code
        resource_dir = ResourceImporter.get_resource_group_path(context=context, resource_group_name=resource_group)
        description = self.client.get_function(FunctionName=arn.split(':')[-1])
        runtime = description['Configuration']['Runtime']

        location = description['Code']['Location']
        zip_content = urlopen(location)
        if 'python' in runtime:
            # If python lambda we can use loose files
            zipped_file = zipfile.ZipFile(io.BytesIO(zip_content.read()))
            extract_path = os.path.join(resource_dir, constant.LAMBDA_CODE_DIRECTORY_NAME, resource_name)
            zipped_file.extractall(path=extract_path)
        else:
            # Write lambda zip
            extract_path = os.path.join(resource_dir, constant.LAMBDA_CODE_DIRECTORY_NAME, resource_name + '.zip')
            with open(extract_path, 'wb') as zipped_file:
                content = io.BytesIO(zip_content.read())
                zipped_file.write(content.read())
...
class SQSImporter(ResourceImporter):
    ...
    def generate_templates(self, arn, resource_name, resource_group, context):
        ...
        description = self.client.get_queue_attributes(QueueUrl=resource_url, AttributeNames=['All'])['Attributes']
        # Generate the template for the resource
        output = {
            'Type': 'AWS::SQS::Queue',
            'Properties': {}
