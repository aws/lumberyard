...
class BaseServiceApiTestCase(base_stack_test.BaseStackTestCase):
    """ Support class to share basic test cases between our normal service API integration test and custom domain name integration test. """
    ...
    def __init__(self, *args, **kwargs):
        super(BaseServiceApiTestCase, self).__init__(*args, **kwargs)

    def test_end_to_end(self):
        self.run_all_tests()

    def setup_deployment_stacks(self):
    ...
    def add_service_api_resources(self):
    ...
    def verify_service_api_resources(self):
    ...
    def call_simple_api_with_no_credentials(self):
    ...
    def add_apis_with_optional_parameters(self):
        with_defaults = True
    ...
    def verify_service_api_mapping(self):
    ...
    def call_complex_api_using_player_credentials(self):
    ...
    def call_complex_api_using_project_admin_credentials(self):
    ...
    def call_complex_api_using_deployment_admin_credentials(self):
    ...
    def call_complex_api_with_missing_string_pathparam(self):
        self.__call_complex_api('string', None, 'b', 'c', expected_status_code=404)

    def call_complex_api_with_missing_string_queryparam(self):
        self.__call_complex_api('string', 'a', None, 'c', expected_status_code=400)

    def call_complex_api_with_missing_string_bodyparam(self):
        self.__call_complex_api('string', 'a', 'b', None, expected_status_code=400)

    def call_api_with_both_parameters(self):
    ...
    def call_interface_directly_with_player_credential(self):
    ...
    def call_interface_directly_without_credential(self):
        client = self.get_service_client(use_aws_auth=False,
                                         stack_id=self.get_resource_group_stack_arn(self.TEST_DEPLOYMENT_NAME, self.TEST_RESOURCE_GROUP_NAME))
        body = {'test': 'data'}
        param = 'test'
    ...
    def invoke_interface_caller(self):
        self.refresh_stack_resources(self.get_deployment_stack_arn(self.TEST_DEPLOYMENT_NAME))
    ...
    def remove_service_api_resources(self):
...
'''
    ...
    def __STACK_CLOUD_GEMS_TO_MODIFY(self):
        return [
                self.TEST_RESOURCE_GROUP_NAME,
                self.INTERFACE_DEFINER_GEM_NAME,
...
def handler(event, context):
    interface_url = cgf_lambda_settings.get_service_url("{INTERFACE_ID}")
