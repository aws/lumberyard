...
export class CloudWatchLogComponent implements Facetable {
    ...
    ngOnInit() {
        ...
        if (this.data["physicalResourceId"])
            this.getCloudWatchLogByStackId(this.data["physicalResourceId"], this.aws.context, this.model)
        else
            this.getCloudWatchLogByLogId(this.data["cloudwatchPhysicalId"], this.aws.context, this.model)
    }
    ...
    getCloudWatchLogByLogId(logId: string, context: AwsContext, model: any): void {
    ...
    getCloudWatchLogByStackId(stackId: string, context: AwsContext, model: any): void {
        var params = {
            StackName: stackId
        };
        this.model.loading = true;
        context.cloudFormation.describeStackResources(params, (err, data) => {
            if (err) {
                model.loading = false;
                ...
                for (var i = 0; i < data.StackResources.length; i++) {
                    ...
                    if (spec[Schema.CloudFormation.RESOURCE_TYPE.NAME] == Schema.CloudFormation.RESOURCE_TYPE.TYPES.LAMBDA) {
                        this.getCloudWatchLogByLogId(spec.PhysicalResourceId, context, model)
                    }
                }
            }
        });
