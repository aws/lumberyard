...
def add_cli_commands(hook, subparsers, add_common_args, **kwargs):
    ...
    subparser.add_argument('--sensitivity-type', required=False, default=None,
                           help='Use S3 native AES256 encryption for the events generated. Options: sensitive | insensitive ')
    subparser.add_argument('--compression-type', required=False, default=None,
                           help='Used to defined whether metric payloads should be compressed.  Enabling will reduce performance. Options: nocompression | compress ')
    subparser.add_argument('--threads', type=int, required=False, default=1,
                           help='Number of threads to send metrics with.')
    subparser.add_argument('--iterations-per-thread', type=int, required=False, default=1,
                           help='Number of iterations per thread')
    subparser.add_argument('--events-per-iteration', type=int, required=False, default=1,
                           help='Number of events to send per iteration per thread')
    subparser.add_argument('--erroneous-metrics', action='store_true', required=False, default=False,
                           help='Send erroneous events')
    subparser.add_argument('--sleep-duration-between-jobs', type=float, required=False, default=0,
                           help='The period in seconds in which to sleep between thread job assignments')
    subparser.add_argument('--use-lambda', action='store_true', required=False, default=False,
                           help='Run the SQS message producer locally or invoke the producer lambda')
    subparser.add_argument('--event-type', required=False, default='random_event',
                           help='Use only a specific type of event.  Options: random_event | defect | shotfired | shotmissed | shotdodged | bug')
    add_common_args(subparser)
    subparser.set_defaults(func=producer.cli)

    import amoeba_launcher
    subparser = metric_subparsers.add_parser('combine-s3-files',
                                             help='Combine S3 files into a single S3 file to improve Athena query response times.  By default this runs in AWS every 20 minutes.')
    add_common_args(subparser)
    subparser.set_defaults(func=amoeba_launcher.cli)

    import graph
    subparser = metric_subparsers.add_parser('graph', help='View Cloudwatch metrics.')
    subparser.add_argument('--function', required=True, default='avg_save_duration',
                           help='The graph api function to execute.  Options are: consumer_save_duration, consumer_messages_processed, consumer_bytes_processed, consumer_rows_added, lambda_invocations, consumer_sqs_delete_duration, dynamodb_context_read_provisioned, dynamodb_context_read_consumed, dynamodb_context_write_provisioned, dynamodb_context_write_consumed, avg_save_duration, avg_delete_duration')
    add_common_args(subparser)
    ...
    if sys.platform == 'win32':
        ...
        subparser.add_argument('--s3-key', required=False, default=None,
                               help='The S3 key to open.  Example: /insensitive/cloudgemmetric')
        add_common_args(subparser)
    ...
    subparser.add_argument('--function', required=False, default='handler',
                           help='The context api function to execute.  Options are: handle, start_crawler, stop_crawler')
