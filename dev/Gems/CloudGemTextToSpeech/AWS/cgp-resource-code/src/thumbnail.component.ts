...
import { TextToSpeechApi } from './index'
...
export class TextToSpeechThumbnailComponent implements Tackable {
    @Input() context: any
    @Input() displayName: string = "Text to Speech";
    @Input() srcIcon: string = "https://m.media-amazon.com/images/G/01/cloudcanvas/images/Text_to_Speech_GEM_optimized._V518452892_.png"
    ...
    ngOnInit() {
        this._apiHandler = new TextToSpeechApi(this.context.ServiceUrl, this.http, this.aws, this.metricservice, this.context.identifier);
        this.report(this.metric);
        this.assign(this.state);
    }

    public report(metric: Measurable) {
        metric.name = "Speech Lines";
        metric.value = "Loading...";
        this._apiHandler.get("speechlib").subscribe(response => {
            let obj = JSON.parse(response.body.text());
            metric.value = obj.result.entries.length;
        }, err => {
