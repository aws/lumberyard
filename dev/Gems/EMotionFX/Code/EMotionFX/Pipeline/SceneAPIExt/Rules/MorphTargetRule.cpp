        ...
        namespace Rule
        {
            ...
            size_t MorphTargetRule::SelectMorphTargets(const AZ::SceneAPI::Containers::Scene & scene, AZ::SceneAPI::DataTypes::ISceneNodeSelectionList & selection)
            {
                ...
                for (auto it = filteredView.begin(); it != filteredView.end(); ++it)
                {
                    ...
                    if (types.find(AZ::SceneAPI::Events::GraphMetaInfo::s_ignoreVirtualType) == types.end())
                    {
            ...
            {
                auto eventId = LyMetrics_CreateEvent(MORPH_TARGET_RULE_ADD_METRIC_EVENT_NAME);
                LyMetrics_SubmitEvent(eventId);
            }

            void MorphTargetRule::OnUserRemoved() const
            {
                auto eventId = LyMetrics_CreateEvent(MORPH_TARGET_RULE_REMOVE_METRIC_EVENT_NAME);
                LyMetrics_SubmitEvent(eventId);
            }
