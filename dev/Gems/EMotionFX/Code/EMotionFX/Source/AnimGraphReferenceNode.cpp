...
namespace EMotionFX
{
    ...
    void AnimGraphReferenceNode::LoadAnimGraphAsset()
    {
        if (m_animGraphAsset.GetId().IsValid())
        {
            ...
            // (that would generate an inifinite loop)

            AZ::Data::AssetBus::MultiHandler::BusConnect(m_animGraphAsset.GetId());

            if (!m_animGraphAsset.IsLoading()) // someone else is loading it, we will get notified
            {
                if (!m_animGraphAsset.IsReady())
                {
    ...
    void AnimGraphReferenceNode::LoadMotionSetAsset()
    {
        if (m_motionSetAsset.GetId().IsValid())
        {
            ...
            if (!m_motionSetAsset.IsLoading()) // someone else is loading it, we will get notified
            {
                if (!m_motionSetAsset.IsReady())
                {
                    if (!m_motionSetAsset.QueueLoad())
                    {
                        // When switching between assets I have seen that the asset is not ready, we call QueueLoad, it fails
                        // because the asset is already loaded, and at that point it assigns the right m_assetData
                        AZ_Assert(m_motionSetAsset.IsReady(), "Expecting a ready motion set asset");
                        OnMotionSetAssetReady();
                    }
                }
                else
    ...
    void AnimGraphReferenceNode::OnAnimGraphAssetReady()
    {
        ...
        m_lastProcessedAnimGraphAsset = m_animGraphAsset;
    }
