...
namespace EMotionFX
{
    ...
    MotionInstance* MotionGroup::AddMotion(Motion* motion, PlayBackInfo* playInfo, uint32 startNodeIndex)
    {
        ...
        // TODO: add security check for mixing and additive motions? (like additive motions need to be mixing or so)

        // create the motion links
        //newInstance->GetMotion()->CreateMotionLinks( newInstance->GetActorInstance()->GetActor(), newInstance );
        newInstance->InitForSampling();
    ...
    void MotionGroup::Output(const Pose* inPose, Pose* outPose)
    {
        ...
        ActorInstance* actorInstance = inPose->GetActorInstance();
