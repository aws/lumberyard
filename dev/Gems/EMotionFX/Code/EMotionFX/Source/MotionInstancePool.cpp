...
namespace EMotionFX
{
    ...
    MotionInstance* MotionInstancePool::RequestNewWithoutLock(Motion* motion, ActorInstance* actorInstance, uint32 startNodeIndex)
    {
        ...
        if (mPool->mFreeList.GetLength() > 0)
        {
            ...
            MotionInstance* result = MotionInstance::Create(location.mAddress, motion, actorInstance, startNodeIndex);
    ...
    MotionInstance* MotionInstancePool::RequestNew(Motion* motion, ActorInstance* actorInstance, uint32 startNodeIndex)
    {
        Lock();
        MotionInstance* result = RequestNewWithoutLock(motion, actorInstance, startNodeIndex);
        Unlock();
