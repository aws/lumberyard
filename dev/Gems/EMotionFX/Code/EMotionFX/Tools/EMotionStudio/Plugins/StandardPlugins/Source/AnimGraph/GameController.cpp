...
HRESULT GameController::InitDirectInput(HWND hWnd)
{
    ...
    {
        MCore::LogWarning("GameController::Init() - No Joystick found!");
        mValid = false;
        //MessageBox( nullptr, TEXT( "Joystick not found." ), TEXT("Info Message"), MB_ICONINFO | MB_OK );
        return S_OK;
    }
...
void GameController::LogError(HRESULT value, const char* text)
{
    // only log errors
...
#include <EMotionFX/Tools/EMotionStudio/Plugins/StandardPlugins/Source/AnimGraph/GameController.moc>
