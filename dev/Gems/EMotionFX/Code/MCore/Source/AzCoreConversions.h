...
namespace MCore
{
    ...
    AZ_FORCE_INLINE void LookAt(AZ::Transform& transform, const AZ::Vector3& view, const AZ::Vector3& target, const AZ::Vector3& up)
    {
        const AZ::Vector3 z = (target - view).GetNormalized();
        const AZ::Vector3 x = (up.Cross(z)).GetNormalized();
        const AZ::Vector3 y = z.Cross(x);

        transform.SetRow(0, x, -x.Dot(view));
        transform.SetRow(1, y, -y.Dot(view));
        transform.SetRow(2, z, -z.Dot(view));
    ...
    AZ_FORCE_INLINE AZ::Transform GetRotationMatrixAxisAngle(const AZ::Vector3& axis, float angle)
    {
        ...
        AZ::Transform mat = AZ::Transform::CreateIdentity();
        mat.SetElement(0, 0, xx);
        mat.SetElement(1, 0, xy + zs);
        mat.SetElement(2, 0, zx - ys);
        mat.SetElement(0, 1, xy - zs);
        mat.SetElement(1, 1, yy);
        mat.SetElement(2, 1, yz + xs);
        mat.SetElement(0, 2, zx + ys);
        mat.SetElement(1, 2, yz - xs);
        mat.SetElement(2, 2, zz);
        return mat;
    }


    AZ_FORCE_INLINE void Skin(const AZ::Transform& inMat, const AZ::Vector3* inPos, const AZ::Vector3* inNormal, AZ::Vector3* outPos, AZ::Vector3* outNormal, float weight)
    {
        outPos->Set(
            outPos->GetX() + (inPos->GetX() * inMat(0, 0) + inPos->GetY() * inMat(0, 1) + inPos->GetZ() * inMat(0, 2) + inMat(0, 3)) * weight,
            outPos->GetY() + (inPos->GetX() * inMat(1, 0) + inPos->GetY() * inMat(1, 1) + inPos->GetZ() * inMat(1, 2) + inMat(1, 3)) * weight,
            outPos->GetZ() + (inPos->GetX() * inMat(2, 0) + inPos->GetY() * inMat(2, 1) + inPos->GetZ() * inMat(2, 2) + inMat(2, 3)) * weight
        );

        outNormal->Set(
            outNormal->GetX() + (inNormal->GetX() * inMat(0, 0) + inNormal->GetY() * inMat(0, 1) + inNormal->GetZ() * inMat(0, 2)) * weight,
            outNormal->GetY() + (inNormal->GetX() * inMat(1, 0) + inNormal->GetY() * inMat(1, 1) + inNormal->GetZ() * inMat(1, 2)) * weight,
            outNormal->GetZ() + (inNormal->GetX() * inMat(2, 0) + inNormal->GetY() * inMat(2, 1) + inNormal->GetZ() * inMat(2, 2)) * weight
        );
    }

    AZ_FORCE_INLINE void Skin(const AZ::Transform& inMat, const AZ::Vector3* inPos, const AZ::Vector3* inNormal, const AZ::Vector4* inTangent, AZ::Vector3* outPos, AZ::Vector3* outNormal, AZ::Vector4* outTangent, float weight)
    {
        outPos->Set(
            outPos->GetX() + (inPos->GetX() * inMat(0, 0) + inPos->GetY() * inMat(0, 1) + inPos->GetZ() * inMat(0, 2) + inMat(0, 3)) * weight,
            outPos->GetY() + (inPos->GetX() * inMat(1, 0) + inPos->GetY() * inMat(1, 1) + inPos->GetZ() * inMat(1, 2) + inMat(1, 3)) * weight,
        ...
        outTangent->Set(
            outTangent->GetX() + (inTangent->GetX() * inMat(0, 0) + inTangent->GetY() * inMat(0, 1) + inTangent->GetZ() * inMat(0, 2)) * weight,
            outTangent->GetY() + (inTangent->GetX() * inMat(1, 0) + inTangent->GetY() * inMat(1, 1) + inTangent->GetZ() * inMat(1, 2)) * weight,
            outTangent->GetZ() + (inTangent->GetX() * inMat(2, 0) + inTangent->GetY() * inMat(2, 1) + inTangent->GetZ() * inMat(2, 2)) * weight,
            inTangent->GetW()
        );
    }

    AZ_FORCE_INLINE void Skin(const AZ::Transform& inMat, const AZ::Vector3* inPos, const AZ::Vector3* inNormal, const AZ::Vector4* inTangent, const AZ::Vector3* inBitangent, AZ::Vector3* outPos, AZ::Vector3* outNormal, AZ::Vector4* outTangent, AZ::Vector3* outBitangent, float weight)
    {
        outPos->Set(
            outPos->GetX() + (inPos->GetX() * inMat(0, 0) + inPos->GetY() * inMat(0, 1) + inPos->GetZ() * inMat(0, 2) + inMat(0, 3)) * weight,
            outPos->GetY() + (inPos->GetX() * inMat(1, 0) + inPos->GetY() * inMat(1, 1) + inPos->GetZ() * inMat(1, 2) + inMat(1, 3)) * weight,
        ...
        outBitangent->Set(
            outBitangent->GetX() + (inBitangent->GetX() * inMat(0, 0) + inBitangent->GetY() * inMat(0, 1) + inBitangent->GetZ() * inMat(0, 2)) * weight,
            outBitangent->GetY() + (inBitangent->GetX() * inMat(1, 0) + inBitangent->GetY() * inMat(1, 1) + inBitangent->GetZ() * inMat(1, 2)) * weight,
            outBitangent->GetZ() + (inBitangent->GetX() * inMat(2, 0) + inBitangent->GetY() * inMat(2, 1) + inBitangent->GetZ() * inMat(2, 2)) * weight
        );
    }
