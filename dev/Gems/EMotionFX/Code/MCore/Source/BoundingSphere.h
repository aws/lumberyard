...
namespace MCore
{
    ...
    public:
        ...
        MCORE_INLINE BoundingSphere()
            : mCenter(Vector3(0.0f, 0.0f, 0.0f))
            , mRadius(0.0f)
            , mRadiusSq(0.0f) {}
         ...
         */
        MCORE_INLINE BoundingSphere(const Vector3& pos, float rad)
            : mCenter(pos)
            , mRadius(rad)
            , mRadiusSq(rad * rad) {}
         ...
         */
        MCORE_INLINE BoundingSphere(const Vector3& pos, float rad, float radSq)
            : mCenter(pos)
            , mRadius(rad)
            , mRadiusSq(radSq)  {}
         ...
         */
        MCORE_INLINE void Init()                                                                { mCenter.Zero(); mRadius = mRadiusSq = 0.0f; }
         ...
         */
        MCORE_INLINE void EncapsulateFast(const Vector3& v)
        {
            Vector3 diff = (mCenter - v);
         ...
         */
        MCORE_INLINE bool Contains(const Vector3& v)                                            { return ((mCenter - v).SquareLength() <= mRadiusSq); }
         ...
         */
        MCORE_INLINE bool Contains(const BoundingSphere& s) const                               { return ((mCenter - s.mCenter).SquareLength() <= (mRadiusSq - s.mRadiusSq)); }
         ...
         */
        MCORE_INLINE bool Intersects(const BoundingSphere& s) const                             { return ((mCenter - s.mCenter).SquareLength() <= (mRadiusSq + s.mRadiusSq)); }
    ...
    private:
        Vector3 mCenter;    /**< The center of the sphere. */
