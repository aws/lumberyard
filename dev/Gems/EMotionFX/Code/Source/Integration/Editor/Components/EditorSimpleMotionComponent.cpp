...
namespace EMotionFX
{
    namespace Integration
    {
        ...
        float EditorSimpleMotionComponent::GetAssetDuration(const AZ::Data::AssetId& assetId)
        {
            ...
            AZ::Data::Asset<MotionAsset> motionAsset = AZ::Data::AssetManager::Instance().GetAsset<MotionAsset>(assetId, true, nullptr, true);

            if (motionAsset && motionAsset.Get()->m_emfxMotion)
            {
                result = motionAsset.Get()->m_emfxMotion.get()->GetMaxTime();
            }
        ...
        void EditorSimpleMotionComponent::Motion(AZ::Data::AssetId assetId)
        {
            if (m_configuration.m_motionAsset.GetId() != assetId)
            {
                ...
                else
                {
                    ...
                    if (assetId.IsValid())
                    {
                        m_configuration.m_motionAsset = AZ::Data::AssetManager::Instance().GetAsset<MotionAsset>(assetId);
                    }
                }
