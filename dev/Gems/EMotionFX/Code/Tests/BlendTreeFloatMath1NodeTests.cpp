...
namespace EMotionFX
{
        ...
        void TestInput(const AZStd::string& paramName, std::vector<inputType> xInputs)
        {
            ...
            for (const inputType& i : xInputs)
            {
                ...
                for (AZ::u8 j = 0; j < BlendTreeFloatMath1Node::MATHFUNCTION_NUMFUNCTIONS; j++)
                {
                    ...
                    EXPECT_FLOAT_EQ(actualOutput, expectedOutput) << "Actual and expected outputs does not match.";
                }
            }
    ...
    private:
        ...
        {
            if ((input > -AZ_FLT_EPSILON) && (input < AZ_FLT_EPSILON))
            {
                return 1.0f;
            }
            return 0.0f;
