...
namespace EMotionFX
{
    ...
    public:
        RaycastRequests::RaycastResult Raycast(AZ::EntityId entityId, const RaycastRequests::RaycastRequest& rayRequest) override
        {
            ...
            const AZ::Vector3 planeNormal = (m_sceneTransform * AZ::Vector3(0.0f, 0.0f, 1.0f)).GetNormalizedSafeExact();
            const AZ::Vector3 pointOnUpperPlane = m_sceneTransform * AZ::Vector3(0.0f, 0.0f, m_rightSideHeight);
            const AZ::Vector3 pointOnLowerPlane = m_sceneTransform * AZ::Vector3(0.0f, 0.0f, m_leftSideHeight);
