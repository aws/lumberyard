...
namespace EMotionFX
{
    ...
    public:
        ...
        void ConstructGraph() override
        {
            ...
            Motion* rightAramUpMotion = EMotionFX::GetImporter().LoadSkeletalMotion(skeletalMotiondata.begin(), skeletalMotiondata.size());
    ...
    TEST_F(BlendTreeMirrorPoseNodeFixture, OutputsCorrectPose)
    {
        ...
        EXPECT_TRUE(mirroredl_upArmPos.GetX().IsClose(-r_upArmPos.GetX()));
        EXPECT_TRUE(mirroredl_upArmPos.GetY().IsClose(r_upArmPos.GetY()));
        EXPECT_TRUE(mirroredl_upArmPos.GetZ().IsClose(r_upArmPos.GetZ()));

        EXPECT_TRUE(mirroredr_upArmPos.GetX().IsClose(-l_upArmPos.GetX()));
        EXPECT_TRUE(mirroredr_upArmPos.GetY().IsClose(l_upArmPos.GetY()));
        EXPECT_TRUE(mirroredr_upArmPos.GetZ().IsClose(l_upArmPos.GetZ()));

        EXPECT_TRUE(mirroredl_loArmPos.GetX().IsClose(-r_loArmPos.GetX()));
        EXPECT_TRUE(mirroredl_loArmPos.GetY().IsClose(r_loArmPos.GetY()));
        EXPECT_TRUE(mirroredl_loArmPos.GetZ().IsClose(r_loArmPos.GetZ()));

        EXPECT_TRUE(mirroredr_loArmPos.GetX().IsClose(-l_loArmPos.GetX()));
        EXPECT_TRUE(mirroredr_loArmPos.GetY().IsClose(l_loArmPos.GetY()));
        EXPECT_TRUE(mirroredr_loArmPos.GetZ().IsClose(l_loArmPos.GetZ()));

        EXPECT_TRUE(mirroredl_handPos.GetX().IsClose(-r_handPos.GetX()));
        EXPECT_TRUE(mirroredl_handPos.GetY().IsClose(r_handPos.GetY()));
        EXPECT_TRUE(mirroredl_handPos.GetZ().IsClose(r_handPos.GetZ()));

        EXPECT_TRUE(mirroredr_handPos.GetX().IsClose(-l_handPos.GetX()));
        EXPECT_TRUE(mirroredr_handPos.GetY().IsClose(l_handPos.GetY()));
        EXPECT_TRUE(mirroredr_handPos.GetZ().IsClose(l_handPos.GetZ()));
    };
