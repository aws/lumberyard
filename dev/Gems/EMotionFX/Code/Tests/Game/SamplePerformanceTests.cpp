...
namespace EMotionFX
{
    ...
    struct PerformanceTestParameters
    {
        ...
        void Print() const
        {
            ...
            AZ_Printf("EMotionFX", "- FPS:            %f\n", m_fps);
            AZ_Printf("EMotionFX", "- Motion Sampling Rate:  %f\n", m_motionSamplingRate);
            AZ_Printf("EMotionFX", "- Total Time (s): %f\n", m_totalTestTimeInSeconds);
            const size_t numFrames = static_cast<int>(m_totalTestTimeInSeconds * m_fps);
            AZ_Printf("EMotionFX", "- Frames:         %d\n", numFrames);
            AZ_Printf("EMotionFX", "- Frames:         %d\n", numFrames);
            AZ_Printf("EMotionFX", "- Num Instances:  %d\n", m_numInstances);
            AZ_Printf("EMotionFX", "- Num Skin Attachents per Instance: %d\n", m_numSkinAttachmentsPerInstance);
            AZ_Printf("EMotionFX", "- LOD Level:      %d\n", m_lodLevel);
            AZ_Printf("EMotionFX", "- Software Skin:  %s\n", m_includeSoftwareSkinning ? "True" : "False");
            AZ_Printf("EMotionFX", "- Multi-threading:%s\n", m_useMultiThreading ? "True" : "False");
        }
    };
    ...
    public:
            ...
            float totalMeshDeformTime)
        {
            ...
            {
                AZ_Printf("EMotionFX", "- Totals:\n");
                AZ_Printf("EMotionFX", "    Total Time (s):                  %.4f s\n", totalTransformUpdateTime + totalMeshDeformTime);
                AZ_Printf("EMotionFX", "    Total Transform Update Time (s): %.4f s\n", totalTransformUpdateTime);
                AZ_Printf("EMotionFX", "    Total Mesh Deform Time (s):      %.4f s\n", totalMeshDeformTime);
                AZ_Printf("EMotionFX", "    Transform Mesh Ratio:            %.4f %%\n", totalTransformUpdateTime / totalMeshDeformTime);
            }
            else
            {
                AZ_Printf("EMotionFX", "- Total Time (s):                    %.4f s\n", totalTransformUpdateTime + totalMeshDeformTime);
            ...
            if (param.m_includeSoftwareSkinning)
            {
                ...
                AZ_Printf("EMotionFX", "- Mesh deforms:\n");
                AZ_Printf("EMotionFX", "    Best Frame:                      %.4f ms\n", meshDeformBest * 1000.0f);
                AZ_Printf("EMotionFX", "    Mean Frame:                      %.4f ms\n", meshDeformMean * 1000.0f);
                AZ_Printf("EMotionFX", "    Worst Frame:                     %.4f ms\n", meshDeformWorst * 1000.0f);
    ...
    TEST_P(PerformanceTestFixture, PerformanceTest)
    {
        const PerformanceTestParameters& param = GetParam();
