...
namespace EMotionFX
{
    ...
    public:
        ...
        {
            EXPECT_TRUE(AZ::IsClose(m_motionInstance->GetCurrentTime(), state.m_currentTime, AZ::g_fltEps)) << "Expected the current play time to be different.";
            EXPECT_TRUE(AZ::IsClose(m_motionInstance->GetLastCurrentTime(), state.m_lastCurrentTime, AZ::g_fltEps)) << "Expected the last current play time to be different.";
