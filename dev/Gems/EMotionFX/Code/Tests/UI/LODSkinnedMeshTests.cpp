...
namespace EMotionFX
{
    ...
    TEST_P(LODSkinnedMeshFixture, CheckLODLevels)
    {
        ...
        const QTreeWidgetItem* meshNodeItem = treeWidget->topLevelItem(0)->child(0);
        EXPECT_TRUE(meshNodeItem);

        const QRect rect = treeWidget->visualItemRect(meshNodeItem);
        ...
        LODPropertyRowWidget* finalRowWidget = reinterpret_cast<LODPropertyRowWidget * >(GetNamedPropertyRowWidgetFromReflectedPropertyEditor(propertyWidget, "Meshes by lod"));
        EXPECT_TRUE(finalRowWidget);
    ...
    TEST_F(LODSkinnedMeshColorFixture, CheckLODDistanceChange)
    {
        ...
        env.p3DEngine = nullptr;
        AZStd::unique_ptr<DataMembers> data;
        data = AZStd::make_unique<DataMembers>();
        env.pRenderer = &data->m_renderer;
        env.pSystem = &data->m_system;
        gEnv = &env;
        ...
        Vec3 newVec{ 0,30,0 };
        data->m_system.SetViewCameraPosition(newVec);
