...
namespace EditorPythonBindings
{
    namespace Internal
    {
            ...
            bool AddCallback(AZStd::string_view eventName, pybind11::function callback)
            {
                ...
                for (int iEvent = 0; iEvent < static_cast<int>(events.size()); ++iEvent)
                {
                    ...
                    if (eventName == e.m_name)
                    {
                        ...
                        auto& callbackIt = m_callbackMap.find(eventNameValue);
        ...
        protected:
            ...
            void OnEventGenericHook(const char* eventName, int eventIndex, AZ::BehaviorValueParameter* result, int numParameters, AZ::BehaviorValueParameter* parameters)
            {
                // find the callback for the event
                auto& callbackEntry = m_callbackMap.find(eventName);
