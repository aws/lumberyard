...
namespace pybind11
{
    namespace detail
    {
        ...
        //! Converts AZ::VectorFloat to/from Python Float
        template <>
        struct type_caster<AZ::VectorFloat>
        {
        public:
            PYBIND11_TYPE_CASTER(AZ::VectorFloat, _("AZ::VectorFloat"));

            bool load(handle pythonSource, bool)
            {
                if (PyFloat_Check(pythonSource.ptr()))
                {
                    value = pybind11::cast<float>(pythonSource);
                    return true;
                }
                return false;
            }

            static handle cast(const AZ::VectorFloat& src, return_value_policy policy, handle parent)
            {
