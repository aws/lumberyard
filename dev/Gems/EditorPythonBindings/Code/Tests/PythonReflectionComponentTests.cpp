...
namespace UnitTest
{
    ...
    struct PythonReflectionTestDoPrint
    {
        ...
        static const char* DoTest(const char* label)
        {
            return "proxy_do_test";
        }

        static void DoPrint(const char* msg)
        {
    ...
    struct PythonReflectionAnotherModule
    {
        AZ_TYPE_INFO(PythonReflectionAnotherModule, "{F1935F7F-6A22-4CA3-BEE7-A2F8E8D5D35F}");
    ...
    struct PythonReflectionContainerSimpleTypes
    {
        ...
        void Reflect(AZ::ReflectContext* context)
        {
            if (AZ::SerializeContext* serializeContext = azrtti_cast<AZ::SerializeContext*>(context))
            {
                ...
                serializeContext->RegisterGenericType<AZStd::vector<AZ::VectorFloat>>();
            }
    ...
    struct PythonReflectionStringTypes
    {
        ...
        static void RawPointerIn(const char* value)
        {
        ...
        static void StringViewIn(AZStd::string_view value)
        {
        ...
        static void AZStdStringIn(const AZStd::string& value)
        {
    ...
    TEST_F(PythonReflectionComponentTests, InstallingPythonReflectionComponent)
    {
        ...
        e.Init();
        EXPECT_EQ(AZ::Entity::State::ES_INIT, e.GetState());
        e.Activate();
        EXPECT_EQ(AZ::Entity::State::ES_ACTIVE, e.GetState());
    ...
    TEST_F(PythonReflectionComponentTests, AtLeast3DeepModules)
    {
        ...
        {
            Skip = 0,
            PythonReflectionAnotherModule_DoWork,
            PythonReflectionTestSimple_DoWork,
        ...
        m_testSink.m_evaluateMessage = [](const char* window, const char* message) -> int
        {
            if (AzFramework::StringFunc::Equal(window, "python"))
            {
                ...
                else if (AzFramework::StringFunc::Equal(message, "PythonReflectionAnotherModule_DoWork"))
                {
                    return static_cast<int>(LogTypes::PythonReflectionAnotherModule_DoWork);
                }
        ...
        try
        {
            pybind11::exec(R"(
                ...
                import azlmbr.legacy.test.subtest
                azlmbr.default.PythonReflectionTestSimple_do_work()
                azlmbr.legacy.test.PythonReflectionTestDoPrint_do_print('PythonReflection_DoPrint')
                azlmbr.legacy.test.subtest.PythonReflectionAnotherModule_do_work()
            )");
        }
        catch (const std::exception& e)
        {
        ...
        EXPECT_EQ(1, m_testSink.m_evaluationMap[static_cast<int>(LogTypes::PythonReflectionAnotherModule_DoWork)]);
        EXPECT_EQ(1, m_testSink.m_evaluationMap[static_cast<int>(LogTypes::PythonReflectionTestSimple_DoWork)]);
        ...
        void Reflect(AZ::ReflectContext* context)
        {
            ...
            if (AZ::BehaviorContext* behaviorContext = azrtti_cast<AZ::BehaviorContext*>(context))
            {
                behaviorContext->Class<PythonReflectionTypesTester>("TypeTests")
                    ...
                    ->Method("return_vectorFloat", &ReturnValue<AZ::VectorFloat>, nullptr, "")
                    ->Method("accept_vectorFloat", &AcceptValue<AZ::VectorFloat>, nullptr, "")
    ...
    TEST_F(PythonReflectionComponentTests, PythonReflectionTypes)
    {
        ...
        try
        {
            ...
            pybind11::exec(R"(
                azlmbr.test.types.TypeTests_accept_vectorFloat(19.897)
                azlmbr.test.types.TypeTests_return_vectorFloat()
            )");
            ++testCount;
            EXPECT_EQ(testCount, PythonReflectionTypesTester::s_accepted);
            EXPECT_EQ(testCount, PythonReflectionTypesTester::s_returned);
    ...
    TEST_F(PythonReflectionComponentTests, PythonReflectionTypeVectorFloat)
    {
        PythonReflectionTypesTester pythonReflectionTypesTester;
        pythonReflectionTypesTester.Reflect(m_app.GetBehaviorContext());
        ...
        try
        {
            int testCount = 0;
            pybind11::exec(R"(import azlmbr.test.types)");
            pybind11::exec(R"(import azlmbr.math)");

            pybind11::exec(R"(
                azlmbr.test.types.TypeTests_accept_vectorFloat(19.897)
                azlmbr.test.types.TypeTests_return_vectorFloat()
                azlmbr.test.types.TypeTests_signal_success()
            )");
            ++testCount;
            EXPECT_EQ(testCount, PythonReflectionTypesTester::s_accepted);
            EXPECT_EQ(testCount, PythonReflectionTypesTester::s_returned);
            EXPECT_EQ(testCount, PythonReflectionTypesTester::s_successCount);

            pybind11::exec(R"(
                testValue = 1.234567
                azlmbr.test.types.TypeTests_accept_vectorFloat(testValue)
                result = azlmbr.test.types.TypeTests_return_vectorFloat()
                result = float(result)
                if (azlmbr.math.Math_IsClose(testValue, result)):
                    azlmbr.test.types.TypeTests_signal_success()
            )");
            ++testCount;
            EXPECT_EQ(testCount, PythonReflectionTypesTester::s_accepted);
            ...
            // test returning a Vector4
            pybind11::exec(R"(
                testVec4 = azlmbr.math.Vector4(0.1, 0.2, 0.3, 0.4)
                azlmbr.test.types.TypeTests_accept_vector4(testVec4)
                result = azlmbr.test.types.TypeTests_return_vector4()
                resultX = float(result.x)
                testX = float(testVec4.x)
                if (azlmbr.math.Math_IsClose(resultX, testX)):
                    azlmbr.test.types.TypeTests_signal_success()
            )");
            ++testCount;
            EXPECT_EQ(testCount, PythonReflectionTypesTester::s_accepted);
            ...
            // test casting VectorFloat members
            pybind11::exec(R"(
                testVec4 = azlmbr.math.Vector4(1.1, 2.2, 3.3, 4.4)
                testX = int(float(testVec4.x))
                testY = int(float(testVec4.y))
                if (testX == 1 and testY == 2):
                    azlmbr.test.types.TypeTests_signal_success()
            )");
            ++testCount;
            EXPECT_EQ(testCount, PythonReflectionTypesTester::s_successCount);
