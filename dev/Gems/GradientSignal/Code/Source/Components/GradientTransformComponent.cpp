...
namespace GradientSignal
{
    ...
    {
        bounds = m_shapeBounds;
        bounds.ApplyTransform(m_shapeTransformInverse.GetInverseFull());
    }
    ...
    void GradientTransformComponent::OnTick(float deltaTime, AZ::ScriptTimePoint time)
    {
        if (m_dirty)
        {
    ...
    void GradientTransformComponent::UpdateFromShape()
    {
        ...
        switch (m_configuration.m_transformType)
        {
            ...
            case TransformType::Relative:
            {
                ...
                shapeTransform = referenceWorldTransform.GetInverseFull() * entityWorldTransform;
                break;
            }
        }
        ...
        {
            m_configuration.m_rotate = AZ::ConvertTransformToEulerDegrees(shapeTransform);
        }

        if (!m_configuration.m_advancedMode || !m_configuration.m_overrideScale)
        {
            m_configuration.m_scale = shapeTransform.RetrieveScale();
        }
        ...
        const AZ::Transform& shapeTransformFinal =
            AZ::Transform::CreateTranslation(m_configuration.m_translate) *
            AZ::ConvertEulerDegreesToTransform(m_configuration.m_rotate) *
            AZ::Transform::CreateScale(m_configuration.m_scale);
