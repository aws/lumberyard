...
namespace GradientSignal
{
    ...
    void ImageGradientComponent::OnAssetMoved(AZ::Data::Asset<AZ::Data::AssetData> asset, void* oldDataPointer)
    {
        AZStd::lock_guard<decltype(m_imageMutex)> imageLock(m_imageMutex);
        m_configuration.m_imageAsset = asset;
    }
    ...
    void ImageGradientComponent::SetImageAssetPath(const AZStd::string& assetPath)
    {
        ...
        if (assetId.IsValid())
        {
            ...
            {
                AZStd::lock_guard<decltype(m_imageMutex)> imageLock(m_imageMutex);
                m_configuration.m_imageAsset = AZ::Data::AssetManager::Instance().GetAsset(assetId, azrtti_typeid<ImageAsset>(), false);
            }
