...
namespace GradientSignal
{
    ...
    void SurfaceAltitudeGradientComponent::GetRequiredServices(AZ::ComponentDescriptor::DependencyArrayType& services)
    {
    }
    ...
    void SurfaceAltitudeGradientComponent::OnTick(float deltaTime, AZ::ScriptTimePoint time)
    {
        if (m_dirty)
        {
    ...
    void SurfaceAltitudeGradientComponent::UpdateFromShape()
    {
        ...
        if (m_configuration.m_shapeEntityId.IsValid())
        {
            AZ::Aabb bounds = AZ::Aabb::CreateFromMinMax(
                AZ::Vector3(-AZ_FLT_MAX, -AZ_FLT_MAX, AZ::GetMin(m_configuration.m_altitudeMin, m_configuration.m_altitudeMax)),
                AZ::Vector3(AZ_FLT_MAX, AZ_FLT_MAX, AZ::GetMax(m_configuration.m_altitudeMin, m_configuration.m_altitudeMax)));
