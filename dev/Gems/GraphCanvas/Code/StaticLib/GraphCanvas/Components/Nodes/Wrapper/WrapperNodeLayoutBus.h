...
namespace GraphCanvas
{
	...
	//! WrapperNodeActionBus
	//! Actions that occur from a WrapperNode that require some interaction with the underlying data model.
    class WrapperNodeActionRequests : public AZ::EBusTraits
    {
    public:
        //! The id here is the SceneId that the node belongs to.
        static const AZ::EBusAddressPolicy AddressPolicy = AZ::EBusAddressPolicy::ById;
        using BusIdType = AZ::EntityId;

        virtual bool ShouldAcceptDrop(const AZ::EntityId& wrapperNode, const QMimeData* mimeData) const = 0;

        virtual void AddWrapperDropTarget(const AZ::EntityId& wrapperNode) = 0;
        virtual void RemoveWrapperDropTarget(const AZ::EntityId& wrapperNode) = 0;
    };

    using WrapperNodeActionRequestBus = AZ::EBus<WrapperNodeActionRequests>;
}