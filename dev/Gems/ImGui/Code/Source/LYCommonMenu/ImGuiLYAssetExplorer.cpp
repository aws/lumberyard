...
namespace ImGui
{
    ...
    void ImGuiLYAssetExplorer::OnTick(float deltaTime, AZ::ScriptTimePoint time)
    {
        ...
        if (gEnv && gEnv->pRenderer && gEnv->pGame && gEnv->pGame->GetIGameFramework() && 
            gEnv->pGame->GetIGameFramework()->GetIViewSystem() && gEnv->pGame->GetIGameFramework()->GetIViewSystem()->GetActiveView() &&
            gEnv->pGame->GetIGameFramework()->GetIViewSystem()->GetActiveView()->GetCurrentParams())
        {
            // Get the Camera View Position from the view system.
            const Vec3& cameraPosVec3 = gEnv->pGame->GetIGameFramework()->GetIViewSystem()->GetActiveView()->GetCurrentParams()->position;
