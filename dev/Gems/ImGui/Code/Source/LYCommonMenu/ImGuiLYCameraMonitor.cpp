...
namespace ImGui
{
    ...
    void ImGuiLYCameraMonitor::OnTick(float deltaTime, AZ::ScriptTimePoint time)
    {
    ...
    void ImGuiLYCameraMonitor::OnTick_GatherCameraData(float deltaTime)
    {
        ...
        const AZ::Vector3& camFacing = camTransform.GetColumn(1);
        const float positionFrameDelta = (camTransform.GetPosition() - currentCam.m_lastWorldPos).GetLength();
        currentCam.m_lastWorldPos = camTransform.GetPosition();
