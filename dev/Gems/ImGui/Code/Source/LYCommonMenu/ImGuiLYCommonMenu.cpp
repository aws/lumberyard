...
namespace ImGui
{
    ...
    static auto SystemCursorStateComboGetter = [](void* contextPtr, int idx, const char** out_text)
    {
    ...
    void ImGuiLYCommonMenu::OnImGuiUpdate()
    {
        if (ImGui::BeginMainMenuBar())
        {
            ...
            if (ImGui::BeginMenu("LumberYard"))
            {
                ...
                auto gameFramework = (gEnv && gEnv->pGame) ? gEnv->pGame->GetIGameFramework() : nullptr;
                auto lvlSystem = (gameFramework) ? gameFramework->GetILevelSystem() : nullptr;
    ...
    void ImGuiLYCommonMenu::OnTick(float deltaTime, AZ::ScriptTimePoint time)
    {
