...
namespace ImageProcessing
{
    namespace TIFFLoader
    {
        ...
        static IImageObject* Load8BitImageFromTIFF(TIFF* tif)
        {
            ...
            std::vector<uint8> buf(dwPitch);

            for (uint32 dwY = 0; dwY < dwHeight; ++dwY)
            {
                TIFFReadScanline(tif, &buf[0], dwY, 0); // read raw row

                const uint8* srcLine = &buf[0];
                uint8* dstLine = &dst[dwPitch * dwY];
                ...
                else if (dwChannels == 2)
                {
                    if (dwPhotometric == PHOTOMETRIC_SEPARATED)
                    {
                        for (uint32 dwX = 0; dwX < dwWidth; ++dwX)
                        {
                            ...
                            dstLine[0] = uint16(1.0f - srcLine[0]);
                            dstLine[1] = uint16(1.0f - srcLine[1]);
                            dstLine[2] = zero;
                            dstLine[3] = one;
