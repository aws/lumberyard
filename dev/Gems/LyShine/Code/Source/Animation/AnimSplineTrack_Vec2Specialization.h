    ...
    auto versionConverter = [](AZ::SerializeContext& context, AZ::SerializeContext::DataElementNode& classElement) -> bool
    {
        ...
        if (classElement.GetVersion() == 1)
        {
            ...
            if (splineElementIdx)
            {
                ...
                auto& splinePtrNodeRef = classElement.GetSubElement(splineElementIdx);
                auto  splinePtrNodeCopy = splinePtrNodeRef;

                // Reset the node, then convert it to an intrusive pointer
                splinePtrNodeRef = AZ::SerializeContext::DataElementNode();
                splinePtrNodeRef.Convert<AZStd::intrusive_ptr<UiSpline::TrackSplineInterpolator<Vec2>>>(context, "Spline");
