...
static void FillTextureRectWithCheckerboard(uint32* data, int textureWidth, int textureHeight,
    int minX, int minY, int rectWidth, int rectHeight,
...
////////////////////////////////////////////////////////////////////////////////////////////////////
static AZ::Entity* CreateCheckbox(const char* name, bool atRoot, AZ::EntityId parent,
    UiTransform2dInterface::Anchors anchors, UiTransform2dInterface::Offsets offsets,
    const char* text, AZ::Color baseColor, AZ::Color selectedColor, AZ::Color pressedColor, AZ::Color textColor)
{
    AZ::Entity* checkboxElem = nullptr;
    {
        if (atRoot)
        {
            EBUS_EVENT_ID_RESULT(checkboxElem, parent, UiCanvasBus, CreateChildElement, name);
        }
        else
        {
            EBUS_EVENT_ID_RESULT(checkboxElem, parent, UiElementBus, CreateChildElement, name);
        }

        CreateComponent(checkboxElem, LyShine::UiTransform2dComponentUuid);
        CreateComponent(checkboxElem, LyShine::UiImageComponentUuid);
        CreateComponent(checkboxElem, LyShine::UiCheckboxComponentUuid);

        AZ_Assert(UiTransform2dBus::FindFirstHandler(checkboxElem->GetId()), "Transform2d component missing");

        EBUS_EVENT_ID(checkboxElem->GetId(), UiTransform2dBus, SetAnchors, anchors, false, false);
        EBUS_EVENT_ID(checkboxElem->GetId(), UiTransform2dBus, SetOffsets, offsets);
        EBUS_EVENT_ID(checkboxElem->GetId(), UiImageBus, SetColor, baseColor);

        EBUS_EVENT_ID(checkboxElem->GetId(), UiInteractableStatesBus, SetStateColor, UiInteractableStatesInterface::StateHover, checkboxElem->GetId(), selectedColor);
        EBUS_EVENT_ID(checkboxElem->GetId(), UiInteractableStatesBus, SetStateAlpha, UiInteractableStatesInterface::StateHover, checkboxElem->GetId(), selectedColor.GetA());

        ISprite* backgroundSprite = gEnv->pLyShine->LoadSprite("textures/defaults/unchecked_nohover.tif");
        EBUS_EVENT_ID(checkboxElem->GetId(), UiImageBus, SetSprite, backgroundSprite);
    }

    AZ::Entity* onElem;
    {
        EBUS_EVENT_ID_RESULT(onElem, checkboxElem->GetId(), UiElementBus, CreateChildElement, "onElem");

        CreateComponent(onElem, LyShine::UiTransform2dComponentUuid);
        CreateComponent(onElem, LyShine::UiImageComponentUuid);

        EBUS_EVENT_ID(onElem->GetId(), UiTransform2dBus, SetAnchors, anchors, false, false);
        EBUS_EVENT_ID(onElem->GetId(), UiTransform2dBus, SetOffsets, offsets);

        ISprite* sprite0 = gEnv->pLyShine->LoadSprite("textures/defaults/checkmark.tif");
        EBUS_EVENT_ID(onElem->GetId(), UiImageBus, SetSprite, sprite0);
    }

    // Link the on and off child entities to the parent checkbox entity.
    EBUS_EVENT_ID(checkboxElem->GetId(), UiCheckboxBus, SetCheckedEntity, onElem->GetId());

    // Trigger all InGamePostActivate
    EBUS_EVENT_ID(onElem->GetId(), UiInitializationBus, InGamePostActivate);
    EBUS_EVENT_ID(checkboxElem->GetId(), UiInitializationBus, InGamePostActivate);

    return checkboxElem;
}

////////////////////////////////////////////////////////////////////////////////////////////////////
static AZ::Entity* CreateSlider(AZ::EntityId parent)
{
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    AZ::Entity* sliderElem = nullptr;
    {
        EBUS_EVENT_ID_RESULT(sliderElem, parent, UiCanvasBus, CreateChildElement, "sliderElem");

        CreateComponent(sliderElem, LyShine::UiTransform2dComponentUuid);
        CreateComponent(sliderElem, LyShine::UiSliderComponentUuid);

        // 500 x 50, + 100 units padding all around
        UiTransform2dInterface::Offsets offsets(-300.0f /*left*/, -75.0f /*top*/, 300.0f /*right*/, 75.0f /*bottom*/);
        EBUS_EVENT_ID(sliderElem->GetId(), UiTransform2dBus, SetOffsets, offsets);

        //ok
        //EBUS_EVENT_ID(sliderElem->GetId(), UiSliderBus, SetMinValue, -10.0f);
        //EBUS_EVENT_ID(sliderElem->GetId(), UiSliderBus, SetMaxValue, -5.0f);

        //ok
        EBUS_EVENT_ID(sliderElem->GetId(), UiSliderBus, SetMinValue, 5.0f);
        EBUS_EVENT_ID(sliderElem->GetId(), UiSliderBus, SetMaxValue, 10.0f);
        ...
        //EBUS_EVENT_ID(sliderElem->GetId(), UiSliderBus, SetStepValue, 0.125f);
        EBUS_EVENT_ID(sliderElem->GetId(), UiSliderBus, SetStepValue, 1.0f);

        //ok
        //EBUS_EVENT_ID(sliderElem->GetId(), UiTransformBus, SetZRotation, -45.0f);
        //EBUS_EVENT_ID(sliderElem->GetId(), UiTransformBus, SetZRotation, -135.0f);
        //EBUS_EVENT_ID(sliderElem->GetId(), UiTransformBus, SetZRotation, 180.0f);
        //EBUS_EVENT_ID(sliderElem->GetId(), UiTransformBus, SetZRotation, -90.0f);
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    AZ::Entity* sliderTrackElem = nullptr;
    {
        EBUS_EVENT_ID_RESULT(sliderTrackElem, sliderElem->GetId(), UiElementBus, CreateChildElement, "sliderTrackElem");

        CreateComponent(sliderTrackElem, LyShine::UiTransform2dComponentUuid);
        CreateComponent(sliderTrackElem, LyShine::UiImageComponentUuid);

        //dark green
        EBUS_EVENT_ID(sliderTrackElem->GetId(), UiImageBus, SetColor, AZ::Color(0.0f, 1.0f / 3.0f, 0.0f, 1.0f));

        // 500 x 50
        UiTransform2dInterface::Offsets offsets(-250.0f /*left*/, -25.0f /*top*/, 250.0f /*right*/, 25.0f /*bottom*/);
        EBUS_EVENT_ID(sliderTrackElem->GetId(), UiTransform2dBus, SetOffsets, offsets);
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    AZ::Entity* sliderFillElem = nullptr;
    {
        EBUS_EVENT_ID_RESULT(sliderFillElem, sliderTrackElem->GetId(), UiElementBus, CreateChildElement, "sliderFillElem");

        CreateComponent(sliderFillElem, LyShine::UiTransform2dComponentUuid);
        CreateComponent(sliderFillElem, LyShine::UiImageComponentUuid);

        //light green
        EBUS_EVENT_ID(sliderFillElem->GetId(), UiImageBus, SetColor, AZ::Color(0.0f, 2.0f / 3.0f, 0.0f, 1.0f));

        // 500 x 50
        UiTransform2dInterface::Offsets offsets(0.0f /*left*/, -25.0f /*top*/, 0.0f /*right*/, 25.0f /*bottom*/);
        EBUS_EVENT_ID(sliderFillElem->GetId(), UiTransform2dBus, SetOffsets, offsets);
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    AZ::Entity* sliderManipulatorElem = nullptr;
    {
        EBUS_EVENT_ID_RESULT(sliderManipulatorElem, sliderTrackElem->GetId(), UiElementBus, CreateChildElement, "sliderManipulatorElem");

        CreateComponent(sliderManipulatorElem, LyShine::UiTransform2dComponentUuid);
        CreateComponent(sliderManipulatorElem, LyShine::UiImageComponentUuid);

        ISprite* sprite0 = gEnv->pLyShine->LoadSprite("textures/defaults/sliderManipulator.tif");
        EBUS_EVENT_ID(sliderManipulatorElem->GetId(), UiImageBus, SetSprite, sprite0);

        // 100 x 100
        UiTransform2dInterface::Offsets offsets(-50.0f /*left*/, -50.0f /*top*/, 50.0f /*right*/, 50.0f /*bottom*/);
        EBUS_EVENT_ID(sliderManipulatorElem->GetId(), UiTransform2dBus, SetOffsets, offsets);
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////////

    // Link the child entities to the parent slider entity.
    EBUS_EVENT_ID(sliderElem->GetId(), UiSliderBus, SetTrackEntity, sliderTrackElem->GetId());
    EBUS_EVENT_ID(sliderElem->GetId(), UiSliderBus, SetFillEntity, sliderFillElem->GetId());
    EBUS_EVENT_ID(sliderElem->GetId(), UiSliderBus, SetManipulatorEntity, sliderManipulatorElem->GetId());

    // Trigger all InGamePostActivate
    EBUS_EVENT_ID(sliderTrackElem->GetId(), UiInitializationBus, InGamePostActivate);
    EBUS_EVENT_ID(sliderFillElem->GetId(), UiInitializationBus, InGamePostActivate);
    EBUS_EVENT_ID(sliderManipulatorElem->GetId(), UiInitializationBus, InGamePostActivate);
    EBUS_EVENT_ID(sliderElem->GetId(), UiInitializationBus, InGamePostActivate);

    EBUS_EVENT_ID(sliderElem->GetId(), UiSliderBus, SetValue, 0.75f);
...
namespace
{
    ...
    public:
        ...
        void OnAction(AZ::EntityId canvasId, const LyShine::ActionName& actionName) override
        {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
// Gets the test canvas pointer from the ID. We do this in case the canvas has been deleted
// (by exiting the game for example)
static UiCanvasInterface* GetTestCanvas()
{
    UiCanvasInterface* canvasInterface = nullptr;
    if (g_testCanvasId.IsValid())
    {
        canvasInterface = UiCanvasBus::FindFirstHandler(g_testCanvasId);
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
static void TestCanvasCreate (IConsoleCmdArgs* Cmd)
{
    if (gEnv->IsEditing())
    {
        return;
    ...
    auto setEnabledCallbackFn = [testImageElem](AZ::EntityId clickedEntityId, AZ::Vector2 point)
        {
    ...
    auto setChangedTextFn = [changedTextElem](AZ::EntityId textInputEntityId, LyShine::StringType textString)
        {
            EBUS_EVENT_ID(changedTextElem->GetId(), UiTextBus, SetText, textString);
        };
    auto setEditedTextFn = [editedTextElem](AZ::EntityId textInputEntityId, LyShine::StringType textString)
        {
            EBUS_EVENT_ID(editedTextElem->GetId(), UiTextBus, SetText, textString);
        };
    auto setEnteredTextFn = [enteredTextElem](AZ::EntityId textInputEntityId, LyShine::StringType textString)
        {
...
static void TestCanvasRemove(IConsoleCmdArgs* Cmd)
{
    if (gEnv->IsEditing())
    {
        return;
