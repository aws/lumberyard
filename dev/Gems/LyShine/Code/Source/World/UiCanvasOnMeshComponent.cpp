...
bool UiCanvasOnMeshComponent::ProcessCollisionInputEventInternal(const AzFramework::InputChannel::Snapshot& inputSnapshot, int triangleIndex, Vec3 hitPoint, IPhysicalEntity* collider, int partIndex)
{
    ...
    if (canvasEntityId.IsValid())
    {
        ...
        EBUS_EVENT_ID_RESULT(renderNode, GetEntityId(), LmbrCentral::RenderNodeRequestBus, GetRenderNode);

        if (renderNode)
        {
            Matrix34A transform;
            IStatObj* statObj = renderNode->GetEntityStatObj(0, 0, &transform, false);

            if (statObj)
            {
                // the true param to GetIndexedMesh tells it to create one if there is none
                IIndexedMesh* indexedMesh = statObj->GetIndexedMesh(true);
                if (indexedMesh)
                {
                    Vec2 texCoord;
