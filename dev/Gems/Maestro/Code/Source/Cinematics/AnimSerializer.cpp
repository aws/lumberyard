...
void AnimSerializer::ReflectAnimTypes(AZ::SerializeContext* context)
{
    ...
    context->Class<IMannequinKey, IKey>()
        ->Field("FragmentName", &IMannequinKey::m_fragmentName)
        ->Field("Tags", &IMannequinKey::m_tags)
        ->Field("Duration", &IMannequinKey::m_duration)
        ->Field("Priority", &IMannequinKey::m_priority);
