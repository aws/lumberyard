...
void DataCache::AddToCache(const std::string& tableName, const std::string& key, const Vec3 &value)
{
    DocumentPtr doc(FindDoc(tableName));
...
void DataCache::AddToArray(const std::string & tableName, const std::string & arrayName, const Vec3 & value)
{
    DocumentPtr doc(FindDoc(tableName));
...
void DataCache::AddToObject(const std::string & tableName, const std::string & objName, const std::string & key, const Vec3 & value)
{
    DocumentPtr doc(FindDoc(tableName));
...
{
    rapidjson::Value v;
    v.SetString(value.c_str(), value.size(), m_allocator);
    return v;
}
...
rapidjson::Value DataCache::Document::ToJson(const Vec3 & value)
{
    rapidjson::Value v(rapidjson::kArrayType);

    v.PushBack( ToJson(value.x), m_allocator);
    v.PushBack( ToJson(value.y), m_allocator);
    v.PushBack( ToJson(value.z), m_allocator);
...
DataCache::Document::rapidJsonValuePtr DataCache::Document::FindValue(const std::string & name, ValueType type)
{
    ...
    {
        valuePtr = std::make_shared<rapidjson::Value>((type == ValueType::Array) ? rapidjson::kArrayType : rapidjson::kObjectType);
