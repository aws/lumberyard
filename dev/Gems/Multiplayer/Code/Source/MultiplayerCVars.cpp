...
namespace Multiplayer
{
    ...
    static void OnLocalLagChanged(ICVar* cvar)
    {
        GridMate::GridSession* session = nullptr;
        EBUS_EVENT_RESULT(session, Multiplayer::MultiplayerRequestBus, GetSession);
        ...
        session->GetReplicaMgr()->SetLocalLagAmt(cvar->GetIVal());
    }
    ...
    void MultiplayerCVars::RegisterCVars()
    {
        if (gEnv && !gEnv->IsEditor())
        {
            ...
            REGISTER_INT_CB("gm_localLagAmount", 0, VF_NULL, "Local Lag - See ReplicaMgr.cpp", OnLocalLagChanged);
