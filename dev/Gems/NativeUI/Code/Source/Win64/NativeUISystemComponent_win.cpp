...
namespace
{
    ...
    MessageDimensions ComputeMessageDimensions(const AZStd::string& message)
    {
        ...
        do 
        {
            ...
            searchPos = message.find('\n', searchPos);

            if (searchPos == AZStd::string::npos)
            {
                lineLength = message.size() - prevNewLine - 1;
            }
            else
            {
...
namespace NativeUI
{
    AZStd::string SystemComponent::DisplayBlockingDialog(const AZStd::string& title, const AZStd::string& message, const AZStd::vector<AZStd::string>& options) const
    {
        ...
        {
            AZ_Assert(false, "Cannot create dialog box with more than 10 buttons");
            return "";
        }
        ...
        int buttonRowsHeight = (buttonRows.size() * BUTTON_HEIGHT) + ((buttonRows.size() - 1) * BUTTON_TO_BUTTON_Y_DELTA);
        ...
        dlgTemplate.m_dlgTemplate.cdit = options.size() + 1;
        dlgTemplate.m_dlgTemplate.x = dlgTemplate.m_dlgTemplate.y = 0;
        dlgTemplate.m_dlgTemplate.cx = dialogWidth;
        dlgTemplate.m_dlgTemplate.cy = dialogHeight;
        ...
        dlgTemplate.m_itemTemplates[0].m_itemTemplate.x = DIALOG_LEFT_MARGIN + (dialogUsableWidth - messageDimensions.m_width)/2;
        dlgTemplate.m_itemTemplates[0].m_itemTemplate.y = DIALOG_TOP_MARGIN;
        dlgTemplate.m_itemTemplates[0].m_itemTemplate.cx = messageDimensions.m_width;
        dlgTemplate.m_itemTemplates[0].m_itemTemplate.cy = messageDimensions.m_height;
        ...
        for (int i = 0, buttonRow = 0, buttonRowIndex = 0; i < options.size(); i++, buttonRowIndex++)
        {
            ...
            dlgTemplate.m_itemTemplates[i + 1].m_itemTemplate.x = DIALOG_LEFT_MARGIN + (dialogUsableWidth - buttonRows[buttonRow].m_width)/2 + buttonXOffset ;
            dlgTemplate.m_itemTemplates[i + 1].m_itemTemplate.y = buttonYOffset;
            dlgTemplate.m_itemTemplates[i + 1].m_itemTemplate.cx = buttonDimensions[i].m_width;
            dlgTemplate.m_itemTemplates[i + 1].m_itemTemplate.cy = buttonDimensions[i].m_height;
            dlgTemplate.m_itemTemplates[i + 1].m_itemTemplate.id = i + 1;
