...
namespace NvCloth
{
    namespace Editor
    {
        ...
        void MeshNodeHandler::ConsumeAttribute(widget_t* GUI, AZ::u32 attrib, AzToolsFramework::PropertyAttributeReader* attrValue, const char* debugName)
        {
        ...
        void MeshNodeHandler::WriteGUIValuesIntoProperty(size_t index, widget_t* GUI, property_t& instance, AzToolsFramework::InstanceDataNode* node)
        {
            instance = GUI->GetComboBox()->currentText().toUtf8().data();
        }

        bool MeshNodeHandler::ReadValuesIntoGUI(size_t index, widget_t* GUI, const property_t& instance, AzToolsFramework::InstanceDataNode* node)
        {
            QSignalBlocker signalBlocker(GUI->GetComboBox());
...
#include <Source/Editor/MeshNodeHandler.moc>
