...
namespace PhysX
{
        ...
        , m_min(min)
    {
        ...
        const AZ::Quaternion localRotation = AZ::Quaternion::CreateFromTransform(localTransform);
        ...
        AZ::Vector3 displacementTranslate = localRotation * AZ::Vector3(ConeHeight, 0.0f, 0.0f);
        displacementTransform.SetPosition(localTransform.GetPosition() + displacementTranslate);
            ...
            [this, sharedState](const AzToolsFramework::LinearManipulator::Action& action)
        {
            ...
            EditorJointRequestBus::EventResult(
                ...
                , PhysX::EditorJointComponentMode::s_parameterTransform);
            const AZ::Quaternion localRotation = AZ::Quaternion::CreateFromTransform(localTransform);
            const AZ::VectorFloat axisDisplacement = action.LocalPositionOffset().Dot(localRotation * action.m_fixed.m_axis);
            ...
            [this, sharedState](const AzToolsFramework::PlanarManipulator::Action& action)
        {
            ...
            const AZ::VectorFloat axisDisplacementY = action.LocalPositionOffset().Dot(localRotation * AZ::Vector3::CreateAxisY());
            const AZ::VectorFloat axisDisplacement = axisDisplacementY.GetMax(action.LocalPositionOffset().Dot(localRotation * AZ::Vector3::CreateAxisZ()));
        ...
        auto mouseDownRotateXCallback = [this, sharedStateXRotate](const AzToolsFramework::AngularManipulator::Action& action) mutable -> void
        {
        ...
        (const AzToolsFramework::AngularManipulator::Action& action) mutable -> void
        {
            ...
            PhysX::EditorJointRequestBus::Event(m_entityComponentId
                ...
                , AZ::Quaternion::CreateFromTransform(newTransform).GetEulerDegrees());
            ...
            m_yLinearManipulator->SetAxis(action.m_current.m_delta * AZ::Vector3::CreateAxisY());
            m_zLinearManipulator->SetAxis(action.m_current.m_delta * AZ::Vector3::CreateAxisZ());
    ...
    void EditorSubComponentModeAngleCone::Refresh()
    {
        ...
        const AZ::Quaternion localRotation = AZ::Quaternion::CreateFromTransform(localTransform);
        const AZ::Vector3 linearManipulatorOffset = localTransform.GetPosition() + localRotation * AZ::Vector3(coneHeight, 0.0f, 0.0f);

        m_xRotationManipulator->SetLocalTransform(localTransform);
        m_xRotationManipulator->SetBoundsDirty();

        localTransform.SetPosition(linearManipulatorOffset);
