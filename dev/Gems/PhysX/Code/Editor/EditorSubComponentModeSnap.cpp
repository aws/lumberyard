...
namespace PhysX
{
        ...
        const AzToolsFramework::ViewportInteraction::MouseInteractionEvent& mouseInteraction)
    {
        if (mouseInteraction.m_mouseEvent == AzToolsFramework::ViewportInteraction::MouseEvent::Move)
        {
            ...
            {
                AZ::Transform worldTransform = PhysX::Utils::GetEntityWorldTransformWithoutScale(m_entityComponentId.GetEntityId());
                const AZ::Quaternion worldRotate = AZ::Quaternion::CreateFromTransform(worldTransform);
                const AZ::Quaternion worldRotateInv = worldRotate.GetInverseFull();

                m_manipulator->SetLocalPosition(worldRotateInv * (m_pickedPosition - worldTransform.GetPosition()));
    ...
    AZ::Vector3 EditorSubComponentModeSnap::GetPosition() const
    {
        ...
        AZ::Quaternion worldRotate = AZ::Quaternion::CreateFromTransform(worldTransform);
        ...
        return worldTransform.GetPosition() + worldRotate * localTransform.GetPosition();
    }
} // namespace PhysX
