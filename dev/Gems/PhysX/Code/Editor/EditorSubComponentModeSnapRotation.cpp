...
namespace PhysX
{
    ...
    void EditorSubComponentModeSnapRotation::InitMouseDownCallBack()
    {
            ...
            [this](const AzToolsFramework::LinearManipulator::Action& action) mutable
        {
            ...
            const AZ::Transform worldTransformInv = worldTransform.GetInverseFull();
            const AZ::Vector3 pickedLocalPosition = worldTransformInv * pickedEntityTransform.GetPosition() - localTransform.GetPosition();

            if (pickedLocalPosition.GetLength().IsZero())
            {
                AZ_Warning("EditorsubComponentModeSnapRotation",
                    false,
