...
namespace PhysX
{
        ...
        AZStd::vector<bool>& lineValidityBufferOut)
    {
        ...
        AZ::Vector3 childAxis = parentWorldRotation.GetConjugate() * childWorldRotation * childLocalRotation *
    ...
    AZStd::shared_ptr<Physics::JointLimitConfiguration> JointUtils::CreateJointLimitConfiguration(AZ::TypeId jointType)
    {
        ...
        const AZ::Quaternion& childLocalRotation)
    {
        ...
        AZ::Quaternion twistQuat = relativeRotation.GetX().IsZero()
    ...
    bool JointUtils::IsD6SwingValid(
        ...
        float swingLimitZ)
    {
        const float epsilon = static_cast<float>(AZ::g_fltEps);
        ...
        AZStd::vector<bool>& lineValidityBufferOut)
    {
        ...
        for (AZ::u32 angularIndex = 0; angularIndex <= angularSubdivisions; angularIndex++)
        {
            ...
            const AZ::Vector3 radialVector = parentLocalRotation * radialVectorRotation * AZ::Vector3::CreateAxisX(scale);
        ...
        AZStd::vector<bool>& lineValidityBufferOut)
    {
        ...
        for (AZ::u32 angularIndex = 0; angularIndex <= angularSubdivisions; angularIndex++)
        {
            ...
            const AZ::Vector3 radialVector = parentLocalRotation * (scale * AZ::Vector3(0.0f, cosf(angle), sinf(angle)));
    ...
    {
        const AZ::Vector3 twistVector = parentLocalRotation * (1.25f * scale * AZ::Vector3(0.0f, cosf(twistAngle), sinf(twistAngle)));
        lineBufferOut.push_back(AZ::Vector3::CreateZero());
        ...
        const AZStd::vector<AZ::Quaternion>& exampleLocalRotations)
    {
        ...
        if (jointLimitTypeId == D6JointLimitConfiguration::RTTI_Type())
        {
            ...
            const AZ::Quaternion childLocalRotation = AZ::Quaternion::CreateShortestArc(AZ::Vector3::CreateAxisX(),
                childWorldRotation.GetConjugate() * normalizedAxis);
