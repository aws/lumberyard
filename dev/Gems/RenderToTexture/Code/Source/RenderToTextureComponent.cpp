...
namespace RenderToTexture
{
    ...
    void RenderToTextureComponent::Activate()
    {
        ...
        else if (gEnv->pRenderer)
        {
            ...
            else if (m_config.m_enabled)
            {
                ...
                else if (gEnv->pGame && gEnv->pGame->GetIGameFramework())
                {
                    gEnv->pGame->GetIGameFramework()->RegisterListener(this, "RenderToTextureComponent", FRAMEWORKLISTENERPRIORITY_HUD);
                }
    ...
    {
        // we only connect to the tickbus for simulation mode in the editor
        // when OnPreUpdate() is not called
    ...
    void RenderToTextureComponent::OnPostUpdate(float fDeltaTime)
    {
        if (m_config.m_displayDebugImage && gEnv->IsEditor() && !gEnv->pConsole->IsOpened())
        {
    ...
    void RenderToTextureComponent::OnPreUpdate()
    {
