...
namespace ScriptCanvasEditor
{
                                    ...
                                    if (variable == nullptr)
                                    {
                                        ...
                                        if (variableIter != variableMapping->end())
                                        {
                                            ...
                                            if (remapVariableOutcome)
                                            {
                                                ...
                                                ScriptCanvas::GraphVariableManagerRequestBus::EventResult(variable, GetScriptCanvasId(), &ScriptCanvas::GraphVariableManagerRequests::FindVariableById, remapVariableOutcome.GetValue());

                                                if (variable)
                                                {
                                                    if (IsRuntimeGraph())
                                                    {
                                                        variable->RemoveScope(ScriptCanvas::VariableFlags::Output);
                                                    }
                                                }
                                            }
                                            else
    ...
    bool Graph::CanExposeEndpoint(const GraphCanvas::Endpoint& endpoint)
    {
        ...
        NodeDescriptorRequestBus::EventResult(isNodeling, endpoint.GetNodeId(), &NodeDescriptorRequests::IsType, NodeDescriptorType::ExecutionNodeling);        
