    ...
    public:
        ...
        bool filterAcceptsRow(int sourceRow, const QModelIndex& sourceParent) const
        {
            ...
            if (variable)
            {
                ...
                // Better helper method exists in a secondary revision that is tied up with non-releasable code. Duplicate the check logic for now.
                auto dynamicGroup = m_slotFilter->GetDynamicGroup();

                if (dynamicGroup != AZ::Crc32())
                {
                    return m_slotFilter->GetNode()->IsValidTypeForGroup(dynamicGroup, dataType).IsSuccess();
                }
                else
                {
