...
namespace ScriptCanvasEditor
{
    ...
    Qt::ItemFlags ScriptCanvasAssetNodeUsageTreeItem::Flags(const QModelIndex& index) const
    {
    ...
    {
        auto asset = AZ::Data::AssetManager::Instance().GetAsset(assetId, azrtti_typeid<ScriptCanvasAsset>(), true, nullptr, false);
        if (!asset.IsReady())
        {
        ...
        if (treeItem == nullptr)
        {
            ...
            AzToolsFramework::AssetSystemRequestBus::BroadcastResult(foundPath, &AzToolsFramework::AssetSystemRequestBus::Events::GetAssetInfoById, assetId, azrtti_typeid<ScriptCanvasAsset>(), assetInfo, rootFilePath);
            ...
            if (foundPath)
            {
                ...
                if (AzFramework::StringFunc::Path::GetFileName(relativePath.c_str(), fileName))
                {
                    ...
                    if (AzFramework::StringFunc::Path::StripComponent(relativePath, stripLastComponent))
                    {
                        ...
                        GraphCanvas::GraphCanvasTreeItem* treeItem = m_categorizer.GetCategoryNode(relativePath.c_str(), this);

                        if (treeItem)
                        {
                            ScriptCanvasAssetNodeUsageTreeItem* usageTreeItem = treeItem->CreateChildNode<ScriptCanvasAssetNodeUsageTreeItem>(fileName);
    ...
    GraphCanvas::GraphCanvasTreeItem* ScriptCanvasAssetNodeUsageTreeItemRoot::CreateCategoryNode(AZStd::string_view categoryPath, AZStd::string_view categoryName, GraphCanvasTreeItem* parent) const
    {
