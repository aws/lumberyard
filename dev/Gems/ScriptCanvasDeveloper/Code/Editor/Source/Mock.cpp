...
namespace ScriptCanvasDeveloper
{
    namespace Nodes
    {
            ...
            if (auto serializeContext = azrtti_cast<AZ::SerializeContext*>(context))
            {
                ...
                if (AZ::EditContext* editContext = serializeContext->GetEditContext())
                {
                    editContext->Class<SlotConfig>("SlotConfig", "Configuration for slot")
                        ...
                        ->DataElement(ScriptCanvas::Attributes::UIHandlers::GenericComboBox, &SlotConfig::m_type, "Slot Type", "The Data Type represented by this slot")
                        ->Attribute(ScriptCanvas::Attributes::GenericValueList, &SlotConfig::GetCreatableTypes)
                        ->Attribute(ScriptCanvas::Attributes::PostChangeNotify, &SlotConfig::OnSlotDataTypeChanged)
                        ;
                }
            }
        }
        ...
        AZStd::vector<AZStd::pair<Data::Type, AZStd::string>> SlotConfig::GetCreatableTypes()
        {
            ...
            SlotType slotType = slot ? slot->GetType() : SlotType::None;
            if (IsExecution(slotType))
            {
                return AZStd::vector<AZStd::pair<Data::Type, AZStd::string>>{};
            }
        ...
        void SlotConfig::OnSlotDataTypeChanged(const Data::Type& oldDataType)
        {
            if (!m_owner)
            {
                return;
            ...
            Slot* slot = m_owner->GetSlot(m_slotId);
            SlotType slotType = slot ? slot->GetType() : SlotType::None;
            if (IsExecution(slotType) || slotType == SlotType::None)
            {
                return;
            }
            ...
            m_slotId = {};
            if (slotType == SlotType::DataIn)
            {
                m_slotId = m_type.IsValid() ? m_owner->AddInputDatumSlot(m_name, m_toolTip, m_type, Datum::eOriginality::Original, false)
                    : m_owner->AddInputDatumOverloadedSlot(m_name, m_toolTip, AZStd::vector<ContractDescriptor>(), false);
            }
            else if (slotType == SlotType::DataOut)
            {
                m_slotId = m_owner->AddOutputTypeSlot(m_name, m_toolTip, m_type, Node::OutputStorage::Optional, false);
            }
        }
        ...
        void Mock::OnNodeDisplayed(const GraphCanvas::NodeId& nodeId)
        {
        }
        ...
        AZ::Crc32 Mock::OnDataInSlotArrayChanged()
        {
            ...
            for (auto& dataInConfig : m_dataInConfigArray)
            {
                ...
                {
                    dataInConfig.m_slotId = dataInConfig.m_type.IsValid() ? AddInputDatumSlot(dataInConfig.m_name, dataInConfig.m_toolTip, dataInConfig.m_type, Datum::eOriginality::Original, false)
                        : AddInputDatumOverloadedSlot(dataInConfig.m_name, dataInConfig.m_toolTip, AZStd::vector<ContractDescriptor>(), false);
        ...
        AZ::Crc32 Mock::OnDataOutSlotArrayChanged()
        {
            ...
            for (auto& dataOutConfig : m_dataOutConfigArray)
            {
                ...
                {
                    dataOutConfig.m_slotId = AddSlot(dataOutConfig.m_name, dataOutConfig.m_toolTip, SlotType::DataOut, AZStd::vector<ContractDescriptor>(), false);
        ...
        AZ::Crc32 Mock::OnExecutionInSlotArrayChanged()
        {
            ...
            for (auto& executionInConfig : m_executionInConfigArray)
            {
                ...
                {
                    executionInConfig.m_slotId = AddSlot(executionInConfig.m_name, executionInConfig.m_toolTip, SlotType::ExecutionIn, AZStd::vector<ContractDescriptor>(), false);
        ...
        AZ::Crc32 Mock::OnExecutionOutSlotArrayChanged()
        {
            ...
            for (auto& executionOutConfig : m_executionOutConfigArray)
            {
                ...
                {
                    executionOutConfig.m_slotId = AddSlot(executionOutConfig.m_name, executionOutConfig.m_toolTip, SlotType::ExecutionOut, AZStd::vector<ContractDescriptor>(), false);
        ...
        void Mock::OnSlotAdded(const AZ::EntityId& slotId)
        {
        ...
        void Mock::OnSlotRemoved(const AZ::EntityId& slotId)
        {
        ...
        void Mock::OnAddedToScene(const AZ::EntityId& sceneId)
        {
            OnNodeTitleChanged();
            OnNodeSubTitleChanged();
            OnNodeColorPaletteChanged();
        ...
        void Mock::OnNodeDeserialized(const GraphCanvas::GraphId& graphId, const GraphCanvas::GraphSerialization& serialization)
        {
