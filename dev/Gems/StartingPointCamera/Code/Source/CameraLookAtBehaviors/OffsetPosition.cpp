...
namespace Camera
{
    ...
    void OffsetPosition::AdjustLookAtTarget(float deltaTime, const AZ::Transform& targetTransform, AZ::Transform& outLookAtTargetTransform)
    {
        if (m_isRelativeOffset)
        {
            outLookAtTargetTransform.SetPosition(outLookAtTargetTransform.GetPosition() + AZ::Quaternion::CreateFromTransform(outLookAtTargetTransform) * m_positionalOffset);
        }
        else
        {
            outLookAtTargetTransform.SetPosition(outLookAtTargetTransform.GetPosition() + m_positionalOffset);
        }
    }
} // namespace Camera
