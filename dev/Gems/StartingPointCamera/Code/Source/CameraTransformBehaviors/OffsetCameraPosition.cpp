...
namespace Camera
{
    ...
    void OffsetCameraPosition::AdjustCameraTransform(float deltaTime, const AZ::Transform& initialCameraTransform, const AZ::Transform& targetTransform, AZ::Transform& inOutCameraTransform)
    {
        AZ::Vector3 currentPosition = targetTransform.GetPosition();
        inOutCameraTransform.SetPosition(AZ::Vector3::CreateZero());
        AZ::Transform rotation = m_isRelativeOffset ? inOutCameraTransform : AZ::Transform::CreateIdentity();
        inOutCameraTransform.SetPosition(currentPosition + rotation * m_offset);
    }
} //namespace Camera