...
function ordered_event_combination:OnActivate()
	for nameCount = 1, #self.Properties.IncomingGameplayEventNames do
		self.BusIds[nameCount] = GameplayNotificationId(self.entityId, self.Properties.IncomingGameplayEventNames[nameCount])
		self.Handlers[nameCount] = GameplayNotificationBus.Connect(self, self.BusIds[nameCount])
	end
