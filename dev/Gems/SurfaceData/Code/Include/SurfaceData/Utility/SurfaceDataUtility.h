...
namespace SurfaceData
{
        ...
        AZ::Vector3& outNormal)
    {
        ...
        if (vertexCount > 0 && vertexCount % 4 == 0)
        {
            ...
            for (size_t vertexIndex = 0; vertexIndex < vertexCount; vertexIndex += 4)
            {
                ...
                AZ::VectorFloat resultDistance = 0.0f;
        ...
        AZ::Vector3& outNormal)
    {
        ...
        if (pStatObj)
        {
            const Vec3 rayOriginLocal = AZVec3ToLYVec3(meshTransformInverse * rayOrigin);
            const Vec3 rayDirectionLocal = AZVec3ToLYVec3(meshTransformInverse.Multiply3x3(rayDirection).GetNormalized());
            ...
            {
                outPosition = meshTransform * LYVec3ToAZVec3(hitInfo.vHitPos);
                outNormal = meshTransform.Multiply3x3(LYVec3ToAZVec3(hitInfo.vHitNormal)).GetNormalizedExact();
                return true;
            }
        }
