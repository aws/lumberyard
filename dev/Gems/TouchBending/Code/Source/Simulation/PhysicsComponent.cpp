...
namespace TouchBending
{
    namespace Simulation
    {
            ...
            const AZ::Aabb&worldAabb, Physics::ITouchBendingCallback* callback, const void * callbackPrivateData)
        {
            ...
            PxVec3 halfExtents(worldAabb.GetWidth() * 0.5f, worldAabb.GetHeight() * 0.5f, worldAabb.GetDepth() * 0.5f);
            AZ::Vector3 position = worldTransform.GetPosition();
            ...
            worldTransfromWithoutScale.ExtractScaleExact();
            ...
            AZ::Quaternion orientation = AZ::Quaternion::CreateFromTransform(worldTransform);
                                              ...
                                              AZStd::shared_ptr<Physics::SpineTree> spineTreeArchetype)
        {
            ...
            
            {
                ...
                //calls DephysicalizeTouchBendingSkeleton() which ends up removing.
                {
                    ...
                    {
                        //The skeleton is not owned by the engine yet. Once OnPrePhysicsUpdate() is called
        ...
        void PhysicsComponent::OnSystemEvent(ESystemEvent event, UINT_PTR wparam, UINT_PTR lparam)
        {
            switch (event)
            {
