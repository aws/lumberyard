...
namespace Vegetation
{
    ...
    bool AreaSystemComponent::UpdateContext::UpdateSectorWorkLists(PersistentThreadData* threadData, VegetationThreadTasks* vegTasks)
    {
        ...
        if (currViewRect.GetViewRectBounds().IsValid())
        {
            ...
            auto sectorCloserToCenter = [sectorCenterX, sectorCenterY](const SectorId& lhs, const SectorId& rhs)
            {
                float lhsMaxDist = ((lhs.first - sectorCenterX) * (lhs.first - sectorCenterX)) + ((lhs.second - sectorCenterY) * (lhs.second - sectorCenterY));
                float rhsMaxDist = ((rhs.first - sectorCenterX) * (rhs.first - sectorCenterX)) + ((rhs.second - sectorCenterY) * (rhs.second - sectorCenterY));
                return (lhsMaxDist < rhsMaxDist) ||         // return if one sector is closer than the other to the center
                    ((lhsMaxDist == rhsMaxDist) &&          // if it's the same distance...
                    ((lhs.second < rhs.second) ||           // return if the sector Y value is smaller
                        ((lhs.second == rhs.second) && (lhs.first < rhs.first))));  // if the Y value is the same, return if the sector X value is smaller
            };

            AZStd::sort(m_updateWorkList.begin(), m_updateWorkList.end(), [sectorCloserToCenter](const auto& lhs, const auto& rhs)
            {
                ...
                return !sectorCloserToCenter(lhs.first, rhs.first);
            });

            AZStd::sort(m_deleteWorkList.begin(), m_deleteWorkList.end(), [sectorCloserToCenter](const auto& lhs, const auto& rhs)
            {
                ...
                return sectorCloserToCenter(lhs, rhs);
            });
        }
