...
namespace Vegetation
{
    ...
    void AreaBlenderComponent::ClaimPositions(EntityIdStack& stackIds, ClaimContext& context)
    {
        ...
        if (!m_isRequestInProgress)
        {
            ...
            for (const auto& entityId : m_configuration.m_vegetationAreaIds)
            {
                VEG_PROFILE_METHOD(DebugNotificationBus::QueueBroadcast(&DebugNotificationBus::Events::FillAreaStart, entityId, AZStd::chrono::system_clock::now()));
                if (context.m_availablePoints.empty())
                {
                    break;
                }
                ...
                AreaNotificationBus::Event(entityId, &AreaNotificationBus::Events::OnAreaDisconnect);
                VEG_PROFILE_METHOD(DebugNotificationBus::QueueBroadcast(&DebugNotificationBus::Events::FillAreaEnd, entityId, AZStd::chrono::system_clock::now(), context.m_availablePoints.size()));
            }
            m_isRequestInProgress = false;
        }
    }
