...
namespace WhiteBox
{
    ...
    WhiteBoxMeshAssetUndoCommand::WhiteBoxMeshAssetUndoCommand(AzToolsFramework::UndoSystem::URCommandID id)
        : AzToolsFramework::UndoSystem::URSequencePoint(id)
    {
    }
    ...
    void WhiteBoxMeshAssetUndoCommand::Undo()
    {
        ...
        m_asset->SetUndoData(m_undoState);
        MeshAssetNotificationBus::Event(m_asset.GetId(), &MeshAssetNotificationBus::Events::OnAssetModified, m_asset);
    }

    void WhiteBoxMeshAssetUndoCommand::Redo()
    {
        Api::ReadMesh(*m_asset->GetMesh(), m_redoState);
        m_asset->SetUndoData(m_redoState);
        MeshAssetNotificationBus::Event(m_asset.GetId(), &MeshAssetNotificationBus::Events::OnAssetModified, m_asset);
    }
