...
namespace OpenMesh
{
    ...
    template<>
    inline void vector_cast(const AZ::Vector3& src, OpenMesh::Vec3f& dst, GenProg::Int2Type<3>)
    {
    ...
    template<>
    inline void vector_cast(const AZ::Vector2& src, OpenMesh::Vec2f& dst, GenProg::Int2Type<2>)
    {
    ...
    template<>
    inline void vector_cast(const OpenMesh::Vec3f& src, AZ::Vector3& dst, GenProg::Int2Type<3>)
    {
    ...
    template<>
    inline void vector_cast(const OpenMesh::Vec2f& src, AZ::Vector2& dst, GenProg::Int2Type<2>)
    {
        dst.SetX(static_cast<vector_traits<Vec3f>::value_type>(src[0]));
        dst.SetY(static_cast<vector_traits<Vec3f>::value_type>(src[1]));
    }
    ...
    namespace IO
    {
        ...
        struct binary<WhiteBox::FaceHandlesInternal>
        {
            ...
            static size_t size_of(const value_type& _v)
            {
                ...
                const uint32_t count = static_cast<uint32_t>(_v.size());
...
namespace WhiteBox
{
    ...
    namespace Api
    {
            ...
            const AZ::Vector3& normal)
        {
            ...
            const auto found_fh = AZStd::find(faceHandles.cbegin(), faceHandles.cend(), faceHandle);

            if (found_fh == faceHandles.cend())
            {
                const AZ::Vector3 nextNormal = FaceNormal(whiteBox, faceHandle).GetNormalizedExact();
        ...
        FaceHandles SideFaceHandles(const WhiteBoxMesh& whiteBox, const FaceHandle faceHandle)
        {
            ...
            SideFaceHandlesInternal(
                whiteBox, faceHandle, faceHandles, FaceNormal(whiteBox, faceHandle).GetNormalizedExact());
            ...
            const WhiteBoxMesh& whiteBox, const FaceHandles& faceHandles)
        {
            ...
            while (!halfedgeHandles.empty())
            {
                ...
                const auto nextHalfedge = AZStd::find_if(
        ...
        static VertexHandles FacesVertexHandles(const WhiteBoxMesh& whiteBox, const FaceHandles& faceHandles)
        {
            ...
            for (const FaceHandle faceHandle : faceHandles)
            {
                ...
                {
                    const auto vertexIt = AZStd::find(vertexHandles.begin(), vertexHandles.end(), faceVertexHandle);
        ...
        static void CalculatePlanarUVs(WhiteBoxMesh& whiteBox, const FaceHandles& faceHandles)
        {
            AZ_PROFILE_FUNCTION(AZ::Debug::ProfileCategory::AzToolsFramework);
        ...
        PolygonHandle AddPolygon(WhiteBoxMesh& whiteBox, const FaceVertHandlesList& faceVertHandles)
        {
            ...
            const PolygonHandle polygonHandle = PolygonHandleFromInternal(polygon);
        ...
        PolygonHandles InitializeAsUnitCube(WhiteBoxMesh& whiteBox)
        {
            ...
            // generate faces
            const PolygonHandles polygonHandles
                {// top
        ...
        PolygonHandle InitializeAsUnitTriangle(WhiteBoxMesh& whiteBox)
        {
            ...
            // generate faces - front
            const auto polygonHandle = AddTriPolygon(whiteBox, vertexHandles[0], vertexHandles[1], vertexHandles[2]);
            ...
            const PolygonHandle& selectedPolygonHandle, const PolygonHandle& adjacentPolygonHandle)
        {
            ...
            {
                auto foundEdgeHandleInSelectedPolygon =
            ...
            FaceVertHandlesCollection& vertsForLinkingAdjacentPolygons)
        {
                ...
                foundHalfedgeHandle.IsValid())
            {
                ...
                auto faceHandleInAdjacentPolygon = AZStd::find(
            ...
            FaceVertHandlesCollection& vertsForLinkingAdjacentPolygons)
        {
            ...
            for (const auto faceHandle : EdgeFaceHandles(whiteBox, edgeHandle))
            {
                        ...
                        faceHandle) == selectedPolygonHandle.m_faceHandles.cend())
                {
                    ...
                    const AZ::VectorFloat angleCosine = adjacentPolygonNormal.Dot(selectedPolygonNormal);
                    ...
                    if (angleCosine.IsClose(AZ::VectorFloat::CreateZero(), AdjacentPolygonNormalTolerance))
                    {
            ...
            FaceVertHandlesCollection& vertsForLinkingAdjacentPolygons)
        {
            ...
            for (size_t index = 0; index < borderVertexHandles.size(); ++index)
            {
                ...
                const auto currentVertexHandlePairIt = AZStd::find_if(
                ...
                const auto nextVertexHandlePairIt = AZStd::find_if(
                    appendedVerts.m_vertexHandlePairs.cbegin(), appendedVerts.m_vertexHandlePairs.cend(),
                ...
                const auto edgeHandleIt = AZStd::find_if(
        ...
        static void RemoveFaces(WhiteBoxMesh& whiteBox, const FaceHandles& faceHandles)
        {
            AZ_PROFILE_FUNCTION(AZ::Debug::ProfileCategory::AzToolsFramework);
        ...
        static AZStd::vector<FaceVertHandles> BuildNewVertexFaceHandles(
            WhiteBoxMesh& whiteBox, const Internal::AppendedVerts& appendedVerts, const FaceHandles& existingFaces)
        {
            AZ_PROFILE_FUNCTION(AZ::Debug::ProfileCategory::AzToolsFramework);
            ...
            for (const FaceHandle faceHandle : existingFaces)
            {
                ...
                for (const VertexHandle vertexHandle : vertexHandles)
                {
                    ...
                    const auto vertexHandlePairIt = AZStd::find_if(
                        appendedVerts.m_vertexHandlePairs.cbegin(), appendedVerts.m_vertexHandlePairs.cend(),
            ...
            AppendVertFn&& appendFn)
        {
            ...
            const AZ::VectorFloat angleCosine = extrudeDirection.Dot(polygonNormal);
            ...
            const bool appendAll = angleCosine.IsGreaterEqualThan(AZ::VectorFloat::CreateZero());
                            ...
                            polygonHalfedgeHandles.cend())
                    {
                        ...
                        const bool adjacentFaceAndPolygonNormalOrthogonal =
                            faceNormal.Dot(polygonNormal)
                                .GetAbs()
                                .IsClose(AZ::VectorFloat::CreateZero(), AdjacentPolygonNormalTolerance);
            ...
            WhiteBoxMesh& whiteBox, const PolygonHandle& polygonHandle, AppendVertexFn&& appendFn)
        {
            ...
            // add top polygon
            const PolygonHandle newPolygonHandle = AddPolygon(whiteBox, topFacesToAdd);
        ...
        static AZ::Transform BuildSpace(const AZ::Vector3& normal, const AZ::Vector3& pivot)
        {
            ...
            return AZ::Transform::CreateFromColumns(axis1, axis2, normal, pivot);
        }
        ...
        {
            const auto vertexPositions = EdgeVertexPositions(whiteBox, edgeHandle);
            return BuildSpace((vertexPositions[1] - vertexPositions[0]).GetNormalizedSafeExact(), pivot);
        }
        ...
        bool WriteMesh(const WhiteBoxMesh& whiteBox, AZStd::vector<AZ::u8>& output)
        {
            ...
            {
                const std::string s = whiteBoxStream.str();
                output.clear();
                output.reserve(s.size());
                AZStd::copy(s.cbegin(), s.cend(), AZStd::back_inserter(output));
        ...
        bool ReadMesh(WhiteBoxMesh& whiteBox, const AZStd::vector<AZ::u8>& input)
        {
            ...
            AZStd::copy(input.cbegin(), input.cend(), AZStd::back_inserter(s));

            std::stringstream whiteBoxStream;
            whiteBoxStream.str(s);

            {
                AZStd::lock_guard lg(g_omSerializationLock);

                OpenMesh::IO::Options options{OpenMesh::IO::Options::FaceTexCoord | OpenMesh::IO::Options::FaceNormal};
                if (OpenMesh::IO::read_mesh(whiteBox.mesh, whiteBoxStream, ".om", options))
                {
                    return true;
                }
                else
