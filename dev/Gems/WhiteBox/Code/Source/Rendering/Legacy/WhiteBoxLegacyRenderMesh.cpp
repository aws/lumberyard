...
namespace WhiteBox
{
    ...
    public:
        ...
        ICharacterInstance* GetEntityCharacter(
            unsigned int slot = 0, Matrix34A* matrix = nullptr, bool returnOnlyVisible = false) override;
    ...
    IStatObj* LegacyRenderNode::GetEntityStatObj(
        unsigned int partId, unsigned int subPartId, Matrix34A* matrix, bool /*returnOnlyVisible*/)
    {
        if (partId == 0)
        {
            if (matrix)
    ...
    /*IRenderNode*/
    ICharacterInstance* LegacyRenderNode::GetEntityCharacter(
        unsigned int /*slot*/, Matrix34A* matrix, bool returnOnlyVisible)
    {
        return nullptr;
    }
    ...
    void LegacyRenderMesh::BuildMesh(const WhiteBoxRenderData& renderData, const AZ::Transform& worldFromLocal)
    {
