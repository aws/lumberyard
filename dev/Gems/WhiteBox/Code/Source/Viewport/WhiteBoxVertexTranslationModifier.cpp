...
namespace WhiteBox
{
        ...
        const AZStd::vector<AZStd::pair<AZ::Vector3, AZ::Vector3>>& edgeBeginEnds)
    {
        ...
        int64_t axisIndex = VertexTranslationModifier::InvalidAxisIndex;
        ...
        for (size_t actionIndex = 0; actionIndex < action.m_actions.size(); ++actionIndex)
        {
            ...
            const auto worldStart = worldFromLocal * edgeAxis.first;
            const auto worldEnd = worldFromLocal * edgeAxis.second;
        ...
        m_translationManipulator->InstallLeftMouseUpCallback(
            [this, sharedState,
             translationManipulator = AZStd::weak_ptr<MultiLinearManipulator>(m_translationManipulator)](
                const MultiLinearManipulator::Action& action)
            {
    ...
    void VertexTranslationModifier::OnTick(float deltaTime, AZ::ScriptTimePoint time)
    {
