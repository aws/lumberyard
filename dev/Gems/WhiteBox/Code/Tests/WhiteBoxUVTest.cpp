...
namespace UnitTest
{
    ...
    float GenerateNoiseWithSignificantFigures(AZ::u32 afterDecimalPlace)
    {
        ...
        const double firstSigFig = azlossy_cast<AZ::u64>(rndFirstSigFig(gen)) * std::pow(10.0, numSigFigs - 1);

        // random value for the other significant digits
        const double otherSigFigs = azlossy_cast<AZ::u64>(rndOtherSigFigs(gen) * std::pow(10.0, numSigFigs - 1));
