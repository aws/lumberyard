...
def use_rec(self, name, **kw):
	...
	for x in self.to_list(getattr(y, 'use', [])):
		# According to this discussion https://groups.google.com/d/topic/waf-users/WlCTS3k4JvM/discussion
		# the prevention of STLIB_ propagation was to prevent some duplicate symbol issues that would
		# cause *some* platforms to crash at run-time.  Seems overly aggressive to prevent this
		# propagation in all cases, such as a static library that depends on another static library.
		if self.env["STLIB_" + x] and y.tmp_use_var != 'STLIB':
			continue
		try:
			p[x].append(name)
		except KeyError:
...
def process_use(self):
	...
	for x in names:
		try:
			...
			if self.env['STLIB_' + x] and self._type != 'stlib':
				continue

			if not x in self.uselib:
				self.uselib.append(x)
		else:
			for k in self.to_list(getattr(y, 'use', [])):
				if self.env['STLIB_' + k] and y._type != 'stlib':
					continue

				if not k in self.uselib:
