...
def python_path(*args, **kwargs):
    if platform.system() == 'Windows':
        return dev_path('Tools', 'Python', 'python3.cmd')
    else:
        return dev_path('Tools', 'Python', 'python3.sh')
    ...
    if os.path.isfile(imports_file_path):
        ...
        for line in lines:
            ...
            if len(parts) != 2 or not parts[0] or not parts[1]:
                raise RuntimeError(
                    'Invalid import "{}" found in {}. Import should have the format <gem-name>.<common-code-subdirectory-name>.'.format(
                        line, imports_file_path))

            gem_name, import_directory_name = parts

            if gem_name != 'CloudGemFramework':
                raise RuntimeError(
                    'The {} gem is not supported for resource manager imports as requested by {}. Only modules from the CloudGemFramework gem can imported.'.format(
                        gem_name, imports_file_path))
            ...
            if not os.path.isdir(path):
                raise RuntimeError(
                    'The {} gem does not provide the {} import as found in {}. The {} directory does not exist.'.format(
                        gem_name, line, imports_file_path, path))
